...
This module's outputs are the ground-truth wLLM verifies its
...
-----
    ...
    from wllm.native.refs.pi05_cfg_reference import Pi05CFGReference
...
# ACP prompt format — matches wllm_native.core.rl.acp_tags. Duplicated
# here so the reference has zero dependency on wllm_native internals.
...
class Pi05CFGReference:
    ...
    are exposed for fine-grained correctness checks against wLLM's
    ...
    def infer(
        ...
        *,
        ...
        # Initial noise. To compare apples-to-apples with wLLM, the
        ...
        # robot DOF) so the returned ``actions`` match what wLLM's
