...
and pointer is expressed through :class:`wllm_native.core.cuda_buffer.CudaBuffer`
so the JAX Pi0.5 FP4 frontend can consume the same ``wllm_native_fp4`` kernels
...
def pick_variant(N: int, K: int) -> int:
    ...
    ``wllm_native.executors.fp4_utils.pick_variant``.
    """
    if N >= 16384:
