...
class VLAModel:
        ...
        *,
        ...
        if not hasattr(self._pipe, "calibrate"):
            raise NotImplementedError(
                ...
                "Upgrade to a recent version of wLLM that includes "
    ...
    def recalibrate(self):
        ...
        """
        from wllm.native.core.quant.calibrator import clear_calibration
        clear_calibration(self._pipe._checkpoint_path)
        if self._framework == "jax":
            from wllm.native.core.weights.weight_cache import clear_weight_cache
...
def load_model(checkpoint, framework="torch", num_views=2, autotune=3,
    ...
    Args:
        ...
        hardware: GPU backend selection. ``"auto"`` (default) detects the
            ...
            best-matching backend:
              SM110 (Jetson Thor)  → ``wllm_native.hardware.thor.*``
              SM120 (RTX 5090)     → ``wllm_native.hardware.rtx.*``
              ...
              SM89  (RTX 4090)     → ``wllm_native.hardware.rtx.*``
              SM87  (Jetson Orin)  → ``wllm_native.hardware.rtx.*`` (experimental,
        ...
        use_fp4: Pi0.5 torch and JAX on Thor. If True, enable NVFP4
            ...
            GEMMs). Requires SM100+ GPU (Thor SM110) and the wllm_native_fp4
        ...
        state_prompt_fixed_max_len: Pi0.5 Thor fixed mode only. Padded
            ...
            Env override: ``WLLM_PI05_STATE_PROMPT_FIXED_MAX_LEN``.
    ...
    if config == "qwen3_vl":
        raise NotImplementedError(
            ...
            "    from wllm.native.frontends.torch.qwen3_vl_fp8_sm89_multimodal "
            "import Qwen3VlFp8Sm89Frontend\n"
            "    from wllm.native.frontends.torch.qwen3_vl_rtx import "
    ...
    if config == "nexn2":
        raise NotImplementedError(
            ...
            "    from wllm.native.frontends.torch.nexn2_rtx import "
    ...
    if use_fp16:
        if config == "pi05" and framework == "torch" and arch == "thor":
            ...
            from wllm.native.frontends.torch.groot_thor_fp16 import (
            ...
            from wllm.native.frontends.torch.groot_n17_thor_fp16 import (
                GrootN17TorchFrontendThorFP16,
            )
            pipe_cls = GrootN17TorchFrontendThorFP16
        else:
            if config == "pi05":
                from wllm.native.frontends.torch.pi05_rtx_fp16 import (
                    Pi05TorchFrontendRtxFP16,
                )
                pipe_cls = Pi05TorchFrontendRtxFP16
            elif config == "groot":
                from wllm.native.frontends.torch.groot_rtx_fp16 import (
                    GrootTorchFrontendRtxFP16,
                )
                pipe_cls = GrootTorchFrontendRtxFP16
            else:  # config == "groot_n17"
                if arch == "rtx_sm89":
                    from wllm.native.frontends.torch.groot_n17_rtx_sm89_fp16 import (
                        GrootN17TorchFrontendRtxSm89FP16,
                    )
                    pipe_cls = GrootN17TorchFrontendRtxSm89FP16
                else:
                    from wllm.native.frontends.torch.groot_n17_rtx_fp16 import (
    ...
    if use_fp4:
        if config != "pi05" or framework not in ("torch", "jax") or arch != "thor":
            ...
            try:
                import wllm.native.wllm_fp4 as _fvk_fp4
                if not _fvk_fp4.has_nvfp4():
                    logger.warning(
                        "wllm_native_fp4 loaded but has_nvfp4()=False (SM100+ required). "
                        "Falling back to FP8.")
                    use_fp4 = False
            except ImportError:
                logger.warning(
                    "wllm_native_fp4 extension not available. Falling back to FP8.")
                use_fp4 = False

            if use_fp4:
                if framework == "torch":
                    from wllm.native.frontends.torch.pi05_thor_fp4 import (
                        Pi05TorchFrontendThorFP4,
                    )
                    pipe_cls = Pi05TorchFrontendThorFP4
                else:  # framework == "jax"
                    from wllm.native.frontends.jax.pi05_thor_fp4 import (
