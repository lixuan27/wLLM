...
to the current action chunk. Pi0.5's current wLLM pipeline is an inference
...
class DenoiserVjpProvider(Protocol):
    ...
    ``wllm_native.subgraphs.capture.register_captured_graph`` or
...
def enable(target: object, *, provider: DenoiserVjpProvider | None = None,
    ...
    A provider is mandatory. The stock Pi0.5 wLLM inference pipeline has no
    autograd/VJP surface, and exporting this plan without a real provider would
    be a false implementation.
    """
    from wllm.native.subgraphs.pi05.context_action import enable as _context_enable
    from wllm.native.subgraphs.pi05 import stage_plans as _stage_plans  # noqa: F401

    if provider is None:
        raise RuntimeError(
            "context_rtc_vjp_guided_action requires a DenoiserVjpProvider. "
            "The current Pi0.5 wLLM inference pipeline has no built-in "
