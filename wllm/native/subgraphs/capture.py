...
def register_capture_hook(pipeline: object, hook: CaptureHook) -> None:
    hooks = getattr(pipeline, "_wllm_subgraph_capture_hooks", None)
    if hooks is None:
        hooks = []
        setattr(pipeline, "_wllm_subgraph_capture_hooks", hooks)
...
def register_frontend_capture_hook(frontend: object,
    ...
    hooks = getattr(frontend, "_wllm_subgraph_frontend_hooks", None)
    if hooks is None:
        hooks = []
        setattr(frontend, "_wllm_subgraph_frontend_hooks", hooks)
...
def apply_frontend_capture_hooks(frontend: object) -> None:
    ...
    if pipeline is None:
        return
    for installer in list(getattr(frontend, "_wllm_subgraph_frontend_hooks", ())):
        installer(pipeline)


def run_capture_hooks(pipeline: object, stream_handle: object,
                      stream_int: int) -> None:
    for hook in list(getattr(pipeline, "_wllm_subgraph_capture_hooks", ())):
        hook(pipeline, stream_handle, stream_int)
...
def register_export_graph(pipeline: object, name: str, graph: object, *,
    ...
    records = getattr(pipeline, "_wllm_subgraph_export_graphs", None)
    if records is None:
        records = []
        setattr(pipeline, "_wllm_subgraph_export_graphs", records)
...
def register_captured_graph(pipeline: object, name: str, cuda_graph: object, *,
    ...
    if len(variants) != 1:
        raise ValueError(
            ...
            "exec graph")
    records = getattr(pipeline, "_wllm_subgraph_captured_graphs", None)
    if records is None:
        records = []
        setattr(pipeline, "_wllm_subgraph_captured_graphs", records)
    records[:] = [r for r in records if r.name != name]
...
def materialize_captured_graphs(pipeline: object) -> None:
    ...
    if ctx is None:
        return
    for rec in getattr(pipeline, "_wllm_subgraph_captured_graphs", ()):
        if rec.materialized:
            continue
...
def export_graph_records(pipeline: object) -> list[ExportGraphRecord]:
    return list(getattr(pipeline, "_wllm_subgraph_export_graphs", ()))
