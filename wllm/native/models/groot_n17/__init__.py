...
imported lazily so that ``import wllm.native.models.groot_n17`` does not
pull in torch.
"""
...
def __getattr__(name: str):
    if name in ("calibrate_pipeline_amax", "amax_to_dev_scale"):
        from wllm.native.models.groot_n17 import calibration
        return getattr(calibration, name)
    if name in ("RopeConfig", "build_cos_sin_tables",
                "build_position_ids_for_segments"):
        from wllm.native.models.groot_n17 import mrope_table
