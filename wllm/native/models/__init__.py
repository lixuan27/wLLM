"""wLLM — model-specific pipeline and calibration code.
...
``wllm_native.hardware.{rtx,thor}``; frontends (weight loading, graph
capture, framework glue) live in ``wllm_native.frontends.{torch,jax}``.
"""
