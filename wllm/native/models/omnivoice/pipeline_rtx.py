"""
OmniVoice wLLM Engine — flashcli bundle module.
...
Architecture:
  wLLMLlm       — FP4 W4A4 forward (CUDA Graph + FA2 + fused kernels)
  wLLMLlmBF16   — BF16 forward (wLLM kernels, no quantization)
  inject()         — Patch OmniVoice model for wLLM acceleration
...
# ── wLLM kernel detection (supports flashcli deployment + local dev) ──
try:
    from wllm.native import wllm_native_kernels as _fvk
except ImportError:
    try:
        import wllm_native_kernels as _fvk
    except ImportError:
        _fvk = None

try:
    from wllm.native import wllm_native_omnivoice as _fvo
except ImportError:
    try:
        import wllm_native_omnivoice as _fvo
    except ImportError:
        _fvo = None
...
# ═══════════════════════════════════════════════════════════════
# wLLMLlm — FP4 W4A4 forward engine
# ═══════════════════════════════════════════════════════════════

class wLLMLlm:
    """wLLM FP4 LLM — NVFP4 W4A4 GEMM + fused QK norm+rope + FA2 + CUDA Graph."""

    def __init__(self, qm, dev="cuda:0"):
        self.device = dev
        layers = qm.layers
        from wllm.native import wllm_native_kernels as fvk
        from wllm.native import wllm_native_omnivoice as fvo
...
# ═══════════════════════════════════════════════════════════════
# wLLMLlmBF16 — BF16 forward engine (no quantization)
# ═══════════════════════════════════════════════════════════════

class wLLMLlmBF16(wLLMLlm):
    """BF16 wLLM — fused kernels + FA2, zero quantization overhead."""
...
def _check_kernels():
    """Verify all required wLLM kernel modules are available.
    ...
    missing = []
    if _fvk is None:
        missing.append("wllm_native_kernels — wLLM base not installed")
    else:
        for sym in (
            ...
            if not hasattr(_fvk, sym):
                missing.append("wllm_native_kernels." + sym)

    if _fvo is None:
        missing.append(
            "wllm_native_omnivoice not found; "
            "rebuild wLLM with: cmake -DWLLM_ENABLE_OMNIVOICE=ON -DGPU_ARCH=120")
    else:
        for sym in ("omnivoice_cfg_logsoftmax_bf16", "omnivoice_qk_norm_rope_bf16"):
            if not hasattr(_fvo, sym):
                missing.append("wllm_native_omnivoice." + sym)

    if missing:
        raise RuntimeError(
            "OmniVoice wLLM engine: required kernel symbols missing.\n"
            + "\n".join("  missing: " + s for s in missing)
        )

def inject(m, cfg_ratio=0.05, bookend=False):
    """Patch OmniVoice model for wLLM acceleration.
    ...
    _frt_bf16 = wLLMLlmBF16(m.llm, str(m.device))
    _orig = m.llm.forward
    _frt_fp4 = wLLMLlm(m.llm, str(m.device))
