"""wLLM -- MelBandRoformer FP8 kernelized inference pipeline.
...
from ``wllm_native_kernels``. The STFT / band-split / band-merge / ISTFT
...
Kernel surface used (all from ``wllm_native_kernels``):
...
The ``mbr_*`` kernels require ``-DWLLM_ENABLE_MELBAND_ROFORMER=ON`` at
...
def _load_kernels():
    """Import ``wllm_native_kernels`` and validate that all ``mbr_*`` symbols exist.
    ...
    build did not include ``-DWLLM_ENABLE_MELBAND_ROFORMER=ON``).
    """
    try:
    ...
    if missing:
        raise RuntimeError(
            "MelBandRoformer kernels are not compiled into wllm_native_kernels. "
            "Rebuild with:  cmake .. -DWLLM_ENABLE_MELBAND_ROFORMER=ON && make -j\n"
