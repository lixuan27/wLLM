...
# wllm_native_kernels build; a build missing either variant must fail fast here
...
def _load_kernels(required_symbols=None):
    """Import wllm_native_kernels and validate required symbols.

    Args:
        required_symbols: tuple of symbol names to check (default: NVFP4
            precision surface plus the shared block-fusion surface)

    Returns the wllm_native_kernels module. Raises RuntimeError if any required
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "wllm_native_kernels is not available. Build wLLM with:\n"
                "    cmake -S . -B build -DGPU_ARCH=120 -DCMAKE_BUILD_TYPE=Release\n"
                "    cmake --build build -j --target wllm_native_kernels\n"
                "After building, the .so file is placed in wllm_native/ directly "
    ...
    if missing:
        names = ", ".join(missing)
        raise RuntimeError(
            f"wllm_native_kernels is missing required symbols: {names}\n"
            "Rebuild wLLM for your target hardware. For the Blackwell NVFP4 "
            "kernels use GPU_ARCH=120/121 (auto-enables NVFP4):\n"
            "    cmake -S . -B build -DGPU_ARCH=120 -DCMAKE_BUILD_TYPE=Release\n"
            "    cmake --build build -j --target wllm_native_kernels\n"
            "FP8 kernels are part of the default build. After building, the .so "
            "lands in wllm_native/ (pip install -e . makes it importable).")
