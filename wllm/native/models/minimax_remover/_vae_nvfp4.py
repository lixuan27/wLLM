"""wLLM — MiniMax-Remover WanVAE NVFP4 (W4A4) conv3d integration.

Replaces eligible 3×3×3 WanCausalConv3d layers in the WanVAE with an
NVFP4 (W4A4) path that uses:

  1. **wLLM fp16_quant_nvfp4_ndhwc** (novel CUDA kernel in this repo):
...
Env knobs:
  WLLM_NVFP4_VAE=1            enable (default ON)
  WLLM_NVFP4_VAE_DECODE_ONLY=0  only patch decoder (default 0 = enc+dec)
  WLLM_NVFP4_VAE_MIN_CI=192   minimum Ci for FP4 (default 192)
"""
...
# Default ON (WLLM_NVFP4_FUSED_NORMQUANT=1): the fused kernel emits the
...
# Set WLLM_NVFP4_FUSED_NORMQUANT=0 to fall back to the separate
# norm→fp16→quant path (Direction-2 cache only).

# Lazy-loaded kernel modules
_fvk = None       # wllm_native_minimax_remover (our new kernels)
_frk = None       # wllm_native_kernels (motus FP4 conv3d + weight quant)
...
def _load_kernels():
    ...
    try:
        ...
        try:
            ...
            logger.error("[nvfp4_vae] wllm_native_minimax_remover not found")
            return False
    try:
        from wllm.native import wllm_native_kernels as _frk
    except ImportError:
        import wllm_native_kernels as _frk
    if not hasattr(_frk, 'quantize_bf16_to_nvfp4'):
        logger.error("[nvfp4_vae] quantize_bf16_to_nvfp4 not in wllm_native_kernels")
        return False
    if not hasattr(_fvk, 'nvfp4_conv3d_ndhwc_fp16out'):
        logger.error("[nvfp4_vae] nvfp4_conv3d_ndhwc_fp16out not in wllm_native_minimax_remover")
        return False
    return True
...
def _nvfp4_conv3d_forward(self, x: torch.Tensor, cache_x=None):
    ...
    # Set WLLM_NVFP4_NO_CACHE=1 to disable (always quantize per call)
    use_fp4_cache = os.environ.get('WLLM_NVFP4_NO_CACHE', '0') != '1'
...
def install_vae_nvfp4(vae, enabled: bool = True) -> Dict:
    ...
    if os.environ.get('WLLM_NVFP4_VAE', '1') != '1':
    ...
    decode_only = os.environ.get('WLLM_NVFP4_VAE_DECODE_ONLY', '0') == '1'
    min_ci = int(os.environ.get('WLLM_NVFP4_VAE_MIN_CI', '192'))
    ...
    WanCausalConv3d._wllm_nvfp4_patched = True
    ...
    # 35.23 dB vs 35.11 baseline. Disable with WLLM_NVFP4_FUSED_NORMQUANT=0.
    no_fused = os.environ.get('WLLM_NVFP4_FUSED_NORMQUANT', '1') == '0'
    try:
        from diffusers.models.autoencoders.autoencoder_kl_wan import (
            WanResidualBlock)
