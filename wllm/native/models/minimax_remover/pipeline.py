"""wLLM -- MiniMax-Remover NVFP4 kernelized inference pipeline.
...
wLLM kernels, fused fp32-stat Triton norm/RoPE, kernel attention and
...
* the N-step Python denoise loop -> a single captured CUDA Graph
  (``WLLM_MANUAL_GRAPH=1``); inside the graph there are zero torch
...
``wllm_native_kernels``. They are gated by the Blackwell NVFP4 build option
...
from wllm.native.models.minimax_remover._fp8_pipeline import MiniMaxRemoverPipelineFP8
from wllm.native.models.minimax_remover._utils import (
...
def _import_runtime():
    ...
    from ._nvfp4_linear import install_wllm_nvfp4
    from ._attention import install_attention
    from ._manual_denoise import ManualRemoverPipeline
    return install_wllm_nvfp4, install_attention, ManualRemoverPipeline
...
class MiniMaxRemoverPipeline:
    ...
    def __init__(self, pipe, num_inference_steps=12, fp4_target="all",
        ...
        install_wllm_nvfp4, install_attention, ManualRemoverPipeline = _import_runtime()
        self.pipe = pipe
        self.transformer = pipe.transformer
        self.num_inference_steps = num_inference_steps

        n_lin = install_wllm_nvfp4(self.transformer, self.fvk,
        ...
        pipe._wllm_wrapper = _FrozenMarker()
    ...
    def _install_smart_call(self):
        ...
        def _smart_call(self, *args, **kwargs):
            w = getattr(self, "_wllm_wrapper", None)
            m = getattr(self, "_manual_pipeline", None)
            ns = os.environ.get("WLLM_NUM_STEPS")
            if ns:
