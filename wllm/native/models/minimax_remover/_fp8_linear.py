"""
wLLM FP8 Linear layer.

Implements true FP8 GEMM acceleration using wLLM-compiled CUDA kernels:
...
Calibration workflow:
  1. install_wllm_fp8(transformer)  -> replace all Linears with wLLMFp8Linear
...
# Optional: wLLM MiniMax-Remover extra kernel module. Used for the
...
_fvk_extra = None
try:
    from wllm.native import wllm_native_minimax_remover as _fvk_extra
except ImportError:
    try:
        import wllm_native_minimax_remover as _fvk_extra  # type: ignore
    except ImportError:
        _fvk_extra = None
...
class wLLMFp8Linear(nn.Module):
    """Linear layer backed by the wLLM FP8 GEMM.
    ...
    @classmethod
    def from_linear(cls, linear: nn.Linear) -> "wLLMFp8Linear":
    ...
    # one kernel (bias_gelu_quant_fp16_fp8 in wllm_native_minimax_remover).
...
def install_wllm_fp8(model: nn.Module, verbose: bool = True, target: str = "all") -> int:
    """Recursively replace Linears in the model with wLLMFp8Linear.
    ...
    for name, linear in targets:
        ...
        attr = parts[-1]
        new_layer = wLLMFp8Linear.from_linear(linear)
    ...
    if verbose:
        logger.info("  Replaced %d Linears -> wLLMFp8Linear", replaced)
    return replaced


def set_calibration(model: nn.Module, on: bool):
    """Toggle calibration mode on all wLLMFp8Linear modules."""
    for module in model.modules():
        if isinstance(module, wLLMFp8Linear):
            module.calibrating = on


def freeze_calibration(model: nn.Module, margin: float = 1.0):
    """Freeze the static act_scale on all wLLMFp8Linear modules."""
    n = 0
    for module in model.modules():
        if isinstance(module, wLLMFp8Linear):
