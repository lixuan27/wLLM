"""wLLM -- MiniMax-Remover FP8 manual graph-capturable denoise pipeline.
...
The code is retained (gated behind ``WLLM_FP8_GRAPH=1``, default off)
because it is correct and would become worthwhile if a fast graph-safe
attention backend is added (e.g. wLLM's vendored ``wllm_native_fa2``,
...
class FP8ManualDenoise:
    ...
    def _capture_graph(self, latents, masked_latents, masks_latents,
        ...
        s = torch.cuda.Stream()
        n_warmup = int(os.environ.get("WLLM_FP8_GRAPH_WARMUP", "1"))
