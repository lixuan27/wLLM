"""wLLM -- MiniMax-Remover VAE optimization: fp16-native fused kernels.

Replaces diffusers WanRMS_norm.forward (4 full-tensor fp32 passes,
~0.45 ms each at [1,384,1,240,432]) with the wLLM
...
# (wllm_native_minimax_remover) that is opt-in:
...
def _wllm_fp16_rms_norm_forward(self, x: torch.Tensor) -> torch.Tensor:
    """wLLM fp16-native RMS_norm replacement for WanRMS_norm.forward.
...
def install_wllm_fp16_rms_norm(vae) -> int:
    """Replace WanRMS_norm.forward (attention sites) with the wLLM fp16
    ...
    if not getattr(WanRMS_norm, "_wllm_fp16_patched", False):
        WanRMS_norm._orig_forward = WanRMS_norm.forward
        WanRMS_norm.forward = _wllm_fp16_rms_norm_forward
        WanRMS_norm._wllm_fp16_patched = True
        logger.info("[minimax-vae] patched WanRMS_norm.forward -> wLLM "
...
def _install_wan_upsample_no_cast(vae) -> int:
    ...
    if not getattr(WanUpsample, "_wllm_nocast", False):
...
def install_vae_optimizations(vae, dtype=None, use_fp8_conv: bool = True) -> Dict:
    ...
    ops are replaced with wLLM fp16 CUDA kernels, and applicable 3x3x3
    conv3d layers are replaced with FP8 implicit-GEMM kernels (fp16 in/out,
    FP8 e4m3 internal compute).

    Requires the standalone ``wllm_native_minimax_remover`` module, built with:
        cmake -DWLLM_ENABLE_MINIMAX_REMOVER=ON -DGPU_ARCH=120 ...
    ...
    Raises:
        ImportError if wllm_native_minimax_remover is not built.
    """
    if _fvk is None:
        raise ImportError(
            "wllm_native_minimax_remover not found; rebuild wLLM with: "
            "cmake -DWLLM_ENABLE_MINIMAX_REMOVER=ON -DGPU_ARCH=120 ...")
    n_fused_blocks = install_wllm_fp16_rms_norm(vae)
...
def _install_channels_last_pipeline(vae) -> int:
    ...
    to preserve the format, and replaces the wLLM norm kernels with
    ...
    if not getattr(WanCausalConv3d, "_wllm_cl_fwd", False):
        ...
        WanCausalConv3d._wllm_cl_fwd = True
    ...
    def _cl_rms_norm_forward(self, x):
        if x.dim() == 4:
            ...
            return _wllm_fp16_rms_norm_forward(self, x)
        B, C, T, H, W = x.shape
        if x.dtype != _FP16:
            x = x.to(_FP16)
    ...
    class _FusedRmsSiluCL(nn.Module):
        ...
        def forward(self, x):
            if x.dim() == 4:
                return F.silu(_wllm_fp16_rms_norm_forward(
                    type('_DummyNorm', (), {
    ...
    WanRMS_norm._wllm_cl_norm = True
...
# ────────────────────────────────────────────────────────────────────────
_FUSE_FP8_NORMQUANT = os.environ.get("WLLM_FP8_FUSED_NORMQUANT", "1") == "1"
...
def _install_fp8_conv3d_pipeline(vae, enabled: bool = True) -> int:
    ...
    WanCausalConv3d._wllm_fp8_patched = True
