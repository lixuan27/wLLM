"""wLLM -- MiniMax-Remover manual pointer-based denoise pipeline.
...
5. Manual block forward (``block_forward_fused``): only kernel calls --
   Triton adaLN, kernel attention, NVFP4 GEMM, Triton gate-mul, wLLM
...
``wllm_native_kernels`` module is passed in (``kern``) so importing this
...
_USE_FUSED_BLOCK = os.environ.get("WLLM_FUSED_BLOCK", "1") == "1"
...
class ManualRemoverPipeline:
    ...
    ``WLLM_MANUAL_GRAPH=1``) and replays it on subsequent calls with
    ...
    ``kern`` is the validated ``wllm_native_kernels`` module.
    """
    ...
    def __call__(self, images, masks, num_frames, height, width,
        ...
        use_graph = os.environ.get("WLLM_MANUAL_GRAPH", "0") == "1"
