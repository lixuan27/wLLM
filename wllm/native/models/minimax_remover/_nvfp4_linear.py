"""wLLM -- MiniMax-Remover NVFP4 (W4A4) Linear layer.

Replaces every eligible ``nn.Linear`` in the Transformer3DModel with a
NVFP4 W4A4 GEMM backed by the generic wLLM NVFP4 kernels:
...
def _pick_gemm(kern):
    mode = os.environ.get("WLLM_FP4_GEMM", "pingpong").lower()
    if mode == "plain":
...
class wLLMNvfp4Linear(nn.Module):
    """Linear layer backed by the wLLM NVFP4 (W4A4) GEMM.
...
def install_wllm_nvfp4(model, kern, verbose=False, target="all"):
    ...
    for name, linear in targets:
        ...
        attr = name.split(".")[-1]
        setattr(parent, attr, wLLMNvfp4Linear.from_linear(linear, kern))
        replaced += 1
...
def set_calibration(model, on):
    for module in model.modules():
        if isinstance(module, wLLMNvfp4Linear):
