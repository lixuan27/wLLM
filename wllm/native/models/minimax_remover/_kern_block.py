"""
wLLM pure-kernel Transformer block fusion (FP8 path).
...
``WLLM_ATTN_MODE``); all Linear layers still run through the wLLM
FP8 GEMM installed by ``install_wllm_fp8``.
...
Fusion points (per block, compared to the original video_subtitle_remover.py):
  ...
  This version:
            ...
            gelu_inplace_fp16      -> 1 in-place wLLM kernel (tanh gelu)

Key correctness details:
  * patch_embedding(...).transpose(1,2) produces a **non-contiguous** output; wLLM's
  ...
  * The default ``WLLM_NORM_MODE=triton`` uses ``ada_layernorm_fp16_io`` which
    accumulates mean/var in fp32 across three passes -- bit-exact with the original
    FP32LayerNorm, unlike wLLM's generic ``ada_layer_norm_fp16`` (the optional
    ``WLLM_NORM_MODE=fp16`` debug path, which fails fast if that symbol is absent).
...
from wllm.native import wllm_native_kernels as kern
...
_fvk = None
try:
    from wllm.native import wllm_native_minimax_remover as _fvk
except ImportError:
    try:
        import wllm_native_minimax_remover as _fvk
    except ImportError:
        pass
...
def install_fused_blocks(transformer, norm_mode=None, gelu_mode=None):
    """Replace each TransformerBlock.forward with the pure wLLM kernel-fused version.
    ...
    if norm_mode is None:
        norm_mode = _os.environ.get("WLLM_NORM_MODE", "triton")
    if gelu_mode is None:
        gelu_mode = _os.environ.get("WLLM_GELU_MODE", "inplace")
    ...
    def _ada_norm(self_hs, scale_v, shift_v, S, D):
        ...
        Why not use wLLM's ada_layer_norm_fp16: its statistics are insufficiently
    ...
    def _ada_norm_wllm_fp16(self_hs, scale_v, shift_v, S, D):
        if not hasattr(kern, "ada_layer_norm_fp16"):
            raise RuntimeError(
                "WLLM_NORM_MODE=fp16 requires the 'ada_layer_norm_fp16' kernel "
                "symbol, which is not present in this wllm_native_kernels build. "
                "Rebuild wllm_native_kernels, or use the default WLLM_NORM_MODE=triton "
    ...
    _has_bgr = (_fvk is not None
                ...
                and os.environ.get("WLLM_DISABLE_BIAS_GATE", "0") != "1")
    ...
    _fuse_ada_qkv = (_fvk is not None
                     ...
                     and os.environ.get("WLLM_DISABLE_ADA_QKV", "0") != "1")
    ...
    _fuse_norm2_ffn = (_fvk is not None
                       and hasattr(_fvk, "fp16_ada_layernorm_quant_fp8")
                       and os.environ.get("WLLM_DISABLE_NORM2_FFN", "0") != "1")
    ...
    def _get_qkv_shared_scale(block):
        ...
        cached = getattr(block, "_wllm_qkv_shared_scale", None)
        ...
        block._wllm_qkv_shared_scale = s
        return s
    ...
    def block_forward(self, hidden_states, temb, rotary_emb):
        ...
        if _fuse_qkv:
            ...
            if norm_mode == "fp16":
                norm1_out = _ada_norm_wllm_fp16(hs, scale_msa, shift_msa, S, D)
            else:
        ...
        if gelu_mode == "torch":
            if norm_mode == "fp16":
                norm2_out = _ada_norm_wllm_fp16(hs, c_scale_msa, c_shift_msa, S, D)
            else:
    ...
    logger.info("  [wLLM-Kern] block fusion: norm=%s gelu=%s", norm_mode, gelu_mode)
