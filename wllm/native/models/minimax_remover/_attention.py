...
The attention backend is selected by ``WLLM_ATTN_MODE``:
  ...
  * ``fa2``                     -- wLLM FA2 (``wllm_native_fa2.fwd_fp16``).

``sageattention`` is an optional third-party dependency (pip); only the
selected backend is imported lazily. ``fa2`` uses wLLM's own vendored
``wllm_native_fa2.so``. This module is the single source of truth for
...
try:
    ...
    try:
        import wllm_native_minimax_remover as _fvk  # type: ignore
    except ImportError:
        pass
_has_fused_rmsnorm_rope = (
    _fvk is not None and hasattr(_fvk, "fp16_rmsnorm_rope_bshd")
    and os.environ.get("WLLM_DISABLE_RMSNORM_ROPE", "0") != "1")
...
_has_fused_rmsnorm_rope_quant = (
    ...
    and os.environ.get("WLLM_DISABLE_FUSED_QUANT", "0") != "1")
...
def _attention_mode():
    return os.environ.get("WLLM_ATTN_MODE", "sage_fp8").lower()
...
def _get_sage():
    ...
    if _SAGE is None:
        try:
            ...
            raise RuntimeError(
                "WLLM_ATTN_MODE=sage_* requires the 'sageattention' package "
                "(the default attention backend). Install the model extra:\n"
                "    pip install -e \".[minimax-remover]\"\n"
                "or switch to the dependency-light wLLM FA2 backend:\n"
                "    WLLM_ATTN_MODE=fa2") from e
        _SAGE = _m
    return _SAGE
...
def _fa2_attn(q, k, v, scale, lse_cache=None):
    """Run wLLM's vendored FA2 backend on [B, S, H, D] fp16 tensors."""
    B, S, H, Dd = q.shape
...
def attention_forward(q, k, v, scale, mode=None, *, lse_cache=None):
    ...
    Single source of truth for the ``WLLM_ATTN_MODE`` routing; shared by
...
class wLLMFA2Processor:
    ...
    is chosen by ``WLLM_ATTN_MODE``. Per-shape cos/sin RoPE tables
    ...
    def __call__(self, attn, hidden_states, rotary_emb=None,
        ...
        # must expose gemm_from_fp8_ext (wLLMFp8Linear post-calibration).
