"""wLLM — RTX SM120 Qwen3 (plain dense) inference pipeline.
...
frontend (``wllm_native.frontends.torch.qwen3_rtx``) — by analogy with
``wllm_native.models.qwen36.pipeline_rtx`` which keeps the same split.
...
class Qwen3Pipeline:
    ...
    ``wllm_native.frontends.torch.qwen3_rtx`` against the
    wllm_native_kernels / wllm_native_fa2 entry points.
