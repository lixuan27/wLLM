"""wLLM — RTX SM120 Qwen3-VL-8B inference pipeline (dim contract).

This file defines the static dimension contract for the Qwen3-VL-8B
NVFP4 inference path. As with ``wllm_native.models.qwen3.pipeline_rtx``,
the actual compute lives in the frontend
(``wllm_native.frontends.torch.qwen3_vl_rtx``) — this module only records
...
class Qwen3VlPipeline:
    ...
    ``wllm_native.frontends.torch.qwen3_vl_rtx`` against the
    wllm_native_kernels / wllm_native_fa2 entry points. This class only holds
    ...
    ``wllm_native.models.qwen3.pipeline_rtx.Qwen3Pipeline``.
    """
