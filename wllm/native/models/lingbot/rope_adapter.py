...
see ``wllm_native.models.lingbot.rope_adapter_vit`` (added later).
...
device tensors and reused as inputs to the existing wllm_native RoPE
...
wllm_native_kernels symbol — the kernel consumes the tensors produced here
unchanged.
"""
...
def apply_rope_with_tables(
    ...
    upstream apply_rope. The wllm_native kernel will do the same math
    on-device.
