"""wLLM — LingBot-VLA model package (Thor / sm_110).
...
prefix / denoise attention via ``wllm_native.hardware.thor.fa4_backend``.
...
It is **not** registered in ``_PIPELINE_MAP`` and ``wllm_native.load_model`` does
**not** dispatch a ``lingbot`` config yet; ``LingbotTorchFrontendThor``
(``wllm_native.frontends.torch.lingbot_thor``) is a scaffold whose methods
