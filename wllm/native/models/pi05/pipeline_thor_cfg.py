"""wLLM — Pi0.5 Thor SM110 classifier-free-guidance inference pipeline.

Backend-agnostic per-chunk CFG class for the Pi0.5 Thor frontend
(torch + JAX), mirroring the RTX file structure
(``wllm_native/models/pi05/pipeline_rtx_cfg.py``).

The B=2 fused-CFG companion lives in
:mod:`wllm_native.models.pi05.pipeline_thor_cfg_batched`
...
-------------------
  ...
  * **No new kernels.** The combine uses the already-shipped
    ...
    ``wllm_native_kernels.so``). All elementwise / GEMM primitives Thor
...
class Pi05ThorCFGPipeline(Pi05ThorPipeline):
    ...
    Args:
        fvk: ``wllm_native.wllm_kernels`` module — the Thor kernel
    ...
    def set_language_embeds_pair(self, cond_embeds, uncond_embeds) -> None:
        ...
        Mirrors :meth:`wllm_native.models.pi05.pipeline_rtx_cfg.Pi05CFGPipeline
