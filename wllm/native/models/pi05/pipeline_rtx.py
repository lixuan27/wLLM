"""wLLM — RTX (consumer discrete GPU) Pi0.5 inference pipeline.

Framework-agnostic pipeline for Pi0.5 on consumer RTX GPUs (Blackwell SM120
/ Ada SM89, 5090 / 4090). Mirrors the ``wllm_native/hardware/thor/pipeline_pi05.py``
...
:class:`wllm_native.core.cuda_buffer.CudaBuffer` to remove that
...
def _fp8_nt_autotune_enabled(hardware: str | None, fp8_layout: str) -> bool:
    ...
    """
    policy = os.environ.get("WLLM_FP8_NT_AUTOTUNE", "auto").strip().lower()
    ...
    if policy != "auto":
        logger.warning(
            "Unknown WLLM_FP8_NT_AUTOTUNE=%r; using auto policy", policy)
...
class Pi05Pipeline:
    ...
    The pipeline composes ``wllm_native_kernels`` kernels + ``GemmRunner``
    cuBLASLt calls + an injected :class:`~wllm_native.hardware.rtx.attn_backend.AttnBackend`
    into the full SigLIP → Gemma encoder → Gemma decoder flow.

    Args:
        gemm:         ``fvk.GemmRunner()`` — cuBLASLt BF16/FP8 GEMM driver.
        fvk:          The ``wllm_native_kernels`` module (raw pointer kernels).
        attn_backend: Attention backend implementing the
                      :class:`~wllm_native.hardware.rtx.attn_backend.AttnBackend`
    ...
    def __init__(self, gemm, fvk, attn_backend, weights, *,
        ...
        if self.fp8_layout == "nk" and not self._autotune_fp8_nt:
            logger.info(
                ...
                "WLLM_FP8_NT_AUTOTUNE=force to override.",
    ...
    # ── opt-in: route graph REPLAY through the wLLM exec contract ──
    # WLLM_PI05_USE_EXEC=1 drives the captured infer/decoder graphs' replay
    ...
    def _exec_lazy_init(self) -> None:
        ...
        self._use_exec = os.environ.get(
            "WLLM_PI05_USE_EXEC", "0") not in ("0", "", "false", "False")
    ...
    def _exec_enable(self) -> None:
        ...
        from wllm.native.models.pi05.runtime_export import exec_enable
        exec_enable(self)
    ...
    def record_infer_graph(self, external_stream_int: int | None = None) -> None:
        ...
        from wllm.native.subgraphs.capture import run_capture_hooks
    ...
    def export_runtime(self, identity=None, extra_regions=None):
        ...
        See :func:`wllm_native.models.pi05.runtime_export.export_runtime` (the
        shared FP8/FP16 producer) for the argument contract.
        """
        from wllm.native.models.pi05.runtime_export import export_runtime
    ...
    def export_model_runtime(self, identity=None, extra_regions=None,
        ...
        See :func:`wllm_native.models.pi05.runtime_export.export_model_runtime`
        (the shared FP8/FP16 producer) for the port/stage contract.
        """
        from wllm.native.models.pi05.runtime_export import export_model_runtime
