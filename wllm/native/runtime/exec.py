"""Thin Python wrapper over the wLLM execution-contract C ABI.
...
then make `_wllm_exec` importable (it is emitted under exec/build/).
"""
...
def _import_native():
    try:
        import _wllm_exec as _c  # noqa: F401
        return _c
    except ImportError:
        pass
    ...
    try:
        ...
        raise ImportError(
            "Could not import _wllm_exec. Build it first:\n"
            "  cmake -S exec -B exec/build -DCMAKE_BUILD_TYPE=Release\n"
            "  cmake --build exec/build -j"
        ) from e
