"""wLLM — FvkContext: per-instance runtime resources.

Wraps the C++ FvkContext (cuBLAS handle) from wllm_native_kernels.so.
Created by frontend, passed to all kernel calls via hardware pipeline.

Usage:
    from wllm.native.core.context import FvkContext
    ctx = FvkContext()
...
class FvkContext:
    """Per-instance runtime resources for wLLM kernels.
