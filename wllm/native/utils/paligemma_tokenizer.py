...
Resolution order (first hit wins):
    1. `$WLLM_PALIGEMMA_TOKENIZER`            — explicit override
    2. `~/.cache/wllm_native/paligemma_tokenizer.model`
...
_DEFAULT_CACHE = Path.home() / ".cache" / "wllm_native"
...
def _candidate_paths() -> list[Path]:
    ...
    out: list[Path] = []
    env = os.environ.get("WLLM_PALIGEMMA_TOKENIZER")
    if env:
...
def _format_help_message(probed: list[Path]) -> str:
    ...
    return (
        ...
        f"         -o {_DEFAULT_PATH}\n\n"
        "Or set $WLLM_PALIGEMMA_TOKENIZER to a local copy.\n\n"
        "Searched:\n"
        f"{probed_str}\n\n"
