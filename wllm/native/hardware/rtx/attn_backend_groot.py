"""wLLM — RTX GROOT N1.6 attention backend.
...
wLLM's vendored FA2 covers all four head_dims (48 → kHeadDim=96,
72 → 96, 128 direct) so we can route everything through the
``wllm_native_fa2`` backend on RTX 5090 without requiring the upstream
...
class RtxFlashAttnBackendGroot:
    ...
    **Protocol compatibility**: implements
    :class:`wllm_native.hardware.backend.AttentionBackend` via
    ...
    def __init__(self, num_views: int, encoder_seq_max: int,
        ...
        try:
            ...
            raise RuntimeError(
                "GROOT RTX backend requires wLLM's vendored FA2 module "
                "(`wllm_native.wllm_fa2`). Build it with CMake on RTX "
