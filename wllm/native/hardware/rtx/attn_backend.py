"""wLLM — RTX attention backend protocol + implementation.
...
Design:
  - The backend **owns** all Q/K/V/O memory. Today that is torch
    ...
    allocator for :class:`wllm_native.core.cuda_buffer.CudaBuffer` to
...
Flash-Attention 2 in :mod:`wllm_native.wllm_fa2` (fp16 and bf16).
...
def _make_flash_attn_proxy(need_legacy: bool):
    ...
    older CUDA images) this lets ``import wllm_native`` and the default
    ...
    if need_legacy:
        try:
            ...
            raise ImportError(
                ...
                "vendored wllm_native_fa2 (default), or\n"
...
class RtxFlashAttnBackend:
    ...
    the vendored Flash-Attention 2 in ``wllm_native.wllm_fa2`` (fp16
    ...
    swap the allocator for :class:`wllm_native.core.cuda_buffer.CudaBuffer`
    ...
    Controlled by env var ``FVK_RTX_FA2`` (default ``"1"``):
      * ``"1"`` — attention call goes to ``wllm_native_fa2.fwd_{fp16,bf16}``.
    ...
    **Protocol compatibility**: this class implements the
    :class:`wllm_native.hardware.backend.AttentionBackend` protocol via
    ...
    ``wllm_native.models.pi05.pipeline_rtx`` and will be retired once
    ...
    def __init__(self, num_views: int, encoder_seq_max: int, chunk_size: int,
        ...
        #   wllm_native_fa2.so (drops pip flash-attn wheel dep), "0" =
        ...
        if self._use_fvk_fa2:
            try:
                from wllm.native import wllm_native_fa2 as _fa2
            except ImportError as e:
                raise RuntimeError(
                    "FVK_RTX_FA2=1 but wllm_native_fa2 module is not built. "
            ...
            # both live in the same wllm_native_fa2 module.
        ...
        # entirely through the vendored ``wllm_native_fa2.so``, so we make the
        # import lazy: environments without a prebuilt flash-attn wheel
        # (Modal / older CUDA images) can still use wLLM for the
    ...
    def set_fixed_shape(self, enabled: bool) -> None:
        ...
        if enabled:
            ...
            if not hasattr(getattr(self, "_fa2", None), "fwd_bf16_seqused"):
                reasons.append("wllm_native_fa2.fwd_bf16_seqused unavailable")
            if not hasattr(getattr(self, "_fa2", None),
                           "fwd_bf16_seqused_splitkv"):
                reasons.append("wllm_native_fa2.fwd_bf16_seqused_splitkv "
                               "unavailable (rebuild wllm_native_fa2)")
            if reasons:
                raise RuntimeError(
...
# External plugins pinned to ``from wllm.native.hardware.rtx.attn_backend
