...
import isolated from the rest of wLLM:

- FA4 is vendored under the **private** package name ``wllm_fa4`` (a trimmed
  ``flash_attn/cute`` forward-only subset, see
  ``csrc/attention/flash_attn_4_src/VENDOR.md``). It is loaded by transiently
  adding the vendor dir to ``sys.path`` and importing ``wllm_fa4.cute`` —
...
# csrc/attention/flash_attn_4_src holds the vendored `wllm_fa4` package.
# parents: [0]=thor [1]=hardware [2]=wllm_native [3]=<repo root>
_VENDOR_DIR = (
    Path(__file__).resolve().parents[3]
    / "csrc" / "attention" / "flash_attn_4_src"
)

_FUNC = None          # wllm_fa4.cute.flash_attn_func (autograd wrapper)
_FWD = None           # wllm_fa4.cute.interface_fwd_sm100._flash_attn_fwd
_TRIED = False
...
def _load() -> None:
    ...
    # Explicit A/B / opt-out switch: WLLM_THOR_FA4=0 forces the fmha path.
    if os.environ.get("WLLM_THOR_FA4", "1") == "0":
        _REASON = "disabled (WLLM_THOR_FA4=0)"
        return
    ...
    try:
        from wllm_fa4.cute import flash_attn_func
        from wllm_fa4.cute.interface_fwd_sm100 import _flash_attn_fwd
        ...
        # already cached in sys.modules under `wllm_fa4`.
        if added:
            try:
                sys.path.remove(src)
            except ValueError:
