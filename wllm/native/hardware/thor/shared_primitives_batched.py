"""wLLM — Thor SM110 model-agnostic primitives, B>=1 batched variants.

Companion to :mod:`wllm_native.hardware.thor.shared_primitives` which
...
:mod:`wllm_native.models.pi05.pipeline_thor` (B=1) and
:mod:`wllm_native.models.pi05.pipeline_thor_batched` (B>=1), and the
...
def encoder_forward_b2(gemm, fvk, bufs, weights, dims, stream=0, *,
    ...
    Buffer contract (``B`` is the leading axis or fold):
      ...
      bufs: same key set as
        :func:`wllm_native.hardware.thor.shared_primitives.encoder_forward`;
    ...
    Args:
        attn: Optional :class:`wllm_native.hardware.thor.attn_backend.ThorFlashAttnBackend`.
