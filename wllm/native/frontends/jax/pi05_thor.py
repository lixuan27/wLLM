"""wLLM — Thor JAX frontend (SM110).

Loads JAX Orbax checkpoints → FP8 quantize (ml_dtypes) → CudaBuffer (cudaMalloc)
→ pipeline.py (wllm_native_kernels.so) on Jetson AGX Thor.
...
from wllm.native.hardware.thor.shared_primitives import (
    siglip_forward,
    postln_project,
    encoder_forward,
    encoder_forward_calibrate,
)
from wllm.native.models.pi05.pipeline_thor import (
    Pi05ThorPipeline,
    decoder_forward,
    decoder_forward_calibrate,
)
from wllm.native.hardware.thor.attn_backend import (
...
from wllm.native.core.thor_frontend_utils import embed_prompt_numpy as _embed_prompt  # noqa: E402
from wllm.native.core.utils.pi05_prompt import PI05_STATE_PROMPT_MAX_LEN  # noqa: E402


class Pi05JaxFrontendThor:
    """Thor SM110 — JAX (Orbax) frontend. CudaBuffer bridge to wllm_native_kernels."""
    ...
    def __init__(self, checkpoint_dir, engine_path=None, fmha_path=None,
        ...
        """
        from wllm.native.core.cuda_buffer import CudaBuffer, sync
        from wllm.native.core.weights.transformer import quantize_fp8_e4m3, compute_time_embeddings
        ...
        mode = os.environ.get("WLLM_PI05_STATE_PROMPT_MODE",
                              state_prompt_mode)
        ...
        if mode == "fixed":
            fixed_cap = os.environ.get(
                "WLLM_PI05_STATE_PROMPT_FIXED_MAX_LEN",
        ...
        from wllm.native.core.utils.norm_stats import (
        ...
        if fmha_path is None:
            ...
            # ``wllm_native/`` package dir (pip / editable install target),
            # fresh cmake ``build/`` output. Use WLLM_FMHA_LIBRARY for an
            # explicit out-of-tree location.
            _here = pathlib.Path(__file__)
            for c in [os.environ.get('WLLM_FMHA_LIBRARY', ''),
        ...
        if weight_cache:
            from wllm.native.core.weights.weight_cache import load_weight_cache
        ...
        if not cache_hit:
            from wllm.native.core.weights.loader import load_weights, detect_format
            from wllm.native.core.weights.transformer import transform_jax_weights
    ...
    def _upload_weights(self, engine_w, quantize_fp8, compute_time_embeddings):
        ...
        from wllm.native.executors.jax_weights import OrbaxDictSource
        from wllm.native.executors.weight_loader import WeightLoader
        from wllm.native.frontends.jax._pi05_thor_spec import build_spec
    ...
    def _save_to_cache(self, checkpoint_path):
        ...
        """
        from wllm.native.core.weights.weight_cache import save_weight_cache
    ...
    def _calibrate(self):
        ...
        """
        from wllm.native.core.quant.calibrator import load_calibration, save_calibration
    ...
    def _capture_siglip_graph(self):
        ...
        from wllm.native.core.cuda_graph import CUDAGraph
        S, D, H, NH, HD, L = self.sig_dims
    ...
    def _capture_enc_ae_graph_b2(self):
        ...
        but uses :class:`wllm_native.core.cuda_graph.CUDAGraph` and
        explicit-stream replay (no ``torch.cuda.Stream`` here). Drives
        a 3x warmup followed by a single capture against
        :func:`wllm_native.hardware.thor.shared_primitives_batched.encoder_forward_b2`
        + :func:`wllm_native.models.pi05.pipeline_thor_batched.decoder_forward_b2`.
        """
        from wllm.native.core.cuda_graph import CUDAGraph
        from wllm.native.hardware.thor.shared_primitives_batched import (
            encoder_forward_b2)
        from wllm.native.models.pi05.pipeline_thor_batched import (
            decoder_forward_b2)
    ...
    def infer_batch(self, observations):
        ...
        """
        from wllm.native.core.utils.actions import (
    ...
    def _set_prompt_rl(self, prompt_text):
        ...
        from wllm.native.core.rl import build_acp_tagged_task
    ...
    def _build_cfg_serial_pipeline_jax(self, cfg_beta: float) -> None:
        ...
        from wllm.native.models.pi05.pipeline_thor_cfg import (
            Pi05ThorCFGPipeline)
        stream = self._stream
        stream_int = stream.value or 0
    ...
    def _build_cfg_batched_pipeline_jax(self, cfg_beta: float) -> None:
        ...
        """
        from wllm.native.models.pi05.pipeline_thor_cfg_batched import (
    ...
    def calibrate(
        ...
        *,
        ...
        (mirrors ``wllm_native/frontends/torch/pi05_thor.py``).
        """
        if isinstance(observations, dict):
        ...
        if n == 1:
            from wllm.native.core.calibration_api import implicit_calibrate
            implicit_calibrate(
                self, obs_list,
    ...
    def _calibrate_multi_frame(
        ...
        the sample axis via ``wllm_native.core.calibration.accumulate_amax``
        ...
        Mirrors:
          * Pi0.5 torch Thor — ``wllm_native/frontends/torch/pi05_thor.py``
            ``_calibrate_multi_frame``
          * Pi0.5 JAX Thor FP4 — ``wllm_native/frontends/jax/pi05_thor_fp4.py``
            ``_calibrate_multi_frame`` (Phase 1 only; FP4 also runs an
            AWQ refit in Phase 2 which is FP4-specific).
        """
        from wllm.native.core.calibration import accumulate_amax
    ...
    def infer(self, observation, debug=False):
        ...
        if self.norm_stats:
            from wllm.native.core.utils.actions import unnormalize_actions, LIBERO_ACTION_DIM
            unnorm = unnormalize_actions(raw_actions, self.norm_stats)
            robot_actions = unnorm[:, :LIBERO_ACTION_DIM]
        else:
            robot_actions = raw_actions
