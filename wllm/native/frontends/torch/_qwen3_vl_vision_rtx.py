"""wLLM — Qwen3-VL ViT tower forward on RTX.
...
All heavy compute runs through ``wllm_native_kernels`` / ``wllm_native_fa2`` /
``wllm_native_qwen3_vl_kernels``; tensor reshapes are metadata-only views.
...
class Qwen3VlVisionRtx:
    ...
    directory and runs the forward against the wLLM kernel modules.
