"""wLLM -- Nex-N2-mini (qwen3_5_moe) kernelized NVFP4 forward.
...
Every heavy op runs on a wLLM kernel -- no ``torch`` matmul, no
...
Compute path:
  ...
  * Full-attn: vendored FA2 causal (``wllm_native_fa2.fwd_bf16_causal``), native
...
from wllm.native.frontends.torch._nexn2_rtx_nvfp4_weights import _sf_swz_bytes
...
# the pre-existing wllm_native_fa2.so (already a hard dep of the decode backend),
...
def _get_fa2():
    global _FA2_MOD
    if _FA2_MOD is None:
        from wllm.native import wllm_native_fa2 as _m
        _FA2_MOD = _m
    return _FA2_MOD
