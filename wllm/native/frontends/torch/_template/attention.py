"""wLLM new-model template — attention specification.

Copy the body of `make_template_attention_spec()` into
`wllm_native/hardware/<hw>/attn_backend.py` (Thor or RTX). The
...
Your model code:                              wLLM AttentionSpec call:
-----------------                             ---------------------------
...
def make_template_attention_spec() -> AttentionSpec:
    ...
    `wllm_native/hardware/<hw>/attn_backend.py` next to the existing
