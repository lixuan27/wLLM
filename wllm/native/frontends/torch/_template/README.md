# wLLM New-Model Template

Skeleton package for adding a new VLA model to wLLM. Targets the
...
running in PyTorch and want to deploy it on wLLM-supported
...
2. `weights_spec.py` — start here. Mapping your checkpoint's tensor
   names to the wLLM weight slots is the most concrete first step.
...
## Model-code → wLLM mapping (the high-level mental model)

| Your model code | wLLM equivalent | Where it lives in template |
|---|---|---|
...
**Key insight**: wLLM replaces your model's `forward()` with a
...
2. Move the files to their final paths (see "Files in this template"
   above)
3. Add 4 lines to `wllm_native/hardware/__init__.py::_PIPELINE_MAP` to
   register your frontend
4. Write a config YAML at `wllm_native/configs/mymodel.yaml` (see any
