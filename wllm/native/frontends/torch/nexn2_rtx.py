"""wLLM -- Nex-N2-mini inference frontend (PyTorch + RTX SM120).
...
Two backends share this surface:
  * ``kernelized=True`` (production): NVFP4 weights loaded directly via the
    ...
    (-DWLLM_ENABLE_QWEN35MOE=ON). See docs/nexn2_usage.md.
...
from wllm.native.models.nexn2.pipeline_rtx import Nexn2Pipeline

# Kernels the kernelized path calls; checked up front so a build without
# -DWLLM_ENABLE_QWEN35MOE=ON fails clearly instead of after loading the 35B
...
def _require_kernels(fvk) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "(wllm_native_fa2), which failed to import. Build with FA2 enabled "
    ...
    if fa2_missing:                                         # pragma: no cover
        raise RuntimeError(
            "wllm_native_fa2 is present but lacks "
...
class Nexn2TorchFrontendRtx:
    ...
    def _build_kernelized_nvfp4(self) -> None:
        ...
        from wllm.native.frontends.torch._nexn2_rtx_nvfp4_weights import (
            extract_weights_nexn2_nvfp4,
        )
    ...
    def infer(self):
        ...
        if self._kernelized:
            ...
            from wllm.native.frontends.torch._nexn2_rtx_forward import (
                nexn2_forward_nvfp4,
            )
