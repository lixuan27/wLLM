"""wLLM -- Wan2.2 TI2V-5B official torch frontend for RTX SM120.

This frontend exposes the official Wan text/image-to-video pipeline through
wLLM's stable ``set_prompt`` / ``infer`` wrapper. ComfyUI integrations
...
class Wan22TorchFrontendRtx:
    ...
    def _candidate_wan_roots() -> list[pathlib.Path]:
        ...
        for key in ("WLLM_WAN22_ROOT", "WAN22_ROOT", "MOTUS_ROOT",
                    "WLLM_MOTUS_ROOT"):
    ...
    def _ensure_wan_importable(cls) -> None:
        ...
        raise ModuleNotFoundError(
            ...
            "package, add it to PYTHONPATH, or set WLLM_WAN22_ROOT to a "
            "directory containing the 'wan' package. A Motus checkout also "
            "works via WLLM_MOTUS_ROOT/MOTUS_ROOT because it vendors "
            "Wan under bak/wan.")
    ...
    def _configure_teacache(
        ...
        *,
        ...
        if not enabled:
            state = getattr(model, "_wllm_teacache", None)
        ...
        model._wllm_teacache = self._new_teacache_state(
            enabled=True,
            threshold=threshold,
    ...
    def _install_teacache(cls, model) -> None:
        ...
        def teacache_forward(self, x, t, context, seq_len, y=None):
            tc = getattr(self, "_wllm_teacache", None)
        ...
        model._wllm_teacache_original_forward = original_forward
