...
class _MergedKernels:
    """Resolve kernel symbols from the dedicated wllm_native_qwen3_vl_kernels
    module first, then fall back to the generic wllm_native_kernels. Lets the SM89
...
def _import_fvk() -> Any:
    """wllm_native_kernels (generic) + wllm_native_qwen3_vl_kernels (SM89 FP8)."""
    from wllm.native import wllm_native_kernels as core
    try:
        from wllm.native import wllm_native_qwen3_vl_kernels as vl
    except ImportError:
        vl = None
...
class Qwen3VlFp8Sm89TextFrontend:
    ...
    def __init__(self, checkpoint_path: str, *,
        ...
        if missing:
            raise RuntimeError(
                'wllm_native_kernels / wllm_native_qwen3_vl_kernels are missing SM89 '
                'Qwen3-VL FP8 symbols: '
                + ', '.join(missing)
                + '. Rebuild with -DGPU_ARCH=89 -DWLLM_BUILD_QWEN3_VL=ON '
                'and build wllm_native_kernels wllm_native_fa2 '
                'wllm_native_qwen3_vl_kernels.')
        self._fvk = fvk
    ...
    def _load_fp8_path(self) -> None:
        ...
        from wllm.native.frontends.torch._qwen3_vl_fp8_weights import (
