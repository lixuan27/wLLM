"""wLLM — PyTorch frontend for Qwen3-VL (multimodal) on RTX SM120.
...
# ``wllm_native_qwen3_vl_kernels`` module (CMake ``-DWLLM_BUILD_QWEN3_VL=ON``).
...
def _check_qwen3_vl_kernels(module) -> None:
    ...
    if missing:
        raise RuntimeError(
            'wllm_native_qwen3_vl_kernels is missing ' + ', '.join(missing)
            + '. Rebuild it with -DWLLM_BUILD_QWEN3_VL=ON '
            '(GPU_ARCH=89 or 120).')
...
def _require_qwen3_vl_kernels():
    ...
    try:
        ...
        raise RuntimeError(
            'wllm_native_qwen3_vl_kernels is not built. Configure with '
            '-DWLLM_BUILD_QWEN3_VL=ON (GPU_ARCH=89 or 120) and build the '
            'wllm_native_qwen3_vl_kernels target.') from e
...
class Qwen3VlTorchFrontendRtx:
    ...
    def __init__(self, checkpoint_path: str, *, device: str = 'cuda:0',
        ...
        from wllm.native.frontends.torch.qwen3_rtx import Qwen3TorchFrontendRtx
