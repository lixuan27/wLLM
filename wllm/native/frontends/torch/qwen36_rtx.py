"""wLLM -- PyTorch frontend for Qwen3.6-27B on RTX SM120.
...
layout, the class contract, and the ``wllm_native.hardware._PIPELINE_MAP``
resolution path.
...
# Set WLLM_QWEN36_HF_PATCH to the patch script path to enable; if
...
_PATCH_PATH = os.environ.get('WLLM_QWEN36_HF_PATCH', '')
if _PATCH_PATH and os.path.isfile(_PATCH_PATH):
    import importlib.util
    _spec = importlib.util.spec_from_file_location(
        '_wllm_qwen36_hf_patch', _PATCH_PATH,
    )
...
from wllm.native.models.qwen36 import Qwen36Pipeline  # noqa: E402


def _qwen36_tq_prefill_gdn_backend() -> str:
    backend = os.environ.get(
        'WLLM_QWEN36_TQ_PREFILL_GDN_BACKEND', 'wy_lt').strip().lower()
    if backend in ('fla', 'fla_chunk', 'fla_ref'):
        raise ValueError(
            'WLLM_QWEN36_TQ_PREFILL_GDN_BACKEND no longer supports '
            f'{backend!r}; use wy_lt or native for wLLM CUDA kernels')
    return backend
...
class Qwen36TorchFrontendRtx:
    ...
    def __init__(self, checkpoint_path: str, *,
        ...
        if quant == 'nvfp4' and self._user_max_seq > self.LONG_CTX_THRESHOLD:
            ...
            bf16_init_seq = int(os.environ.get(
                'WLLM_QWEN36_LONG_CTX_BF16_WINDOW',
                str(default_window),
        ...
        self._long_ctx_route_min_seq = int(os.environ.get(
            'WLLM_QWEN36_LONG_CTX_ROUTE_MIN_SEQ',
            str(route_default),
    ...
    def _load_hf_model(self) -> None:
        ...
        # path through the vendored FA2 (wllm_native_fa2.so) instead of
    ...
    # _MAX_PUBLIC_SPEC_K.
    MAX_Q_SEQ: int = int(os.environ.get('WLLM_QWEN36_MAX_Q_SEQ', '2048'))
    ...
    #     ``WLLM_QWEN36_GRAPH_CACHE_MAX``; setting it to ``0``
    ...
    GRAPH_CACHE_MAX: int = int(
        os.environ.get('WLLM_QWEN36_GRAPH_CACHE_MAX', '256'))
    ...
    #   - Override via env ``WLLM_QWEN36_LONG_CTX_THRESHOLD``.
    ...
    LONG_CTX_THRESHOLD: int = int(
        os.environ.get('WLLM_QWEN36_LONG_CTX_THRESHOLD', '16384'))
    ...
    def _alloc_buffers(self) -> None:
        ...
        from wllm.native.hardware.rtx.attn_backend_qwen36 import (
    ...
    def _load_nvfp4_path(self, alloc_own_forward_buffers: bool) -> None:
        ...
        (env ``WLLM_QWEN36_MTP_CKPT_DIR`` — unset disables spec
        ...
        mtp_dir = os.environ.get('WLLM_QWEN36_MTP_CKPT_DIR')
        mtp_path = None
        if mtp_dir:
    ...
    def _alloc_buffers_nvfp4(self) -> None:
        ...
        self._enable_lin_ab96_kernel = bool(int(os.environ.get(
            'WLLM_QWEN36_LIN_AB96_KERNEL', '1') or '0'))
        self._enable_full_gate_sigmoid_mul = bool(int(os.environ.get(
            'WLLM_QWEN36_FULL_GATE_SIGMOID_MUL', '0') or '0'))
        ...
        self._decode_fastgemm = bool(int(os.environ.get(
            'WLLM_QWEN36_DECODE_FASTGEMM', '0') or '0'))
        ...
        self._decode_splitk_down = bool(int(os.environ.get(
            'WLLM_QWEN36_DECODE_SPLITK_DOWN', '0') or '0'))
        self._decode_warpsplit_out = bool(int(os.environ.get(
            'WLLM_QWEN36_DECODE_WARPSPLIT_OUT', '0') or '0'))
        self._splitk_down_split = int(os.environ.get(
            'WLLM_QWEN36_SPLITK_DOWN_SPLIT', '4') or '4')
        self._splitk_down_stages = int(os.environ.get(
            'WLLM_QWEN36_SPLITK_DOWN_STAGES', '4') or '4')
        ...
        self._verify_warpsplit = bool(int(os.environ.get(
            'WLLM_QWEN36_VERIFY_WARPSPLIT', '0') or '0'))
        self._verify_ws_split = int(os.environ.get(
            'WLLM_QWEN36_VERIFY_WS_SPLIT', '2') or '2')
        self._verify_ws_stages = int(os.environ.get(
            'WLLM_QWEN36_VERIFY_WS_STAGES', '6') or '6')
    ...
    def _gemv_nvfp4(self, A_packed, B_packed, D, N, K,
        ...
        N). With ``WLLM_QWEN36_DECODE_FASTGEMM=1``: the hand-tuned
    ...
    def _layer_forward_lin_K_nvfp4(self, L: int, h_in_K, K: int):
        ...
        if self._enable_lin_ab96_kernel:
            ...
            if use_ab96_lt:
                ...
                and os.environ.get(
                    'WLLM_QWEN36_PREFILL_AB96_M4_PAIR',
        ...
        use_wy_chunk = (
            K > self._K_save_max
            and gdn_backend in ('wy', 'native_wy', 'wllm_wy', 'wy_lt')
        ...
        if not use_direct_gdn and not conv_gqa_ready:
            ...
            exact_verify_gating = (
                os.environ.get(
                    'WLLM_QWEN36_TQ_VERIFY_EXACT_GATING', '0') == '1'
            )
            use_fused_gating = (
            ...
            if exact_verify_gating or not use_fused_gating:
                raise RuntimeError(
                    'wLLM fused GDN gating is required for the '
        ...
        fast_chunk_mode = os.environ.get(
            'WLLM_QWEN36_TQ_PREFILL_GDN_FAST_CHUNK', 'bf16')
        ...
        use_cuda_wy_chunk_h = (
            ...
            and os.environ.get(
                'WLLM_QWEN36_TQ_PREFILL_GDN_CHUNK_H_CUBLASLT_REF',
        ...
        if use_wy_chunk:
            ...
            if use_wy_chunk:
                ...
                use_wy_lt_norm_pack_qk = (
                    ...
                    and os.environ.get(
                        'WLLM_QWEN36_TQ_PREFILL_GDN_PACK_K_IN_NORM',
                        '1').strip().lower() not in ('0', 'false', 'off'))
                ...
                use_fused_kkt_solve = (
                    ...
                    and os.environ.get(
                        'WLLM_QWEN36_TQ_PREFILL_GDN_FUSED_KKT_SOLVE',
                        '1').strip().lower() not in ('0', 'false', 'off'))
                ...
                if use_wy_lt_ai_pack_from_solve:
                    use_fused_solve_pack = (
                        ...
                        and os.environ.get(
                            'WLLM_QWEN36_TQ_PREFILL_GDN_FUSED_SOLVE',
                            '1').strip().lower() not in ('0', 'false', 'off'))
                    use_fused_solve_pack_only = (
                        ...
                        and os.environ.get(
                            'WLLM_QWEN36_TQ_PREFILL_GDN_SOLVE_PACK_ONLY',
                            '1').strip().lower() not in ('0', 'false', 'off'))
                    if use_fused_kkt_solve:
                ...
                use_cuda_wy_output_o_rawk = (
                    ...
                    and os.environ.get(
                        'WLLM_QWEN36_TQ_PREFILL_GDN_OUTPUT_RAWK',
                        '1').strip().lower() not in ('0', 'false', 'off'))
        ...
        if self._enable_silu_mul_quant_fusion:
            if self._enable_mlp_gate_up_fusion:
                grouped_silu_mode = os.environ.get(
                    'WLLM_QWEN36_GROUPED_MLP_SILU_QUANT',
                    '32').strip().lower()
                if (
    ...
    def forward_mtp_head_nvfp4(self, prev_h, prev_token_id, cur_pos: int,
        ...
        if mtp is None:
            raise RuntimeError(
                'MTP head not loaded — set WLLM_QWEN36_MTP_CKPT_DIR')
    ...
    def _snapshot_capsule_long(self):
        ...
        if mode != 'fp8':
            raise NotImplementedError(
                ...
                f'{mode!r}; set WLLM_QWEN36_LONG_KV_CACHE=fp8')
        cur_pos = int(self._agent_stream_cur_pos)
    ...
    def long_prefill_chunk_size(self) -> int:
        ...
        return min(
            int(os.environ.get('WLLM_QWEN36_TQ_PREFILL_CHUNK',
                               str(self.MAX_Q_SEQ))),
            self.MAX_Q_SEQ)
    ...
    def _layer_forward_lin_K(self, L: int, h_in_K, K: int):
        ...
        # and state update stay in wLLM kernels.
    ...
    def forward_own_decode_K(self, token_ids_K, cos_K, sin_K,
        ...
        # 0) Embed K tokens through a fixed wLLM kernel.
        fvk.qwen36_embedding_lookup_bf16(
    ...
    def forward_mtp_head(self, prev_h, prev_token_id, cur_pos: int):
        ...
        # 0) Embed prev_token through a fixed wLLM kernel.
        if prev_token_id.ndim == 1:
            prev_token_id = prev_token_id.view(1, 1)
        fvk.qwen36_embedding_lookup_bf16(
            prev_token_id.view(-1).data_ptr(),
    ...
    def generate_own(self, input_ids, *, max_new_tokens: int,
        ...
        Prefill is handled by the frontend's native wLLM kernels.
    ...
    # ---------- opt-in: route replay through the wLLM exec contract ----
    # WLLM_QWEN36_USE_EXEC=1 drives captured-graph REPLAY through the
    ...
    def _exec_lazy_init(self) -> None:
        ...
        self._use_exec = os.environ.get(
            'WLLM_QWEN36_USE_EXEC', '0') not in ('0', '', 'false', 'False')
    ...
    def _long_tq_effective_k(
        ...
        tq_spec_k = os.environ.get('WLLM_QWEN36_TQ_SPEC_K', '')
        if tq_spec_k:
    ...
    def warmup_long_ctx_decode_graphs(
        ...
        prefill_chunk = int(os.environ.get(
            'WLLM_QWEN36_TQ_PREFILL_CHUNK', str(self.MAX_Q_SEQ)))
        ...
        stride_raw = os.environ.get(
            'WLLM_QWEN36_LONG_WARMUP_STRIDE', 'auto').lower()
        max_graphs_per_shape = int(os.environ.get(
            'WLLM_QWEN36_LONG_WARMUP_MAX_GRAPHS', '96') or '96')
        min_free_mb = int(os.environ.get(
            'WLLM_QWEN36_LONG_WARMUP_MIN_FREE_MB', '1024') or '0')
    ...
    def warmup_long_ctx_prefill_graphs(
        ...
        max_chunks_per_shape = int(os.environ.get(
            'WLLM_QWEN36_LONG_PREFILL_WARMUP_MAX_CHUNKS', '0') or '0')
        min_free_mb = int(os.environ.get(
            'WLLM_QWEN36_LONG_WARMUP_MIN_FREE_MB', '1024') or '0')
    ...
    def _long_tq_graph_capture_allowed(self) -> bool:
        min_free_mb = int(os.environ.get(
            'WLLM_QWEN36_LONG_GRAPH_MIN_FREE_MB', '768') or '0')
        if min_free_mb <= 0:
            return True
        try:
            import torch
    ...
    def _long_prefill_graph_capture_allowed(self, prompt_len: int) -> bool:
        max_ctx = int(os.environ.get(
            'WLLM_QWEN36_LONG_PREFILL_GRAPH_MAX_CTX',
            '131072') or '0')
    ...
    def _ensure_mtp_chain_graph_nvfp4(
        ...
        def _run_chain():
            mtp_argmax_parts = int(os.environ.get(
                'WLLM_QWEN36_MTP_ARGMAX_PARTS', '128') or '128')
            for k in range(K):
                self.forward_mtp_head_nvfp4(
                    self._mtp_static_prev_h,
                    self._mtp_static_prev_token,
    ...
    def _enter_long_ctx_mode(self) -> None:
        ...
        selected by ``WLLM_QWEN36_LONG_KV_CACHE``.  ``fp8`` is the
        ...
        mode = os.environ.get(
            'WLLM_QWEN36_LONG_KV_CACHE', 'fp8').strip().lower()
        if mode not in ('tq', 'fp8'):
            raise ValueError(
                'WLLM_QWEN36_LONG_KV_CACHE must be tq or fp8, '
                f'got {mode!r}')
        ...
        if self._weights.ptrs.get('mtp') is not None:
            ...
            compact_mtp = int(os.environ.get(
                'WLLM_QWEN36_LONG_MTP_CACHE_TOKENS', '4096') or '4096')
            compact_mtp = max(
    ...
    def _long_mtp_prefill_tail_for_prompt(self, prompt_len: int) -> int:
        raw = os.environ.get(
            'WLLM_QWEN36_LONG_MTP_PREFILL_TAIL', 'auto') or 'auto'
        if raw.lower() != 'auto':
    ...
    def _prefill_long_ctx_tq_chunked(self, input_ids, *, retain_rows: int = 0):
        ...
        use_prefill_graph = (
            os.environ.get(
                'WLLM_QWEN36_TQ_PREFILL_GRAPH', '0').lower()
            in ('1', 'true', 'yes', 'on')
    ...
    def prefill_long_ctx_nvfp4_agent(
        ...
        if os.environ.get('WLLM_QWEN36_TQ_STRICT_NEXT', '0') == '1':
            raise NotImplementedError(
        ...
        use_kernel_accept = (
            os.environ.get('WLLM_QWEN36_KERNEL_ACCEPT', '1') == '1'
        )
        accept_parts = int(os.environ.get(
            'WLLM_QWEN36_ACCEPT_PARTS', '32') or '32')
        ...
        with torch.no_grad():
            ...
            if mtp_tail > 0:
                ...
                used_kv_only = False
                if os.environ.get(
                        'WLLM_QWEN36_LONG_MTP_TAIL_KV_ONLY',
                        '1') == '1':
    ...
    def decode_long_ctx_nvfp4_committed_stream(
        ...
        adaptive_k = (
            not os.environ.get('WLLM_QWEN36_TQ_SPEC_K', '')
            and K >= 4
            and os.environ.get('WLLM_QWEN36_TQ_ADAPTIVE_K', '1') == '1'
        )
        use_kernel_accept = (
            os.environ.get('WLLM_QWEN36_KERNEL_ACCEPT', '1') == '1'
        )
        ...
        with torch.no_grad():
            ...
            while emitted < int(max_new_tokens):
                ...
                if draft_k > 0:
                    if os.environ.get(
                            'WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH',
                            '1') == '1':
                        gs = self._graph_stream
                ...
                use_verify_graph = (
                    os.environ.get(
                        'WLLM_QWEN36_TQ_VERIFY_GRAPH', '1') == '1'
                )
    ...
    def _generate_long_ctx_speculative_KN_nvfp4(
        ...
        ``WLLM_QWEN36_LONG_KV_CACHE=fp8``, prefill and verify use an
        ...
        strict_next = (
            os.environ.get('WLLM_QWEN36_TQ_STRICT_NEXT', '0') == '1'
        )
        strict_next_graph = (
            strict_next
            and os.environ.get(
                'WLLM_QWEN36_TQ_STRICT_NEXT_GRAPH', '1') == '1'
        )
        accept_parts = int(os.environ.get(
            'WLLM_QWEN36_ACCEPT_PARTS', '32') or '32')
        use_partitioned_accept = (
        ...
        with torch.no_grad():
            ...
            while generated_len < max_new_tokens:
                ...
                # WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH=0 only when optimizing
                # first-request latency without prewarm.
                if os.environ.get(
                        'WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH', '1') == '1':
                    gs = self._graph_stream
                    cg = self._ensure_mtp_chain_graph_nvfp4(
    ...
    def _load_turboquant_kv(self, b_v: int = 3, b_k_total: int = 3,
        ...
        """
        from wllm.native.frontends.torch._qwen36_rtx_turboquant import (
            TurboQuantSetup,
        )
    ...
    def _load_fp8_kv_cache(self, max_seq: int) -> None:
        ...
        stage_layers_raw = os.environ.get(
            'WLLM_QWEN36_FP8_STAGE_LAYERS', 'auto').lower()
        if stage_layers_raw == 'auto':
            ...
            reserve_mb = int(os.environ.get(
                'WLLM_QWEN36_FP8_STAGE_RESERVE_MB', '1024') or '0')
            try:
                free_bytes, _ = torch.cuda.mem_get_info()
                mem_cap = max(
    ...
    def _load_turboquant_packed(self, max_seq_tq: int = 65536,
        ...
        if not hasattr(self, '_tq_cache_packed'):
            ...
            stage_reserve_mb = int(os.environ.get(
                'WLLM_QWEN36_TQ_PER_LAYER_STAGE_RESERVE_MB', '1024'))
            stage_cap_raw = os.environ.get(
                'WLLM_QWEN36_TQ_PER_LAYER_STAGE_CAP', 'auto').lower()
            if stage_cap_raw == 'auto':
            ...
            stage_layers_raw = os.environ.get(
                'WLLM_QWEN36_TQ_PER_LAYER_STAGE_LAYERS', 'auto').lower()
            if stage_layers_raw == 'auto':
                if stage_cap <= 100000:
                    policy_cap = 16
            ...
            hot_cap_raw = os.environ.get(
                'WLLM_QWEN36_TQ_HOT_STAGE_CAP', 'auto').lower()
            if hot_cap_raw == 'auto':
                hot_cap = min(max_seq_tq, 132096)
            else:
                hot_cap = max(1, min(max_seq_tq, int(hot_cap_raw)))
            hot_layers_raw = os.environ.get(
                'WLLM_QWEN36_TQ_HOT_STAGE_LAYERS', 'auto').lower()
            if hot_layers_raw == 'auto':
                ...
                try:
                    ...
                    hot_reserve_mb = int(os.environ.get(
                        'WLLM_QWEN36_TQ_HOT_STAGE_RESERVE_MB', '1536'))
    ...
    def _tq_dequant_into_stage(self, layer: int, end_pos: int) -> None:
        ...
        cache = self._tq_cache_packed
        try:
            from wllm.native import wllm_native_kernels as _fvk
    ...
    def init_dflash_drafter(self, ckpt_dir: str | None = None) -> None:
        ...
        ``ckpt_dir`` falls back to ``WLLM_QWEN36_DFLASH_CKPT_DIR``.
    ...
    def _load_dflash_drafter(self, ckpt_dir: str | None = None) -> None:
        ...
        ``WLLM_QWEN36_DFLASH_CKPT_DIR`` env var (raises if neither is
        provided). Idempotent.
        """
        import os
        ...
        from wllm.native import wllm_native_kernels as fvk
        from wllm.native.frontends.torch._qwen36_rtx_dflash_weights import (
            assert_dflash_extraction_invariants,
            extract_dflash_weights_nvfp4,
        )
        from wllm.native.frontends.torch._qwen36_rtx_dflash_forward import (
    ...
    def generate_own_speculative_DFlash_nvfp4(
        ...
        if self._weights.ptrs.get('dflash') is None:
            raise RuntimeError(
                ...
                'first or set WLLM_QWEN36_DFLASH_CKPT_DIR')
        max_spec_k = min(self.MAX_Q_SEQ - 1, self._MAX_PUBLIC_SPEC_K)
        if K < 1 or K > max_spec_k:
            raise ValueError(
                f'K={K} out of range — need 1<=K<={max_spec_k}')
        ...
        if pertoken:
            from wllm.native.frontends.torch._qwen36_rtx_dflash_forward import (  # noqa: E501
        ...
        relaxed = None
        if os.environ.get(
                'WLLM_QWEN36_DFLASH_RELAXED_THINKING', '0',
            ...
            if isinstance(think_open, int) and think_open >= 0:
                relaxed = {
                    'topk': max(1, int(os.environ.get(
                        'WLLM_QWEN36_DFLASH_RELAXED_TOPK', '3'))),
                    'delta': float(os.environ.get(
                        'WLLM_QWEN36_DFLASH_RELAXED_DELTA', '1.0')),
                    'open': int(think_open),
