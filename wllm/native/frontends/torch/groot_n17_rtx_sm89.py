"""wLLM -- GROOT N1.7 FP8 torch frontend for RTX SM89.

Framework-conforming FP8 path for GROOT N1.7 on RTX SM89. The whole VLM
backbone (ViT / DeepStack / LLM / VL self-attn) runs through wLLM FP8
kernels via ``wllm_native.models.groot_n17.pipeline_rtx_sm89`` with runtime
weights stored as ``[N, K]`` and GEMMs dispatched through ``fp8_nt_dev``.

Activation scales follow the wLLM calibration convention:
...
class _GrootN17FP8BackboneMixin:
    ...
    def _run_kernel_backbone_fp8(self, aux: dict) -> "torch.Tensor":
        ...
        from wllm.native.models.groot_n17 import pipeline_rtx_sm89 as P
