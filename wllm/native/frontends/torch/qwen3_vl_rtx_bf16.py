...
stack is a direct BF16 Qwen3 decoder over generic wLLM kernels.
...
def _require_qwen3_vl_rtx_bf16_kernels():
    try:
        ...
        from wllm.native import wllm_native_qwen3_vl_kernels as vlk
    except ImportError as e:
        raise RuntimeError(
            'Qwen3VlTorchFrontendRtxBF16 requires wllm_native_kernels and '
            'wllm_native_qwen3_vl_kernels. Configure with '
            '-DGPU_ARCH=87 -DWLLM_BUILD_QWEN3_VL=ON and build '
            'wllm_native_kernels wllm_native_fa2 wllm_native_qwen3_vl_kernels.'
        ) from e
    ...
    if missing_core or missing_vl:
        pieces = []
        if missing_core:
            pieces.append('wllm_native_kernels: ' + ', '.join(missing_core))
        if missing_vl:
            pieces.append('wllm_native_qwen3_vl_kernels: ' + ', '.join(missing_vl))
        raise RuntimeError(
...
class Qwen3VlTorchFrontendRtxBF16:
    ...
    def __init__(self, checkpoint_path: str, *, device: str = 'cuda:0',
        ...
        from wllm.native.frontends.torch._qwen3_vl_bf16_weights import (
            assert_extraction_invariants_qwen3_vl_bf16,
            extract_weights_qwen3_vl_bf16,
        )
        from wllm.native.frontends.torch._qwen3_vl_vision_rtx import (
            Qwen3VlVisionRtx,
        )
        ...
        if max_prefill_graphs is None:
            max_prefill_graphs = int(os.environ.get(
                'WLLM_QWEN3_VL_PREFILL_GRAPH_CACHE_MAX', '256'))
        if max_decode_graphs is None:
            max_decode_graphs = int(os.environ.get(
                'WLLM_QWEN3_VL_DECODE_GRAPH_CACHE_MAX', '256'))
    ...
    def _build_mrope_caches(self) -> None:
        from wllm.native.frontends.torch import _qwen3_vl_geometry as geo
