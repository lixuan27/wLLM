"""wLLM — PyTorch frontend for Qwen3 (plain dense) on RTX SM120.
...
loader + every wllm_native_kernels NVFP4 entry point this path needs
is wired correctly. The full forward
(``forward_own_decode_nvfp4``) is below.

Sibling reference: ``wllm_native.frontends.torch.qwen36_rtx`` (the 27B
...
class Qwen3TorchFrontendRtx:
    ...
    def _load_nvfp4_path(self) -> None:
        ...
        from wllm.native.frontends.torch._qwen3_rtx_nvfp4_weights import (
        ...
        # wllm_native_kernels module (no separate FP4 shared lib).
        if not fvk.has_nvfp4():
            raise RuntimeError(
                'wllm_native_kernels.has_nvfp4()==False — built without '
    ...
    def _alloc_buffers_nvfp4(self) -> None:
        ...
        # WLLM_QWEN3_RESID_FUSE=0 or WLLM_QWEN3_NO_RESID_FUSE=1 to
        # restore the separate-add path for A/B checks.
        import os as _os
        if (_os.environ.get('WLLM_QWEN3_RESID_FUSE', '1') != '1'
                or _os.environ.get('WLLM_QWEN3_NO_RESID_FUSE', '0') == '1'):
        ...
        # WLLM_QWEN3_SWIGLU_FOLD=0 to restore the split GEMM + silu_mul path.
        import os as _os
        ...
        self._enable_swiglu_fold_prefill = (
            _os.environ.get('WLLM_QWEN3_SWIGLU_FOLD', '1') == '1'
