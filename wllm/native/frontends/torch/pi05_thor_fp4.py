...
from wllm.native.hardware.thor.shared_primitives import encoder_forward
from wllm.native.models.pi05.pipeline_thor import decoder_forward
from wllm.native.hardware.thor.shared_primitives_fp4 import encoder_forward_with_fp4_subset
from wllm.native.executors.fp4_utils import (
...
class Pi05TorchFrontendThorFP4(Pi05TorchFrontendThor):
    ...
    def __init__(self, checkpoint_dir, num_views: int = 2,
        ...
        if self._fp4_layers:
            if not _HAS_FP4:
                raise RuntimeError(
                    "use_fp4_encoder_ffn=True but wllm_native_fp4 not available. "
                    "Ensure wllm_native_fp4.so is built with NVFP4 support.")
    ...
    def _alloc_fp4_scratch_for_Se(self, Se: int):
        ...
        if self.use_p1_split_gu:
            from wllm.native.executors.fp4_utils import FP4Buffer
