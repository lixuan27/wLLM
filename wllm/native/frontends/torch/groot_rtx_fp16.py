"""wLLM -- RTX GROOT N1.6 full-FP16 torch frontend.
...
:mod:`wllm_native.models.groot.pipeline_rtx_fp16`. It exists as an A/B
reference against the default FP8 path.

Usage::

    from wllm.native.frontends.torch.groot_rtx_fp16 import GrootTorchFrontendRtxFP16
...
from wllm.native.models.groot.pipeline_rtx_fp16 import (
    GrootDiTFP16 as GrootDiT,
    GrootQwen3FP16 as GrootQwen3,
    GrootSigLIP2FP16 as GrootSigLIP2,
)
from wllm.native.models.groot.pipeline_rtx import (
    DIT_D,
    DIT_H,
    DIT_HD,
    DIT_L,
...
#    wllm_native/hardware/groot_embodiments.py) ──
...
class GrootTorchFrontendRtxFP16:
    ...
    def set_prompt(self, prompt: str) -> None:
        ...
        reproducible):
          ...
          2. the ``WLLM_QWEN3_TOKENIZER`` environment variable, if set
          3. ``~/.cache/wllm_native/qwen3_tok``

        If none resolve, a clear error explains how to provide the
        tokenizer; wLLM never reaches out to the network on its own.
        """
        ...
        if not hasattr(self, "_tokenizer"):
            ...
            env_tok = os.environ.get("WLLM_QWEN3_TOKENIZER")
            if env_tok:
                tokenizer_candidates.append(env_tok)
            tokenizer_candidates.append(
                str(pathlib.Path.home() / ".cache" / "wllm_native" / "qwen3_tok"))
            ...
            if not hasattr(self, "_tokenizer"):
                raise RuntimeError(
                    ...
                    "WLLM_QWEN3_TOKENIZER environment variable, or "
                    "~/.cache/wllm_native/qwen3_tok. Pre-download once with "
                    "`hf download Qwen/Qwen3-1.7B --include 'tokenizer*' 'vocab*' "
                    "'merges*' --local-dir ~/.cache/wllm_native/qwen3_tok`.")
