...
Hand-written forward over wllm_native_kernels + wllm_native_fa2: a dense Qwen3-4B
...
path; an explicit temperature fallback uses wLLM's dependency-free CUDA
...
from wllm.native.models.higgs_audio_v3.pipeline_rtx import HiggsAudioV3Dims
...
class HiggsAudioV3TorchFrontendRtx:
    ...
    def __init__(self, checkpoint_path: str, *,
        ...
        Args:
          ...
          fp8: decode precision. ``None`` (default) auto-selects by hardware —
            ...
            the same kernelised graph; selection follows the wLLM
        ...
        # (both FP8 and BF16 paths). Set WLLM_HIGGS_BATCHED_PREFILL=0 for eager.
        self._batched_prefill = os.environ.get(
            "WLLM_HIGGS_BATCHED_PREFILL", "1") != "0"
        # Single position-agnostic decode CUDA graph (devpos KV-write + FA2
        # seqused_k), both paths. Set WLLM_HIGGS_GRAPH=0 to force eager decode.
        self._use_graph = os.environ.get("WLLM_HIGGS_GRAPH", "1") != "0"
    ...
    def _resolve_precision(fp8: bool | None) -> bool:
        """Pick the decode precision from the hardware, wLLM-style: probe the
        ...
        from wllm.native.core.utils.hardware import (
            get_gpu_name,
            get_gpu_sm_version,
        )
        ...
        if not hasattr(fvk, "delayed_codebook_argmax_embed_bf16"):
            raise RuntimeError(
                ...
                "build; rebuild with WLLM_ENABLE_AUDIO_CODEBOOK=ON.")
            ...
            if not fp8_ok:
                if not bf16_ok:
                    raise RuntimeError(
                        ...
                        f"(SM{sm} {name}); rebuild wLLM with GPU_ARCH "
                        f"matching the GPU.")
                log.warning(
        ...
        if not use_fp8 and not bf16_ok:
            raise RuntimeError(
                ...
                f"(SM{sm} {name}); rebuild wLLM with GPU_ARCH matching the "
                f"GPU.")
    ...
    def _alloc_buffers(self) -> None:
        from wllm.native.hardware.rtx.attn_backend_qwen3 import (
    ...
    def _ensure_decoder(self) -> None:
        ...
        if self._dec is not None:
            return
        if self.fp8:
            from wllm.native.frontends.torch._higgs_audio_v3_fp8 import (
                HiggsAudioV3Fp8Decoder,
            )
            dec = HiggsAudioV3Fp8Decoder(self)
            dec.calibrate(self.build_prompt(_FP8_CALIBRATION_TEXT))
        else:
            from wllm.native.frontends.torch._higgs_audio_v3_bf16 import (
    ...
    def _ensure_codec(self) -> None:
        if self._codec is not None:
            return
        from wllm.native.models.higgs_audio_v3.codec import HiggsAudioV3Codec
    ...
    def decode_stream(self, *, temperature: float = 0.0,
        ...
        if sampled and not hasattr(fvk, "delayed_codebook_sample_embed_bf16"):
            raise RuntimeError(
                ...
                "with WLLM_ENABLE_AUDIO_CODEBOOK=ON or use temperature=0.")
