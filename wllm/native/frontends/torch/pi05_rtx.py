"""wLLM -- RTX Pi0.5 torch frontend.

Loads HuggingFace PyTorch safetensors checkpoints + drives the
framework-agnostic :class:`~wllm_native.models.pi05.pipeline_rtx.Pi05Pipeline`.

This is the "reference" RTX frontend. The RTX JAX frontend
(:mod:`wllm_native.frontends.jax.pi05_rtx`) mirrors this API but loads
from Orbax and uses JAX for weight quantization.

Usage::

    from wllm.native.frontends.torch.pi05_rtx import Pi05TorchFrontendRtxRtx
...
from wllm.native.models.pi05.pipeline_rtx import (
    Pi05Pipeline,
...
from wllm.native.models.pi05.pipeline_rtx_cfg import Pi05CFGPipeline
from wllm.native.models.pi05.pipeline_rtx_batched import Pi05BatchedPipeline
from wllm.native.models.pi05.pipeline_rtx_cfg_batched import Pi05CFGBatchedPipeline
from wllm.native.hardware.rtx.attn_backend_batched_pi05 import (
    PI05_BATCH_SIZE,
    RtxFlashAttnBatchedBackendPi05,
)
from wllm.native.core.utils.hardware import supports_fp8
from wllm.native.core.utils.pi05_prompt import PI05_STATE_PROMPT_MAX_LEN, format_pi05_prompt
...
def _embed_prompt(prompt_text: str, embedding_weight: torch.Tensor,
    ...
    # `wllm_native.utils.paligemma_tokenizer` for the search order and
    # the download instructions emitted on failure.
    try:
        # Preferred: openpi's PaligemmaTokenizer (exact same vocab,
        # same prompt prefix logic wLLM was built against).
        ...
        # wLLM helper (clear error if not found — never silent
        # segfault).
...
class Pi05TorchFrontendRtx:
    ...
    def __init__(self,
        ...
        # Env override: WLLM_PI05_STATE_PROMPT_MODE.
        ...
        self._frame_count = 0
        from wllm.native.models.pi05.pipeline_rtx import VIS_L as _VIS_L
        ...
        self._noise_out = torch.empty(
            self.chunk_size, ACTION_DIM, dtype=bf16, device="cuda")
        from wllm.native.core.cuda_buffer import _cudart
    ...
    def _calibrate_single_frame(self, sample) -> None:
        ...
        with torch.cuda.stream(self._graph_torch_stream):
            ...
            from wllm.native.subgraphs.capture import apply_frontend_capture_hooks
    ...
    def _warn_if_scale_ceiling_exceeded(self, label: str = "pi05_rtx") -> None:
        ...
        from wllm.native.core.calibration import check_scale_ceiling
        scales = {
