"""wLLM -- Pi05TorchFrontendThor: complete inference using ONLY wllm_native_kernels.so.

All computation goes through wllm_native_kernels (pybind11 + CUTLASS + cuBLASLt).
...
from wllm.native.core.utils.pi05_prompt import PI05_STATE_PROMPT_MAX_LEN
...
from wllm.native.core.thor_frontend_utils import embed_prompt_torch as embed_prompt  # noqa: E402
...
class Pi05TorchFrontendThor:
    """Complete Pi0.5 inference pipeline using only wllm_native_kernels.so.

    Interface compatible with wLLMModel.predict():
    ...
    def __init__(self, checkpoint_dir: str, num_views: int = 2,
        ...
        _spm = os.environ.get("WLLM_PI05_STATE_PROMPT_MODE", state_prompt_mode)
    ...
    def _load_weights(self, safetensors_path):
        ...
        from wllm.native.frontends.torch._pi05_thor_spec import build_spec

        from wllm.native.executors.torch_weights import _autodetect_strip_prefix
    ...
    def _capture_model_runtime_context_action_graphs(
        ...
        if getattr(self, "_model_runtime_context_graph", None) is None:
            ...
            from wllm.native.subgraphs.capture import register_captured_graph
    ...
    def _capture_enc_ae_graph_b2(self):
        ...
        and :func:`wllm_native.models.pi05.pipeline_thor_batched.decoder_forward_b2`.
    ...
    def set_batched_mode(self, *, enable: bool = True,
        ...
        Mirrors :meth:`wllm_native.frontends.torch.pi05_rtx.Pi05TorchFrontendRtx.set_batched_mode`
