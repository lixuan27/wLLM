"""wLLM -- TurboQuant KV cache quantization (Phase 2B).
...
class TurboQuantKVCache:
    ...
    def write_kv_fast(self, layer: int, pos_start: int, pos_end: int,
        ...
        if os.environ.get('WLLM_QWEN36_TQ_KERNEL_WRITE', '1') != '1':
            raise ValueError(
                'WLLM_QWEN36_TQ_KERNEL_WRITE=0 is no longer supported '
        ...
        if not use_kernel_gemm:
            raise RuntimeError(
                'TurboQuant write requires wLLM cuBLASLt GEMM wrappers')
