...
All operations via wllm_native_kernels (BF16 native) and wllm_native_wm_kernels (BF16 ports).
Zero PyTorch tensor creation / arithmetic in forward(). CUDA Graph safe.

Modeled after CKernelQwen3 (wllm_native/models/groot/pipeline_thor.py).
...
import wllm_native_wm_kernels as fwk
...
class CKernelBagel:
    ...
    def forward(self, s=0):
        ...
        for i in range(L):
            ...
            # pins its pointer across graph replays (see wllm_native TorchFlashAttnBackend).
