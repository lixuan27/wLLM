...
STR_BACKEND_ENV_VAR: str = "WLLM_ATTENTION_BACKEND"


def find_nccl_library() -> str:
    """
    We either use the library file specified by the `WLLM_NCCL_SO_PATH`
    environment variable, or we find the library file brought by PyTorch.
    After importing `torch`, `libnccl.so.2` or `librccl.so.1` can be
    found by `ctypes` automatically.
    """
    so_file = envs.WLLM_NCCL_SO_PATH

    # manually load the nccl library
    if so_file:
        logger.info(
            "Found nccl from environment variable WLLM_NCCL_SO_PATH=%s",
            so_file)
    else:
...
def import_pynvml():
    ...
    """
    import wllm.serving.third_party.pynvml as pynvml
    return pynvml
...
def get_ip() -> str:
    ...
    warnings.warn(
        ...
        "The value can be set by the environment variable WLLM_HOST_IP.",
        stacklevel=2)
    return "0.0.0.0"
...
def get_loopback_ip() -> str:
    loopback_ip = envs.WLLM_LOOPBACK_IP
    if loopback_ip:
        return loopback_ip

    # WLLM_LOOPBACK_IP is not set, try to get it based on network interface
    ...
    if test_loopback_bind("127.0.0.1", socket.AF_INET):
        ...
        raise RuntimeError(
            ...
            "Set the WLLM_LOOPBACK_IP environment variable explicitly.")
...
def force_spawn() -> None:
    if os.environ.get("WLLM_WORKER_MULTIPROC_METHOD") == "fork":
        logger.warning("We must use the `spawn` multiprocessing start method.")
        os.environ["WLLM_WORKER_MULTIPROC_METHOD"] = "spawn"


def get_mp_context() -> BaseContext:
    """Get a multiprocessing context with a particular method (spawn or fork).
    By default we follow the value of the WLLM_WORKER_MULTIPROC_METHOD to
    determine the multiprocessing method (default is fork). However, under
    certain conditions, we may enforce spawn and override the value of
    WLLM_WORKER_MULTIPROC_METHOD.
    """
    force_spawn()
    mp_method = envs.WLLM_WORKER_MULTIPROC_METHOD
