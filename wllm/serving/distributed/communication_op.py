...
from wllm.serving.distributed.utils import (unpad_sequence_tensor,
...
def global_broadcast(input_: torch.Tensor, src: int = 0) -> torch.Tensor:
    if not torch.distributed.is_initialized():
        return input_
    return get_world_group().broadcast(input_, src)

def global_barrier():
    if not torch.distributed.is_initialized():
        return None
    return get_world_group().barrier()
...
def sequence_model_parallel_shard(input_: torch.Tensor,
    ...
    if sp_world_size <= 1:
        return input_
    input_ = torch.chunk(input_, dim=dim, chunks=sp_world_size)[sp_rank]
