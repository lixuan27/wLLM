...
def padded_head_count_for_sp(num_heads: int, sp_world_size: int) -> int:
    """Smallest H' >= num_heads such that H' % sp_world_size == 0 and
    (H' / sp_world_size) % 2 == 0. The two-at-a-time constraint comes
    from the apply_rope_qkv triton kernel autotuning at BH=2; the
    divisibility constraint comes from the equal-shard all_to_all_4D.
    """
    if sp_world_size <= 1:
        return num_heads
    step = 2 * sp_world_size
    return (num_heads + step - 1) // step * step
