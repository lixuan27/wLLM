...
def _ensure_vllm_single_rank_runtime(
    vllm_config: VllmConfig,
    *,
    device: torch.device,
) -> None:
    """Initialize the minimal vLLM runtime state needed by detached modules."""
    ...
    with set_current_vllm_config(vllm_config):
        if not torch.distributed.is_available() or not torch.distributed.is_initialized():
            ...
            init_distributed_environment(
                world_size=1,
                rank=0,
                distributed_init_method=_free_tcp_init_method(),
...
class VllmOmniTalker(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        _ensure_vllm_single_rank_runtime(self.vllm_config, device=device)
    ...
    def load_weights_from_checkpoint(self) -> None:
        ...
        with torch.cuda.device(self.device):
            ...
            # On gfx950 the vLLM CUDAGraphWrapper (FULL mode) replay in
            # forward_decode / talker_mtp_forward races with the async
            # static-buffer input copies, surfacing as a queue-level
            # HSA_STATUS_ERROR_EXCEPTION. Set
            # WLLM_DISABLE_TALKER_CUDAGRAPH=1 to leave both graphs as None,
            # which falls the helpers back to their eager runnables.
            disable_cudagraph = os.environ.get(
                "WLLM_DISABLE_TALKER_CUDAGRAPH", ""
            ) in ("1", "true", "TRUE")
            if (
                not disable_cudagraph
...
def load_vllm_talker_model(
    ...
    *,
    ...
    logger.info("vLLM-Omni Talker ready on %s (dtype=%s)", device, dtype)
