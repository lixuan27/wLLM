...
# huggingface class name: (component_name, wllm module name, wllm class name)
_TEXT_TO_VIDEO_DIT_MODELS = {
    "LiveAvatarTransformer3DModel": ("dit", "liveavatar", "LiveAvatarTransformer3DModel"),
    "KreaRealtimeTransformer3DModel": ("dit", "krea_realtime", "KreaRealtimeTransformer3DModel"),
    "WorldPlayWanTransformer3DModel": ("dit", "wan_worldplay", "WorldPlayWanTransformer3DModel"),
}
...
_VAE_MODELS = {
    "AutoencoderKLWan": ("vae", "wan_vae", "AutoencoderKLWan"),
}
...
_WLLM_MODELS = dict(_DISCOVERED_MODELS)
for model_arch, spec in _LEGACY_WLLM_MODELS.items():
    if model_arch in _WLLM_MODELS:
        continue
...
ModelRegistry = _ModelRegistry({
    ...
    for model_arch, (component_name, mod_relname,
                     cls_name) in _WLLM_MODELS.items()
})
