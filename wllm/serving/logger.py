...
"""Logging configuration for wllm."""
import datetime
import json
import logging
import os
...
import wllm.serving.envs as envs

WLLM_CONFIGURE_LOGGING = envs.WLLM_CONFIGURE_LOGGING
WLLM_LOGGING_CONFIG_PATH = envs.WLLM_LOGGING_CONFIG_PATH
WLLM_LOGGING_LEVEL = envs.WLLM_LOGGING_LEVEL
WLLM_LOGGING_PREFIX = envs.WLLM_LOGGING_PREFIX
...
_FORMAT = (f"{WLLM_LOGGING_PREFIX}%(levelname)s %(asctime)s.%(msecs)03d "
...
def _configure_wllm_root_logger() -> None:
    logging_config = dict[str, Any]()

    if not WLLM_CONFIGURE_LOGGING and WLLM_LOGGING_CONFIG_PATH:
        raise RuntimeError(
            "WLLM_CONFIGURE_LOGGING evaluated to false, but "
            "WLLM_LOGGING_CONFIG_PATH was given. WLLM_LOGGING_CONFIG_PATH "
            "implies WLLM_CONFIGURE_LOGGING. Please enable "
            "WLLM_CONFIGURE_LOGGING or unset WLLM_LOGGING_CONFIG_PATH."
        )

    if WLLM_CONFIGURE_LOGGING:
        logging_config = DEFAULT_LOGGING_CONFIG

    if WLLM_LOGGING_CONFIG_PATH:
        if not path.exists(WLLM_LOGGING_CONFIG_PATH):
            raise RuntimeError(
                "Could not load logging config. File does not exist: %s",
                WLLM_LOGGING_CONFIG_PATH)
        with open(WLLM_LOGGING_CONFIG_PATH, encoding="utf-8") as file:
...
def init_logger(name: str) -> _wLLMLogger:
    """The main purpose of this function is to ensure that loggers are
    retrieved in such a way that we can be sure the root wllm logger has
    ...
    return cast(_wLLMLogger, logger)


# The root logger is initialized when the module is imported.
# This is thread-safe as the module is only imported once,
# guaranteed by the Python GIL.
_configure_wllm_root_logger()
...
def _trace_calls(log_path, root_dir, frame, event, arg=None):
    if event in ['call', 'return']:
        ...
        if not filename.startswith(root_dir):
            # only log the functions in the wllm root_dir
            return
...
def enable_trace_function_call(log_file_path: str, root_dir: str | None = None):
    ...
    """
    logger.warning(
        "WLLM_TRACE_FUNCTION is enabled. It will record every"
        " function executed by Python. This will slow down the code. It "
        "is suggested to be used for debugging hang or crashes only.")
    logger.info("Trace frame log is saved to %s", log_file_path)
    if root_dir is None:
        # by default, this is the wllm root directory
