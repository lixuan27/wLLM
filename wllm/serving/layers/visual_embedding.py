...
class WanCamControlPatchEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.proj = ReplicatedLinear(self.in_features,
                                    embed_dim,
                                    bias=bias)
...
class TimestepEmbedder(nn.Module):
    ...
    def __init__(
        ...
        self.linear_1 = ReplicatedLinear(
            ...
            hidden_size,  # For activation func like SiLU that need 2x width
            params_dtype=dtype)

        self.act = get_act_fn(act_layer)
        self.linear_2 = ReplicatedLinear(hidden_size,
                                       hidden_size,
                                       params_dtype=dtype)
    ...
    def forward(self,
        ...
        if timestep_seq_len is not None:
            t = t.unflatten(0, (1, timestep_seq_len))
        t = t.to(self.linear_1.weight.dtype)
        
        t_emb, _ = self.linear_1(t)
...
@CustomOp.register("timesteps")
class Timesteps(CustomOp):
    ...
    def forward_native(self, timesteps: torch.Tensor) -> torch.Tensor:
    ...
    def forward_cuda(self, timesteps: torch.Tensor) -> torch.Tensor:

        return self.forward_native(timesteps)
...
class PixArtAlphaTextProjection(nn.Module):
    ...
    def __init__(self, in_features, hidden_size, out_features=None, act_fn="gelu_pytorch_tanh"):
        super().__init__()
        if out_features is None:
            out_features = hidden_size
        self.linear_1 = ReplicatedLinear(in_features, hidden_size)
        self.act_1 = get_act_fn(act_fn)
        self.linear_2 = ReplicatedLinear(hidden_size, out_features)
