...
from wllm.serving.distributed import (
    divide,
    get_tp_rank,
    get_tp_world_size,
...
# yapf: disable
from wllm.serving.models.parameter import (BasevLLMParameter,
...
class MergeRowReplicatedLinear(LinearBase):
    """Merge Row Replicated linear layer.
    ...
    def __init__(
        ...
        self.quant_method.create_weights(
            ...
            self.output_size,
            self.params_dtype,
            weight_loader=self.weight_loader_w,
        )
        ...
        if bias:
            ...
            set_weight_attrs(
                ...
                {
                    "output_dim": 0,
                    "weight_loader": self.weight_loader_b,
                },
            )
        else:
            self.register_parameter("bias", None)

    def weight_loader_w(self, param: Parameter,
                      loaded_weight: torch.Tensor, partition_id: int) -> None:
        partition_id = int(partition_id)
        assert (partition_id < self.num_partitions) and (partition_id >= 0)
        assert loaded_weight.shape == (self.output_size, self.partition_size)
        param.data[:, partition_id * self.partition_size: (partition_id + 1) * self.partition_size].copy_(loaded_weight)

    def weight_loader_b(self, param: Parameter,
                      loaded_weight: torch.Tensor, partition_id: int) -> None:
        partition_id = int(partition_id)
        assert (partition_id < self.num_partitions) and (partition_id >= 0)
        assert param.size() == loaded_weight.size(), (
