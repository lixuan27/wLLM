"""Streaming VAD segmenter — behavior-identical to the reference worker's
``StreamingVADSegmenter`` (``wllm/apps/longlive/reference/worker.py``).

Vendored here (not imported from the reference) so every variant
shares the exact VAD the reference uses; any difference in utterance
boundaries would otherwise show up as a spurious prompt/output mismatch.
"""
