...
# [VENDORED] from wllm/apps/qwen3_omni/reference/runner.py (read-only). Divergence:
# loads the Talker via the TP-aware vendored loader so the talker can be
# sharded across `tp_size` SPMD ranks; the helpers + step logic are
# unchanged so the codec frames stay bit-exact at tp_size=1 and the TP
# math is validated by teacher-forced logit parity.
from wllm.apps.qwen3_omni.backend.cuda.talker_tp.qwen3_omni_talker_tp import (
...
class Qwen3OmniTalkerRunner:
    ...
    def __init__(
        ...
        *,
        ...
        mtp_top_p: float = 0.8,
        debug: bool = False,
        # Tensor-parallel: tp_size>1 shards the talker across `tp_world_size`
        # SPMD ranks sharing `tp_init_method`. Each rank passes its own
        # gpu_index (its physical device) and distinct tp_rank.
        tp_size: int = 1,
        tp_rank: int = 0,
        tp_world_size: int = 1,
        tp_init_method: Optional[str] = None,
        ...
        # Skip when eager: there is no torch.compile to warm, and under TP
        # the warmup's ~20 dummy forwards each pay a full per-layer all-reduce
        # over PCIe (no NVLink) -> minutes of pointless startup. The first
        # real prompt's forward is the same SPMD path either way.
        if not bool(enforce_eager):
