...
def norm_kernel(
    ...
    for base in tl.static_range(0, D, BLK):
        x = tl.load(x_row + base + offs).to(tl.float32)
...
def norm_fp32(x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor, eps: float):
    B, S, D = x.shape
    assert B == 1 and D == 3072
    assert weight.shape == (D,)
    assert bias.shape == (D,)

    o = torch.empty((B, S, D), device=x.device, dtype=torch.bfloat16)
    grid = (S,)
...
def add_gate_kernel(
    ...
    for base in tl.static_range(0, D, BLK):
        x = tl.load(x_row + base + offs).to(tl.float32)
        y = tl.load(y_row + base + offs).to(tl.float32)
        gate = tl.load(gate_msa_row + base + offs, mask=base+offs<D).to(tl.float32)
        z = (x + y * gate).to(tl.bfloat16)
        tl.store(o_row + base + offs, z, mask=base+offs<D)
...
def add_gate(
    ...
    add_gate_kernel[grid](x, y, gate_msa, o, token_length, D)
