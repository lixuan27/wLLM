...
inference framework `wllm_native/`; will be released as a separate package
...
[`wllm_native/core/rl/`](../wllm_native/core/rl/) and the same FP8
...
wLLM's training stack is one of several PyTorch options the
...
path on Ada/Blackwell consumer GPUs**: the same kernels wLLM's
...
- Trained LoRA checkpoints load directly into the wLLM inference
...
│   ├── advantage.py / reward.py / value_function.py — see wllm_native/core/rl/
...
[`wllm_native/core/rl/`](../wllm_native/core/rl/) (`acp_tags.py`,
...
[`wllm_native/core/rl/`](../wllm_native/core/rl/) so train and serve
...
wLLM's `Pi05TorchFrontendRtx` — can load it directly.
...
| RECAP / ACP algorithm primitives ([`wllm_native/core/rl/`](../wllm_native/core/rl/)) | no | ✅ pure paper-aligned algorithm — tag strings, advantage, soft-bin loss, CFG combine |
...
rank 16; the wLLM rows use `Pi05Trainer.compile(InjectionConfig(
...
| wLLM FP8+LoRA, sync data, eager | True | None | 1.46 | 5.84 | 13.88 GB |
...
The two wLLM bottom rows are the recommended starting points,
...
`training/_runs/` archive), measured directly for the wLLM
...
| wLLM, speed-priority recipe | **~124 min** (≈2 h 4 min, measured) |
| wLLM, memory-priority recipe | **~127 min** (≈2 h 7 min, extrapolated) |

The wLLM speed-priority number is from a real 30k-step LIBERO
...
`cache_bf16_weight=False` and get effectively the same wLLM
