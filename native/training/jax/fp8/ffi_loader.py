"""Locate ``wllm_native_jax_ffi.so`` and register its handlers with JAX.

The ``.so`` is installed by the CMake build into the package root
(``wllm_native/wllm_native_jax_ffi.so``). Two XLA FFI handlers are
...
def _candidate_so_paths() -> list[Path]:
    """Return possible install locations of ``wllm_native_jax_ffi.so``."""
    here = Path(__file__).resolve()
    candidates: list[Path] = []
    # 1. Adjacent to the wLLM package: <repo>/wllm_native/wllm_native_jax_ffi.so
    repo_root = here.parents[3]                       # training/jax/fp8 → repo
    candidates.append(repo_root / "wllm_native" / "wllm_native_jax_ffi.so")
    # 2. CMake build dir during dev: <repo>/build/wllm_native_jax_ffi.so
    candidates.append(repo_root / "build" / "wllm_native_jax_ffi.so")
    # 3. Optional override
...
def get_so_path() -> Path:
    """Return the resolved path to ``wllm_native_jax_ffi.so`` or raise."""
    for p in _candidate_so_paths():
        if p.is_file():
            return p
    raise FileNotFoundError(
        "wllm_native_jax_ffi.so not found. Candidates checked: "
        + ", ".join(str(p) for p in _candidate_so_paths())
        + ". Build the JAX FFI target via `cmake --build build "
          "--target wllm_native_jax_ffi`, or set FLASHVLA_JAX_FFI_SO."
    )
