...
This document covers the wLLM NVFP4 inference path for Qwen3.6-27B,
...
long-context runtime uses native wLLM CUDA/CUTLASS kernels and does
...
```python
# 1) Build the kernels (one-time, from the wLLM repo root)
#    cmake -S . -B build && cmake --build build -j --target wllm_native_kernels
#    wllm_native_kernels*.so lands directly in wllm_native/ — no manual cp.
...
# (see §1 below on why).
os.environ['WLLM_QWEN36_MTP_CKPT_DIR'] = '<path-to-FP8-ckpt-dir>'
...
`WLLM_QWEN36_MTP_CKPT_DIR` environment variable. FP8-source MTP is
...
better drafter alignment; set `WLLM_QWEN36_MTP_KEEP_BF16=0` to force
...
The loader source of truth is [`wllm_native/frontends/torch/_qwen36_rtx_nvfp4_weights.py`](../wllm_native/frontends/torch/_qwen36_rtx_nvfp4_weights.py)
...
The default long-context route is `WLLM_QWEN36_LONG_KV_CACHE=fp8`,
...
passes the measured `WLLM_QWEN36_FP8_XQA_MIN_CTX=auto` bucket policy,
and keeps the older FP8->BF16-stage + FA2 bridge in buckets where that
path measured faster.
Set `WLLM_QWEN36_LONG_KV_CACHE=tq` to use the TurboQuant
...
`WLLM_QWEN36_TQ_SPEC_K` to force a fixed K. TQ verify and the MTP
...
linear-attention layers use the native wLLM WY/cuBLASLt Gated
...
`WLLM_QWEN36_FUSE_MLP_GATE_UP=0`.
...
uses `WLLM_QWEN36_TQ_PREFILL_GDN_BACKEND=wy_lt` with the f32 GEMM
...
export WLLM_QWEN36_LONG_CTX_ROUTE_MIN_SEQ=512
```
...
Build (from the wLLM repo root):

```bash
cmake -S . -B build
cmake --build build -j --target wllm_native_kernels
# wllm_native_kernels*.so lands in wllm_native/ via CMake's
...
import os
os.environ['WLLM_QWEN36_MTP_CKPT_DIR'] = '<FP8_CKPT>'
