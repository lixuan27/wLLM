...
first-class wLLM model (`config="cosmos3_video"`). Run it through the standard
`wllm_native.load_model(...)` API.
...
| wLLM **bf16** | 4.4 s | 2.3 s | 6.7 s | 0.9989 | CUDA graph + static text-KV cache |
| wLLM **fp8** | 2.5 s | 2.3 s | 4.8 s | 0.986 | near-lossless |
| wLLM **fp8 + TeaCache(3,5,7) + fp4-VAE** | ~1.8 s | 1.26 s | **~3.0 s** | 0.981 | default-recommended |
...
recomputes the static text tower every step, so most of the ~18× is wLLM's CUDA
...
- **GPU**: RTX 5090 (sm120). wLLM sm120 image (`wllm_native_kernels.so` + `wllm_native_fa2.so`).
- Build the model-local kernels once on the target GPU (isolated; does not rebuild
  `wllm_native_kernels.so`):

```bash
cd wllm_native/models/cosmos3_video/kernels && python3 setup.py build_ext --inplace
```
...
```python
import wllm_native
model = wllm_native.load_model("<weights>", config="cosmos3_video",
...
- **Additive & isolated**: the production `wllm_native_kernels.so` and its CMake are
