...
This page documents the wLLM official-pipeline route for
Wan2.2-TI2V-5B on RTX SM120.

wLLM exposes a stable `set_prompt()` / `infer()` API around the official
Wan Python pipeline. ComfyUI support is handled outside the core repository
through custom nodes that call wLLM public APIs.
...
```bash
export WLLM_WAN22_ROOT=/path/to/Wan2.2/source
...
|---|---:|---:|---:|---:|---|
| wLLM official pipeline | off | 20/20 | **178.6 s** | 24.37 GiB | baseline |
| wLLM official pipeline | 0.3 | 8/20 | **114.2 s** | 24.37 GiB | 1.56x faster; visible quality drift on the test prompt |
