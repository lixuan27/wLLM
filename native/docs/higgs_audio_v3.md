# Higgs Audio v3 TTS-4B on wLLM (RTX 5090 / SM120)
...
> temperature-sampling fallback are both implemented by wLLM-owned CUDA
...
| **wLLM** | Built with `GPU_ARCH=120` (FP8 + BF16) or `GPU_ARCH=89` (BF16 only) — auto-detected from the GPU if unset. See [Build & install](../README.md#build--install) (`cmake .. && make -j` produces `wllm_native/wllm_native_kernels*.so` and `wllm_native/wllm_native_fa2.so`). |
...
> raise on import. The codec never needs it; wLLM neutralises it at import
> time (`wllm_native/models/higgs_audio_v3/_codec/env_guard.py`), so **no manual
...
```python
from wllm.native.frontends.torch.higgs_audio_v3_rtx import HiggsAudioV3TorchFrontendRtx
...
wav = fe.generate("Hello from wLLM.")     # text -> 24 kHz mono waveform [L] (cpu f32)

# Explicit dependency-free CUDA sampling fallback. Greedy stays the default.
wav = fe.generate("Hello from wLLM.", temperature=1.0, seed=1234)

# or split the stages:
codes = fe.predict("Hello from wLLM.")    # [T, 8] acoustic codes (int64, cpu)
...
a repeated-code attractor under greedy decoding in both wLLM and the
...
wLLM kernel: stable softmax, stateless categorical sampling, delay masking,
...
positive temperature, a separate wLLM CUDA helper performs the categorical
...
difference feeds back and compounds, so wLLM, the BF16 reference, and the
...
- **GPU support / precision auto-selection.** The build auto-detects the GPU
  ...
  for **sm89 and sm120**. At construction the frontend follows wLLM's
...
- Codec source: the `bosonai/higgs-audio` v2 tokenizer (decode path only),
  vendored under `wllm_native/models/higgs_audio_v3/_codec/`.
