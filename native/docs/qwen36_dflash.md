...
```
  ...
  1/16/31/46/61). wLLM quantizes every drafter linear to NVFP4 at
...
os.environ["WLLM_QWEN36_MTP_CKPT_DIR"] = "/models/Qwen3.6-27B-FP8"
os.environ["WLLM_QWEN36_DFLASH_CKPT_DIR"] = "/models/Qwen3.6-27B-DFlash"
os.environ["WLLM_QWEN36_LONG_KV_CACHE"] = "fp8"
...
fe.init_dflash_drafter()          # reads WLLM_QWEN36_DFLASH_CKPT_DIR
...
| Env | Default | Meaning |
|---|---|---|
| `WLLM_QWEN36_DFLASH_CKPT_DIR` | unset | Drafter checkpoint directory (required). |
| `WLLM_QWEN36_DFLASH_PERTOKEN` | `1` on Thor | Per-token window mode. |
| `WLLM_QWEN36_DFLASH_WINDOW` | `128` | Per-token window length (tokens, <= 256). |
| `WLLM_QWEN36_DFLASH_WINDOW_SEED` | `1` | Seed the window from the prompt tail at prefill (Thor). |
...
| Env | Default | Meaning |
|---|---|---|
| `WLLM_QWEN36_DFLASH_RELAXED_THINKING` | `0` | Enable relaxed acceptance inside `<think>`. |
| `WLLM_QWEN36_DFLASH_RELAXED_TOPK` | `3` | Candidate set size. |
| `WLLM_QWEN36_DFLASH_RELAXED_DELTA` | `1.0` | Logit margin vs the argmax (equals a log-prob margin). |
...
`WLLM_QWEN36_THOR_LIN_CHUNK_SAVES=1` routes the DFlash verify's
...
- Degenerate prompts that repeat one sentence verbatim can steer the
  ...
  (`WLLM_QWEN36_DFLASH_WINDOW_SEED=0`) for representative numbers.
- Greedy-parity comparisons must use the FP8-KV MTP route as the
  reference (`WLLM_QWEN36_LONG_CTX_ROUTE_MIN_SEQ=0` forces it for
