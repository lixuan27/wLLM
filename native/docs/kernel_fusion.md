...
>
> **TL;DR**:
> - The library ships **~120 pybind entries across three modules** — `wllm_native_kernels` (98 memory-bound / fp8 / cuBLAS-wrapper / FMHA entries), `wllm_native_fp4` (23 NVFP4 entries), and `wllm_native_fa2` (2 FA2 forward entries). Counts can drift ±a handful between releases; treat the number as "about 100", not exact. `wllm_native_kernels` is organized into 10 groups by purpose (see §2 below).
...
wLLM uses **two distinct naming conventions** for the same kernel depending on layer:

- **C++ layer** (`csrc/`, symbol names, file names): named after the **math primitive** the kernel implements, e.g. `wllm_native::fused_fp4::silu_mul_two_fp4_to_fp4` — "apply `silu(a) * b` to two fp4 streams, output fp4". The kernel does not know (or care) what a caller uses it for.
- **Python layer** (`wllm_native_kernels` / `wllm_native_fp4` pybind modules, docs, user-facing API): named after the **semantic role** in the model architecture, e.g. `gate_geglu_merged_fp4`. Different architectures can bind the same C++ primitive under different Python names when the semantics differ.
...
**New-model forwards do not call these directly** — they go through the [`AttentionBackend` protocol](../wllm_native/hardware/backend.py) via `attn.run(site=..., layer_idx=..., ...)`. See [`adding_new_model.md §2.1`](adding_new_model.md).
...
in `wllm_native_jax_ffi.so` — the binding is in
...
**Pi0-FAST dispatches on hardware via `hasattr(fvk, 'cutlass_fp8_sq')`** (see [`frontends/torch/pi0fast.py`](../wllm_native/frontends/torch/pi0fast.py)). If your new model needs to run on both Thor and RTX, follow the runtime-fork pattern used by Pi0-FAST.
...
Compared against the Myelin-compiled version (70.2ms) which implements the same logic in **~20 kernels**, wLLM has folded 3+3 elementwise + quant ops into the fused RMSNorm (steps 1 and 7) and another 3 into `gate_geglu` (step 9).
