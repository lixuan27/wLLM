...
through `wllm_native/subgraphs/` modules before graph capture.
...
Built-in and customer cuts live under `wllm_native/subgraphs/`:

```
wllm_native/subgraphs/
...
```python
from wllm.native.subgraphs.pi05.context_action import enable
...
`wllm_native.subgraphs.pi05.stage_plans` declares:
...
```python
from wllm.native.subgraphs.pi05.rtc_prefix import enable
...
```python
from wllm.native.subgraphs.pi05.rtc_vjp_guided import enable
...
This mode is intentionally provider-based. Pi0.5's stock wLLM pipeline is an
...
```python
from wllm.native.subgraphs.capture import (
    capture_graph,
...
```python
from wllm.native.subgraphs.stage_plan import StagePlan, register_stage_plan
