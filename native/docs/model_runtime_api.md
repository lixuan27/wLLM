...
Authoritative header: [`runtime/include/wllm/model_runtime.h`](../runtime/include/wllm/model_runtime.h)
...
Providers with no wLLM resources use
...
(`wllm_native/models/pi05/runtime_export.py`, via
`wllm_native.runtime.export.build_model_runtime`) and the native Pi0.5 verb
...
```
wllm_native/models/<model>/runtime_export.py     ports, stages, identity, export helpers
wllm_native/models/<model>/pipeline_rtx.py       RTX graph capture and live buffers
wllm_native/models/<model>/pipeline_thor.py      Thor graph capture and live buffers
...
- `stage_plan="context_action"` exports `context -> decode_only` only after
  enabling `wllm_native.subgraphs.pi05.context_action` before graph capture.
- `stage_plan="context_rtc_prefix_action"` exports
  `context -> decode_rtc_prefix` only after enabling
  `wllm_native.subgraphs.pi05.rtc_prefix` before graph capture. This mode adds
...
Stage-plan names are resolved by `wllm_native.subgraphs.stage_plan`:

```python
from wllm.native.subgraphs.stage_plan import Stage, StagePlan, register_stage_plan
...
validated in the wLLM-Nexus repository.
