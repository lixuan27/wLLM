...
wLLM multimodal inference path for
...
image + text prompt, 1581 LLM tokens (1564 vision + 17 text), wLLM.png
```

| Metric | HF SDPA (bf16) | wLLM |
|---|---:|---:|
| TTFT (full request, image+text) | 206 ms | **~100 ms** |
| Decode (warm CUDA Graph)   | ~48 tok/s | **~150 tok/s** |

Same 5090, wLLM.png, full resolution (1581 prompt tokens). wLLM TTFT
...
python examples/qwen3_vl_quickstart.py \
    --checkpoint /path/to/Qwen3-VL-8B-wLLM-NVFP4 \
    --image wLLM.png --benchmark 20
```
...
```bash
# 1. Build the wLLM checkpoint once (NVFP4 language linears +
...
python tools/quantize_qwen3_vl_nvfp4.py \
    ...
    --dst /path/to/Qwen3-VL-8B-wLLM-NVFP4
...
fe = Qwen3VlTorchFrontendRtx('/path/to/Qwen3-VL-8B-wLLM-NVFP4')
messages = [{'role': 'user', 'content': [
    {'type': 'image', 'image': Image.open('wLLM.png').convert('RGB')},
    {'type': 'text', 'text': 'Describe this image in one sentence.'},
]}]
print(fe.generate(messages, max_new_tokens=128))
```

The build needs the `wllm_native_qwen3_vl_kernels` module
(`cmake -B build -S . -DGPU_ARCH=120 -DWLLM_BUILD_QWEN3_VL=ON` then
build that target); the shared `wllm_native_kernels.so` is unchanged.
...
New kernels (general, in the separate `wllm_native_qwen3_vl_kernels` module):
