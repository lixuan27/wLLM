...
>
> **Source**: [`wllm_native/core/quant/calibrator.py`](../../wllm_native/core/quant/calibrator.py) — cache implementation. Per-frontend `_calibrate(...)` methods follow the same protocol.

---

## 1. What the calibration framework owns

Calibration is the most cross-cutting piece of wLLM's quantization story. It is the only component that:

- runs per-checkpoint AND per-sequence-length
- has a disk side-effect (`~/.wllm/calibration/`)
...
```
~/.wllm/calibration/{ckpt_hash}_Se{N}.json
```

`save_calibration` and `load_calibration` in [`calibrator.py`](../../wllm_native/core/quant/calibrator.py) are the only public read / write entry points.
...
To force recalibration: `rm ~/.wllm/calibration/{ckpt_hash}_Se*.json`. There is no environment flag to disable the cache; if you want fresh calibration every run, delete the file in your boot script.
...
```python
from wllm.native.core.quant.calibrator import (
...
In TensorRT, calibration is a build-time concern — the calibrator runs during `trtexec`, the result is baked into a `.engine` file, and you cannot recalibrate without rebuilding. wLLM's calibration:
...
| What | File |
|---|---|
| Cache key + save / load + path constants | [`wllm_native/core/quant/calibrator.py`](../../wllm_native/core/quant/calibrator.py) |
| Mechanics doc (FP8 dynamic range, scales, alpha math) | [`../calibration.md`](../calibration.md) |
| Pi0.5 frontend `_calibrate` | `wllm_native/frontends/torch/pi05_thor.py` |
| GROOT frontend `_calibrate` | `wllm_native/frontends/torch/groot_thor.py` |
| Multi-sample helper | `tests/test_thor_multi_sample_calibrate.py` |
| AWQ refit utility | `wllm_native/core/quant/awq.py` (when present) |
...
1. Cache directory `~/.wllm/calibration/`.
