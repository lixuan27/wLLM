...
>
> **Source**: [`wllm_native/hardware/backend.py`](../../wllm_native/hardware/backend.py) — protocol + base class + `SiteSpec` / `AttentionSpec`. Concrete backends live in `wllm_native/hardware/{thor,rtx}/attn_backend*.py`.

---

## 1. The problem this solves

Every model in wLLM has 1–4 distinct attention shapes (for example, GROOT has SigLIP vision, Qwen3 backbone, DiT self-attention, DiT cross-attention). Each shape has its own preferred kernel, and each hardware target has its own preferred kernel for the same shape:
...
```python
from wllm.native.hardware import make_attention_backend
...
| What | File |
|---|---|
| Protocol, `SiteSpec`, `AttentionSpec`, base class | [`wllm_native/hardware/backend.py`](../../wllm_native/hardware/backend.py) |
| Thor Pi0/Pi0.5 backend | `wllm_native/hardware/thor/attn_backend.py` |
| Thor GROOT backend | `wllm_native/hardware/thor/attn_backend_groot.py` |
| RTX Pi0/Pi0.5 backend | `wllm_native/hardware/rtx/attn_backend.py` |
| RTX GROOT backend | `wllm_native/hardware/rtx/attn_backend_groot.py` |
| Pipeline call site (Pi0.5, all hardware) | `wllm_native/models/pi05/pipeline.py` |
| `make_attention_backend` resolver | `wllm_native/hardware/__init__.py` |
