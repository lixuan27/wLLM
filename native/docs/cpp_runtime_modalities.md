...
- `runtime/include/wllm/runtime.h`
- `frt_runtime_export_v1`
...
- action: `(chunk, 32)` model output -> first 7 robot dims, unnormalized by
  deployment stats.
- `wllm::models::pi05::RuntimeIo` binds those specs to concrete tensor
  views and exposes `prepare_vision()` / `read_actions()`.
- `wllm::models::pi05::Runtime` is the full C++ runtime shell for the
...
- complete build target: `wllm_cpp_pi05`;
- C host ABI target: `wllm_cpp_pi05_c`, exporting
