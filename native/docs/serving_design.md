# wLLM serving design — graph-replay-native execution-state capsules

This document explains the **signature serving idea** of wLLM and why it is
...
> blocks). **wLLM keeps full-graph capture and buys prefix reuse back with
...
wLLM — not by taste, but by construction:
...
gathers KV from arbitrary physical blocks every forward. wLLM captures the
...
```
  ...
  ───────────────────────────────────────────────────────────────────────────────────
  wllm_native/ (frontend) snapshot_capsule() / restore_capsule(): copy the boundary buffers
...
The flag is not "we also have prefix caching". It is: **wLLM sessions are
...
1. **Frontend snapshot/restore (wllm_native/) — DONE, in-GPU (D2D).**
...
3. **Pin API.** Let the host pin a capsule for the shared prefix
   ...
   OpenAI-side field (`wllm_pin_prefix`) or a `/v1/sessions` capsule option.
...
different point in the design space, chosen for wLLM's scenario: latency-first,
...
prefix discovery and paged memory. If that is the workload, use them — wLLM does
not target it.
...
- **Keeping full-graph capture while still reusing work.** Paged caching is what
  ...
  up a single captured graph over the whole forward. wLLM keeps the captured
...
6. **Scenario comparison study (post-development).** A fair, measured comparison on
   wLLM's turf — consumer GPU (5090) and edge (Orin/Thor), single/few-session
