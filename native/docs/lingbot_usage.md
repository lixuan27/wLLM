...
> `wllm_native.load_model` does **not** dispatch a `lingbot` config; this is **not**
> the stable `load_model()` API. `LingbotTorchFrontendThor`
> (`wllm_native/frontends/torch/lingbot_thor.py`) is a **G1 scaffold** whose methods
> raise `NotImplementedError`. Use `examples/lingbot_quickstart.py` /
> `benchmarks/lingbot_thor_latency.py`, not `wllm_native.load_model("lingbot")`.
...
compiled **into `wllm_native_kernels`** — same pattern as the qwen36 kernels in
`csrc/kernels/`. There is **no separate `wllm_native_lingbot.so`**. The kernels are
`lingbot_`-prefixed and gated behind `ENABLE_LINGBOT`, built only when
`WLLM_ENABLE_LINGBOT=ON` (default) **and** `GPU_ARCH=110` (Thor). RTX
(sm_120) / Orin / L40 builds compile neither the sources nor the bindings.

```bash
git clone --depth 1 --branch v4.4.2 \
    https://github.com/NVIDIA/cutlass.git third_party/cutlass
cmake -B build -S . -DGPU_ARCH=110            # -DWLLM_ENABLE_LINGBOT=OFF to skip
cmake --build build -j --target wllm_native_kernels wllm_native_fp4 fmha_fp16_strided
...
```bash
python - <<'PY'
import wllm.native.wllm_kernels as k
...
- The FA4 forward source is **vendored, trimmed, and privately namespaced** at
  `csrc/attention/flash_attn_4_src/` (package `wllm_fa4`, a forward /
  ...
  > packaging is needed later, move the vendor under `wllm_native/` or add it to
  > `package-data`.
- The import is isolated in `wllm_native/hardware/thor/fa4_backend.py` — the only
...
```bash
WLLM_THOR_FA4=1 python benchmarks/lingbot_thor_latency.py ...   # FA4
WLLM_THOR_FA4=0 python benchmarks/lingbot_thor_latency.py ...   # force fmha
```
...
`csrc/attention/flash_attn_4_src` exists, (3) `WLLM_THOR_FA4` is not `0`.
...
| wLLM (FA4)           | 10 | 0.996245 |  64.1 ms |
| wLLM (fmha fallback) | 10 | 0.996067 |  73.0 ms |
| wLLM (FA4)           | 25 | 0.995721 |  97.5 ms |
| wLLM (fmha fallback) | 25 | 0.994890 | 118.1 ms |
| wLLM (FA4)           | 50 | 0.995455 | 155.8 ms |
| wLLM (fmha fallback) | 50 | 0.994928 | 193.8 ms |
...
| steps | TRT aligned FP4 loop | wLLM full E2E | Speedup |
|------:|---------------------:|-----------------:|--------:|
