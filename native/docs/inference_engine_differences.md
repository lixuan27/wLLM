...
> Pick by **workload**, not by tok/s. Each engine below is the right answer for some scenario; wLLM is the right answer for small-batch realtime fixed-architecture workloads. Small batches (CFG / multi-policy / multi-frame) are supported; what's not the design point is high-batch throughput.
...
| **Fixed-arch multimodal** (VLA, world model, audio gen, realtime DiT) | **wLLM** | TensorRT (after build) |

---

## 2. User workflow — checkpoint to running production

| Stage | TensorRT | vLLM / SGLang | wLLM |
|---|---|---|---|
...
| Calibrate | baked into `.engine` at build time | n/a (LLM-side, online) | first call, ~3 s, JSON cached to `~/.wllm/calibration/` |
...
| Step | TensorRT | vLLM / SGLang | wLLM |
|---|---|---|---|
...
| **wLLM** | Hand-written forward per `(model, framework, hardware)` | No compile, no export, direct kernel control, fast dev loop |
...
| **Small-batch realtime VLA / robotics / on-device DiT or audio gen** | **wLLM** |

Multiple stacks coexisting in one project is normal, not a contradiction. A robot running Pi0.5 control on wLLM at 23 Hz alongside an LLM agent on vLLM solves two different problems.

---

## 5. Today's scope

wLLM today focuses on the small-batch realtime / fixed-architecture workload above. A few related capabilities are deliberately handled by other layers, with explicit room for future extension:
...
- **Multi-tenant serving** — one process serves one captured graph. A fan-out serving layer on top of wLLM workers is an explicit extension path for B≥1 realtime services; cross-request continuous batching is what vLLM / SGLang are shaped for.
...
- [`architecture.md`](architecture.md) — wLLM's eight infra components
