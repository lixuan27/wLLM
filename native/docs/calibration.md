...
> - Calibration cache location: `~/.wllm/calibration/{ckpt_hash}_Se{N}.json`.
...
**How this compares to vLLM and similar frameworks**: vLLM defaults to eager BF16 with no FP8 quantization. When AWQ or GPTQ is enabled, the quantization is **weight-only** (activations stay in FP16). wLLM runs **W8A8**, which requires calibrating both sides.
...
Computed when loading the checkpoint, inside [`_load_weights`](../wllm_native/frontends/torch/pi05_thor.py) via the `Quant()` transform in WEIGHT_SPEC:

```python
# wllm_native/core/thor_frontend_utils.py
def quant_fp8(w):
    w = w.contiguous()
...
Reference implementations: [`shared_primitives.py::encoder_forward_calibrate`](../wllm_native/hardware/thor/shared_primitives.py) (lines 491-610) and [`decoder_forward_calibrate`](../wllm_native/hardware/thor/shared_primitives.py) (from line 610 on).
...
`~/.wllm/calibration/{ckpt_hash}_Se{N}.json`, where:
- `ckpt_hash` = SHA256(first 64 KB + file_size), first 16 hex chars (see [`calibrator.py::_checkpoint_hash`](../wllm_native/core/quant/calibrator.py)).
...
Automatic invalidation conditions in [`load_calibration`](../wllm_native/core/quant/calibrator.py):
...
Manual invalidation:
```bash
rm ~/.wllm/calibration/{ckpt_hash}_Se{N}.json
# or wipe everything:
rm -rf ~/.wllm/calibration/
```
...
| Content | File |
|------|------|
| `quant_fp8` weight quantization function | [`wllm_native/core/thor_frontend_utils.py`](../wllm_native/core/thor_frontend_utils.py) |
| Calibration-cache read/write | [`wllm_native/core/quant/calibrator.py`](../wllm_native/core/quant/calibrator.py) |
| `encoder_forward_calibrate` / `decoder_forward_calibrate` | [`wllm_native/hardware/thor/shared_primitives.py`](../wllm_native/hardware/thor/shared_primitives.py) |
| Pi0.5 `_calibrate` example | [`wllm_native/frontends/torch/pi05_thor.py`](../wllm_native/frontends/torch/pi05_thor.py) (L499-649) |
| `_recalibrate_with_real_data` | [`wllm_native/frontends/torch/pi05_thor.py`](../wllm_native/frontends/torch/pi05_thor.py) (L954) |
| GROOT split calibrate (Qwen3 + DiT separate) | [`wllm_native/frontends/torch/groot_thor.py`](../wllm_native/frontends/torch/groot_thor.py), [`wllm_native/frontends/torch/groot_rtx.py`](../wllm_native/frontends/torch/groot_rtx.py) |
...
pose variation that a single frame does not cover — wLLM also
...
via [`wllm_native.datasets.libero`](../wllm_native/datasets/libero.py):
...
two FP8 outputs** (different calibrations of the same wLLM pipeline)
...
[`wllm_native/frontends/torch/pi05_thor.py`](../wllm_native/frontends/torch/pi05_thor.py)
...
`groot_rtx` (see
[`wllm_native/frontends/torch/groot_rtx.py`](../wllm_native/frontends/torch/groot_rtx.py)
