...
Both paths end at the same verification step — `import wllm_native;
wllm_native.__version__` returns the installed version, and
`wllm_native.wllm_kernels` is importable.
...
| Python | 3.10 / 3.11 / 3.12 | One venv; the interpreter that runs `cmake` MUST match the interpreter that later imports `wllm_native` |
| GCC / G++ | 11+ (C++17) | |
| CMake | 3.24+ | |
...
wLLM's main FP8/FP4 GEMM path is built against **CUTLASS 4.x**, not
...
the `wllm_native/` source tree; only editable install makes that
directory importable without an extra copy step. A plain
`pip install .` would snapshot `wllm_native/` BEFORE the kernels are
built, and `import wllm_native` later would fail with a missing
`wllm_native_kernels` error.
...
step. CMake writes every `.so` directly into `wllm_native/` at build
...
| Target  | `wllm_native_kernels.so` | `wllm_native_fp4.so` | `wllm_native_fa2.so` | `libfmha_fp16_strided.so` |
|---------|:----------------------:|:------------------:|:------------------:|:-------------------------:|
...
The default build keeps wLLM's broad compatibility surface. It compiles
...
```bash
cmake -B build -S . -DGPU_ARCH=<arch> -DWLLM_SLIM_BUILD=ON
cmake --build build -j$(nproc) --target wllm_native_kernels
```

`WLLM_SLIM_BUILD` is OFF by default and only changes what is compiled into
`wllm_native_kernels`. It does not change kernel math, launch parameters, dtype
...
Do not use `WLLM_SLIM_BUILD=ON` for compatibility builds or for model paths
...
```bash
python -c "
import wllm_native, torch, numpy
print('wllm_native:', wllm_native.__version__)
print('torch    :', torch.__version__, torch.cuda.get_device_capability())
print('numpy    :', numpy.__version__)
from wllm.native import wllm_native_kernels
print('kernels CUTLASS SM100:', wllm_native_kernels.has_cutlass_sm100())
"
```
...
If `import wllm_native` fails with "no module named wllm_native_kernels",
...
vendored `wllm_native_fa2.so` (built from `csrc/attention/flash_attn_2_src/`)
...
The GROOT N1.6 / N1.7 RTX backends also use wLLM's vendored
...
- Orbax 0.6+ changed the default metadata layout for `StandardRestore`;
  our `load_from_cache` path in `wllm_native/frontends/jax/` expects
  the 0.5.x layout.
...
wLLM does not bundle model weights. Bring your own Pi0 / Pi0.5 /
...
| `No module named 'wllm_native_kernels'` | Step 6's `cp *.so` step skipped, OR non-editable install |
...
environment that imports `wllm_native`:
