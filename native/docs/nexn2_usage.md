...
`wllm_native.load_model(config="nexn2")` raises with a redirect; construct the
frontend directly.
...
| Weight quant | `nvfp4` |
| Build flag | `-DWLLM_ENABLE_QWEN35MOE=ON` (required) |
...
```bash
cmake -S . -B build -DWLLM_ENABLE_QWEN35MOE=ON
cmake --build build -j
```

This produces `wllm_native/wllm_native_kernels*.so` (with the qwen3_5_moe kernels) and
`wllm_native/wllm_native_fa2*.so` (the attention kernel). Omitting the flag still
builds `wllm_native_kernels`, but the qwen3_5_moe symbols are absent and the
...
The kernelized path uses native wLLM CUDA kernels only — no Triton / FLA
Python kernels.

## 3. Quickstart

```python
from wllm.native.frontends.torch.nexn2_rtx import Nexn2TorchFrontendRtx
...
Env knobs: `WLLM_NEXN2_PREFILL_CHUNK` (chunked-prefill block size, default 8192; 0 disables), `WLLM_NEXN2_GRAPH_CACHE_MAX` (decode CUDA-graph LRU cap, default 256).
...
decode 193–259 tok/s. wLLM crosses the prefill target from ~1k context and
...
import time, torch
from wllm.native.frontends.torch.nexn2_rtx import Nexn2TorchFrontendRtx
from wllm.native.frontends.torch._nexn2_rtx_decode import (
...
* Requires the `-DWLLM_ENABLE_QWEN35MOE=ON` build.
* Long context is handled by the chunked prefill (above), so the per-layer
  ...
  `WLLM_NEXN2_PREFILL_CHUNK` (default 8192; lower trades a little throughput
