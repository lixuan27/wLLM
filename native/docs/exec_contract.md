# wLLM Execution Contract (common-layer spec)

> Status: design draft · 2026-05-28 · branch `spec/exec-contract`
> This layer defines wLLM's **common execution contract**: a clean, minimal C ABI that
...
### Mechanism
wLLM runs a model by capturing its kernel sequence into a **CUDA Graph once** (setup), then
...
```
  (A) today — Python frontend drives; exec is the C++ library it calls
      Python frontend ──pybind──▶ C++ exec (libwllm_exec) ──cudaGraphLaunch──▶ GPU kernel graph
  ...
  (B) deployment — a native host drives; Python only at setup (same process) or absent
      C++/Rust host ──C ABI──▶ C++ exec (libwllm_exec.so) ──cudaGraphLaunch──▶ GPU kernel graph
...
Same `libwllm_exec`; the host language is chosen per scenario.
```
...
What wLLM already does (see `wllm_native/core/cuda_graph.py`, `wllm_native/frontends/`):
...
## 2. C ABI (authoritative form: `exec/include/wllm/exec.h`)
...
the `record` callback enqueues onto a stream (wLLM kernels / torch ops / raw CUDA), and only ever
...
exec/                              # NEW top-level: execution-contract layer (orchestration, not kernels; zero csrc dep)
  include/wllm/exec.h           #   public C ABI — the authoritative form of this spec
  src/
  ...
  bindings/
    exec_pybind.cpp                #   pybind -> wllm_native.runtime.exec (dev/migration only)
  CMakeLists.txt                   #   builds libwllm_exec (.so) as an independent target

wllm_native/runtime/exec.py           # thin Python wrapper (sibling to rtc.py), dev only
...
1. **Spec (~1 week)**: this document + `exec/include/wllm/exec.h`. Acceptance: use the §4 table to
...
- [x] Qwen3.6: `WLLM_QWEN36_USE_EXEC=1` (decode + spec + rollback)
- [x] Pi0.5 FP16: `WLLM_PI05_USE_EXEC=1` (`Pi05PipelineFP16`)
- [x] Pi0.5 FP8/default: `WLLM_PI05_USE_EXEC=1` (`Pi05Pipeline`)
...
# wllm_native fa2 with fp16 (needed by Pi0.5 FP16): default FA2_DTYPES already fp16;bf16
cmake -S . -B build -DGPU_ARCH=120 && cmake --build build -j --target wllm_native_fa2
# tests
python exec/tests/test_exec.py
python exec/tests/test_plan_vla.py
python exec/tests/test_adopt_qwen36.py          # needs WLLM_QWEN36_{NVFP4,MTP}_CKPT_DIR
...
- **Drives the model through the frontend, not the contract** — `qwen36_engine.py` is a thin adapter to
  ...
  `WLLM_QWEN36_USE_EXEC`, not from the serving layer.
