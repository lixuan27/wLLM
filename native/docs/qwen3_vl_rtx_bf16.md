...
runtime still uses wLLM fixed-shape CUDA Graph replay and a small set of
...
Build the regular wLLM kernels, FlashAttention module, and Qwen3-VL helper
module for SM87:
...
cmake -B build -S . \
  ...
  -DWLLM_BUILD_QWEN3_VL=ON
cmake --build build -j4 \
  --target wllm_native_kernels wllm_native_fa2 wllm_native_qwen3_vl_kernels
```

On SM87, `wllm_native_qwen3_vl_kernels` provides BF16 Qwen3-VL helper kernels.
...
```python
wllm_native.frontends.torch.qwen3_vl_rtx_bf16.Qwen3VlTorchFrontendRtxBF16
```
...
The language stack uses the generic wLLM BF16 Qwen3 helpers:
...
BF16 prefill GEMMs use a cuBLASLt helper from `wllm_native_qwen3_vl_kernels`.
...
`wLLM.png` at full resolution produces 6256 vision patches and 1581 prompt
...
- Checkpoint: `/root/models/Qwen3-VL-2B-Instruct`
- Workload: `wLLM.png`, prompt `Describe this image in one sentence.`

Local checks:

```bash
python -m py_compile \
  wllm_native/frontends/torch/qwen3_vl_rtx_bf16.py \
...
wLLM BF16 eager (`--no-graph`), and wLLM BF16 graph paths. wLLM eager
...
`WLLM_BF16_CUBLASLT_AUTOTUNE_ALGOS=1`, the same binary measured:
