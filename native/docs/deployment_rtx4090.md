# wLLM RTX 4090 (SM89) Deployment Guide
...
> built into `wllm_native/wllm_native_fa2.so` during `cmake && make`.
...
4. [Build `wllm_native_kernels.so` (for SM89)](#4-build-wllm_native_kernelsso-for-sm89)
...
This work uses **`wllm-rtx-x86`** (custom image built on top of
...
you already have `wllm-rtx-x86` on the 5090 box):

```bash
# On 5090 host
docker save wllm-rtx-x86 | gzip > wllm-rtx-x86.tar.gz
scp wllm-rtx-x86.tar.gz user@4090-host:/tmp/

# On 4090 host
docker load < /tmp/wllm-rtx-x86.tar.gz
```
...
# NOTE: `pip install flash-attn` is NOT required any more. wLLM
# vendors the FA2 fp16 + bf16 fwd kernels at source level under
# csrc/attention/flash_attn_2_src/ and builds them into
# wllm_native/wllm_native_fa2.so during `cmake && make`. The runtime
# imports `from wllm.native import wllm_native_fa2` directly, so there is
...
rsync -avz --exclude='build/' --exclude='__pycache__/' \
      ...
      /path/to/wLLM/ \
      user@4090-host:/path/on/4090/wLLM/
```
...
## 4. Build `wllm_native_kernels.so` (for SM89)
...
-- FA2 pybind module: wllm_native_fa2 (separate .so)
```
...
- **In-SO Flash-Attention 2** (fp16 + bf16) via `wllm_native_fa2.so`.
...
```bash
cp build/wllm_native_kernels.cpython-312-x86_64-linux-gnu.so \
   wllm_native/wllm_native_kernels.cpython-312-x86_64-linux-gnu.so
cp build/wllm_native_fa2.cpython-312-x86_64-linux-gnu.so \
   wllm_native/wllm_native_fa2.cpython-312-x86_64-linux-gnu.so
```

Verify the bindings are loadable:

```bash
python -c "from wllm.native import wllm_native_kernels as fvk; \
...
```bash
python -c "
from wllm.native.hardware import detect_arch, _PIPELINE_MAP
arch = detect_arch()
...
pi0 torch dispatch: ('wllm_native.frontends.torch.pi0_rtx', 'Pi0TorchFrontendRtx')
pi0 jax dispatch:   ('wllm_native.frontends.jax.pi0_rtx',   'Pi0JaxFrontendRtx')
```
...
```bash
python -c "
from wllm.native.frontends.torch.pi0_rtx   import Pi0TorchFrontendRtx
from wllm.native.frontends.jax.pi0_rtx     import Pi0JaxFrontendRtx
from wllm.native.frontends.torch.pi05_rtx  import Pi05TorchFrontendRtx
from wllm.native.frontends.jax.pi05_rtx    import Pi05JaxFrontendRtx
from wllm.native.frontends.torch.groot_rtx import GrootTorchFrontendRtx
...
same: load the model via `wllm_native.load_model(...)`, run `predict()`
...
`wllm_native.models.groot.embodiments`). Requires the HuggingFace
...
### `ImportError: cannot import name 'wllm_native_fa2' from 'wllm_native'`
You skipped the FA2 `.so` install step. The main kernel build
produces TWO `.so` files on RTX targets — copy both:
```bash
cp build/wllm_native_kernels*.so wllm_native/
cp build/wllm_native_fa2*.so     wllm_native/
```
If `build/wllm_native_fa2*.so` does not exist, check that CMake
...
```bash
rm -f wllm_native/wllm_native_kernels*.so wllm_native/wllm_native_fa2*.so \
      build/wllm_native_kernels*.so     build/wllm_native_fa2*.so
cd build && rm CMakeCache.txt && cmake .. && make -j && cd ..
cp build/wllm_native_kernels.*.so build/wllm_native_fa2.*.so wllm_native/
```
...
# 1. Build (produces wllm_native_kernels.so + wllm_native_fa2.so on RTX)
cd build && cmake .. && make -j$(nproc) && cd ..
cp build/wllm_native_kernels.cpython-312-*.so wllm_native/
cp build/wllm_native_fa2.cpython-312-*.so     wllm_native/
