...
wLLM RTX backend inside FlashVLA. It covers the current E2E inference
...
wLLM does not run Qwen/T5 preprocessing inside the hot path. Motus
...
Motus FP4/VAE kernels are built into `wllm_native.wllm_kernels` by CMake.
...
## 3. Build wLLM kernels

```bash
cd "${FLASHVLA_ROOT}"
export PYTHONPATH="${FLASHVLA_ROOT}"
export WLLM_MOTUS_ROOT="${MOTUS_ROOT}"
...
mkdir -p build
cd build
cmake .. -DGPU_ARCH=120 -DWLLM_ENABLE_MOTUS=ON
make -j"$(nproc)"
cd ..
```
...
print("wllm_native ok", wllm_native.__version__)
print("kernels ok", hasattr(wllm_native_kernels, "GemmRunner"))
PY
```
...
Use this contract when comparing wLLM numbers with algorithm baselines:
...
- Qwen/T5 preprocessing is outside the wLLM hot path. If a baseline
...
export PYTHONPATH="${FLASHVLA_ROOT}"
export WLLM_MOTUS_ROOT="${MOTUS_ROOT}"
export WLLM_MOTUS_FP4_PROFILE=fast
export PYTORCH_ALLOC_CONF=expandable_segments:True
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
...
```bash
export WLLM_MOTUS_USE_EASYCACHE=1     # auto-pick schedule from time-embedding distance
export WLLM_MOTUS_USE_TAYLORSEER=1    # order-0/1 final-velocity Taylor forecast
export WLLM_MOTUS_USE_MIXCACHE=1      # hybrid per-skip-step order-0/1 by extrap coeff
```
...
```bash
export WLLM_MOTUS_FP4_PROFILE=off
python examples/motus_quickstart.py \
  --checkpoint "${MOTUS_CHECKPOINT}" \
  --motus-root "${MOTUS_ROOT}" \
  --wan-path "${MOTUS_WAN_PATH}" \
...
os.environ["WLLM_MOTUS_ROOT"] = str(motus_root)
os.environ["WLLM_MOTUS_FP4_PROFILE"] = "fast"
# Optional: turn TeaCache on for the ~100 ms operating point
# os.environ["WLLM_MOTUS_USE_TEACACHE"] = "1"
...
```bash
WLLM_MOTUS_FP4_PROFILE=fast          # validated Stage3 fast profile (default)
WLLM_MOTUS_FP4_PROFILE=off          # explicit FP8 trajectory baseline
WLLM_MOTUS_FP4_PROFILE=fast-cache   # latency-oriented FP4/NVFP4 without tiny-FP8 dispatch
WLLM_MOTUS_FP4_PROFILE=on           # explicit FP4/NVFP4 experiment
```
...
```bash
WLLM_MOTUS_FFN_MULTI_STREAM=0
```

The TeaCache step cache is orthogonal to the profile switch:

```bash
WLLM_MOTUS_USE_TEACACHE=1           # turn on TeaCache (default off)
WLLM_MOTUS_TEACACHE_SKIP_STEPS=2,3,4,5,6,7,8   # default schedule for 10 steps
```

Avoid mixing the profile switch with low-level kernel flags such as
`WLLM_MOTUS_USE_NVFP4_FFN_VIDEO` in the same run. Low-level flags
...
| Symptom | Fix |
|---|---|
| `No module named wllm_native_kernels` | Re-run the build and copy `wllm_native*.so` into `wllm_native/`. |
| `ModuleNotFoundError` for Motus/Wan modules | Set `WLLM_MOTUS_ROOT` or pass `--motus-root`. |
...
| `--fp4-profile on` does not enable VAE FP4 | Rebuild with `cmake -B build -S . -DGPU_ARCH=120` and confirm the Motus VAE FP4 symbols are present in `wllm_native_kernels`. |
...
| TeaCache wall not dropping | Confirm `WLLM_MOTUS_USE_TEACACHE=1` is set **before** `python` starts; the schedule is baked at install time and cannot be flipped per replay. |
