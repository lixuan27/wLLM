...
`wllm_native.runtime.rtc_temporal_fusion` provides an optional host-side runtime
...
The adapter boundary deliberately matches `wllm_native.runtime.rtc`: it receives
...
export WLLM_CUDART_PATH="$CUDA_HOME/lib64/libcudart.so.12"
PYTHONPATH=$PWD \
  python -m pytest tests/test_rtc_temporal_fusion_pi05.py -q -s
```

This command imports wLLM from the current checkout only. Build or install
