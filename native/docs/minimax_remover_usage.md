# MiniMax-Remover — wLLM Inference Pipeline

MiniMax-Remover video inpainting (subtitle / object removal) with wLLM
kernelized inference on Blackwell SM120. Two precision paths ship under
`wllm_native.models.minimax_remover`:
...
Both reuse the **generic** wLLM kernels for the transformer denoise path
...
`wllm_native_minimax_remover` module (opt-in build, see [Build](#build)):
...
cmake -S . -B build -DGPU_ARCH=120 -DCMAKE_BUILD_TYPE=Release \
      -DWLLM_ENABLE_MINIMAX_REMOVER=ON
cmake --build build -j --target wllm_native_kernels wllm_native_minimax_remover
...
`wllm_native_minimax_remover`, following the same opt-in pattern as
`wllm_native_omnivoice`. They are **not** compiled into the default
`wllm_native_kernels` target — enable explicitly:

```bash
cmake -DWLLM_ENABLE_MINIMAX_REMOVER=ON -DGPU_ARCH=120 ...
cmake --build build -j --target wllm_native_minimax_remover
```

| Kernel | Module | Replaces | Effect |
|--------|--------|----------|--------|
| `fp16_rms_norm_ncdhw` | `wllm_native_minimax_remover` | `WanRMS_norm.forward` (4 full-tensor fp32 passes) in attention blocks | ~6x per-call; fp16 in/out, fp32 stats, **no dtype cast** |
| `fp16_rms_silu_ncdhw` | `wllm_native_minimax_remover` | `WanRMS_norm` + `F.silu` two-pass in every `WanResidualBlock` | fused single-pass; eliminates intermediate tensor R/W |
| `fp16_rms_norm_ndhwc` | `wllm_native_minimax_remover` | channels-last (NDHWC) variant of the above for attention-block norm | keeps pipeline in CL → eliminates nchw↔nhwc conversion |
| `fp16_rms_silu_ndhwc` | `wllm_native_minimax_remover` | channels-last fused norm+silu for residual blocks | CL-native, contiguous C reads → faster than NCDHW variant |
| `fp16_rms_silu_amax_ndhwc` | `wllm_native_minimax_remover` | channels-last fused norm+silu+**amax** | fuses amax into the norm+silu pass; saves 1 full read of the activation tensor per conv layer |
| `fp16_rms_silu_quant_fp8_ndhwc` | `wllm_native_minimax_remover` | channels-last fused norm+silu→**FP8 e4m3** quantize | eliminates the fp16 intermediate between norm and conv (reads pre-computed amax) |
| `fp16_rms_silu_amax_quant_fp8_ndhwc` | `wllm_native_minimax_remover` | 2-pass norm+silu+amax+quant→FP8 | combined launcher: pass 1 computes amax (no write), pass 2 quantizes; produces ONLY fp8 + scale |
| `fp8_conv3d_mm_ndhwc_fp16out` | `wllm_native_minimax_remover` | cuDNN fp16 conv3d for applicable 3×3×3 causal convs | FP8 e4m3 implicit-GEMM (no im2col materialization); per-channel weight dequant; fp16 in/out |
| `amax_fp16` + `quantize_fp16_fp8_with_amax` | `wllm_native_minimax_remover` | PyTorch multi-pass `abs().max()` + scale + cast for activation quantization | fused 2-pass device-side amax + quantize; no host sync; shared scale across cache+new frames |
| `quantize_fp16_fp8_with_amax_dual` | `wllm_native_minimax_remover` | two separate `quantize_fp16_fp8_with_amax` calls (cache + new) | single kernel launch quantizes two buffers with shared amax; saves 1 launch per conv layer |
| `bias_gelu_quant_fp16_fp8` | `wllm_native_minimax_remover` | transformer FFN `add_bias_fp16` + `gelu_inplace_fp16` + `quantize_fp8_static_fp16` (3 kernels) | fused single-pass: fp16 GEMM-out + bias → tanh-gelu → fp8 e4m3; output is the pre-quantised input of the next FP8 Linear (skips its activation quantise); eliminates 3 full-tensor fp16 round-trips per FFN block |
| `bias_quant_fp16_fp8` | `wllm_native_minimax_remover` | `add_bias_fp16` + `quantize_fp8_static_fp16` (identity activation variant) | fused bias + quant for Linear→Linear chains with no activation |
| `fp16_bias_gate_residual_bcast` | `wllm_native_minimax_remover` | transformer O-proj / FFN-down `add_bias_fp16` + `gate_mul_residual_bcast` (2 kernels) | fused single-pass fp16x8 (uint4) kernel: `residual[m,d] += (out[m,d] + bias[d]) * gate[d]`; eliminates one full [S,D] fp16 read-modify-write per call (~720 slots / denoise) |
| `fp16_add_bias_vec8` | `wllm_native_minimax_remover` | scalar `add_bias_fp16` (decoder_fused) | vectorised fp16x8 (uint4) in-place bias add; used for Q/K/V and any bias-only slot; ~8× fewer memory transactions |
| `fp16_ada_layernorm_quant_fp8` | `wllm_native_minimax_remover` | Triton `ada_layernorm_fp16_io` + `quantize_fp8_static_fp16` + 3× per-Linear activation quant (Q/K/V) | fused single-pass fp32-stat LayerNorm + adaLN modulation + per-tensor FP8 quantise; feeds a shared-scale FP8 tensor into Q/K/V (one quantise for three Linears, three descales) |
| `fp16_rmsnorm_rope_bshd` | `wllm_native_minimax_remover` | Triton `rms_norm_fp32stat` + `rope_apply_bshd` (2 kernels, 2 full R/W of Q or K) | fused per-token RMSNorm (fp32 stats + fp16 affine) + interleaved RoPE on native [B,S,H,Dd] fp16 layout; one full R/W per tensor; skips the intermediate fp16 write of the normalised Q/K |
...
| `nvfp4_conv3d_ndhwc_fp16out` | `wllm_native_minimax_remover` | FP8 conv3d for eligible 3×3×3 causal convs (Ci % 64 == 0) | **Purpose-built NVFP4 W4A4** implicit-GEMM conv3d using `mma.sync.kind::mxf4nvf4` (e2m1×e2m1, UE4M3 block scales). fp16 NDHWC output (eliminates bf16→fp16 + NCDHW→NDHWC conversions). 2× MMA throughput vs FP8. ~16% VAE speedup. |
| `fp16_quant_nvfp4_ndhwc` | `wllm_native_minimax_remover` | PyTorch multi-pass fp16→FP8 quant for VAE activations | Fused fp16 NCDHW → NVFP4 packed + UE4M3 block-scale (NDHWC output). Single-pass quantization with per-16-element block scales. |
| `fp16_quant_nvfp4_cl_ndhwc` | `wllm_native_minimax_remover` | same, for channels-last 3D input | Channels-last variant: reads NDHWC physical layout directly (channel is innermost → coalesced). Eliminates `contiguous()` copy. |
| `fp16_rms_silu_quant_nvfp4_cl_ndhwc` | `wllm_native_minimax_remover` | separate RMS+SiLU + fp16→FP4 quant (3 kernels) | Fused RMS_norm + SiLU + NVFP4 quantization in one kernel (channels-last input). fp32 statistics, fp32 SiLU (preserves fp16 mantissa precision). |
...
3. The wLLM norm kernels are swapped to NDHWC variants
...
An env-var toggle `WLLM_DISABLE_BIAS_GATE=1` disables this fusion
for A/B verification without a rebuild.

Importing `wllm_native.models.minimax_remover` always succeeds — it needs
...
`load_nvfp4_kernels()` / `load_fp8_kernels()` (`wllm_native/models/minimax_remover/_utils.py`),
...
The attention backend (`WLLM_ATTN_MODE`) optionally pulls in
`sageattention` (Sage); `fa2` uses the vendored `wllm_native_fa2.so` and is the
...
`wllm_native/models/minimax_remover/_fp8_pipeline.py`. Uses static calibration:
...
`wllm_native/models/minimax_remover/pipeline.py`. It wraps a loaded diffusers
...
- the N-step flow-matching denoise loop replaced by a manual, graph-
  ...
  projection skips re-quantisation. With `WLLM_MANUAL_GRAPH=1` the
...
wLLM fp16 fused kernels, all `WanCausalConv3d` weights are converted
...
python3 examples/minimax_remover_quickstart.py ... --no-wllm   # fp16 reference
```
...
against the `--no-wllm` fp16 reference output on the same input.

### End-to-end, full-frame (single segment, all frames at once)

All rows compare against the non-wLLM `--no-wllm` fp16 reference on the
...
|--------------------------|-------|-----------|---------------------|-------------------------------|-------------|
| tennis (70 frames, 432x240) | fp16 reference (`--no-wllm --no-vae-opt`) | 17.33 s | 1.0x | — | — |
| tennis (70 frames, 432x240) | wLLM FP8 + VAE opt + CL (`--no-fp8-conv`) | 10.01 s | 1.73x | 40.8 / 37.0 dB | 0.99981 |
| tennis (70 frames, 432x240) | wLLM FP8 + VAE opt + CL + FP8 conv3d | 8.58 s | 2.02x | 39.9 / 36.4 dB | 0.99981 |
| tennis (70 frames, 432x240) | wLLM FP8 + VAE opt + CL + FP8 conv3d + fused FFN epilogue | 7.56 s | 2.29x | 40.0 / 36.4 dB | 0.99981 |
| tennis (70 frames, 432x240) | wLLM FP8 + VAE opt + CL + FP8 conv3d + fused FFN epilogue + fused bias-gate residual | 7.57 s | 2.30x | 40.0 / 36.2 dB | 0.99981 |
...
| tennis (70 frames, 432x240) | wLLM NVFP4 transformer (`--use-fp4`) | 9.52 s | 1.82x | 7.0 / 6.2 dB | 0.00000 (broken — transformer FP4 error accumulates over 12 denoise steps) |
| bmx-trees (80 frames, 432x240) | fp16 reference (`--no-wllm`) | 19.76 s | 1.0x | — | — |
| bmx-trees (80 frames, 432x240) | wLLM FP8 (default) | 13.24 s | **1.49x** | 35.1 / 32.0 dB | 0.99912 |
| bmx-trees (80 frames, 432x240) | wLLM NVFP4 transformer (`--use-fp4`) | 10.72 s | 1.84x | 7.3 / 7.0 dB | 0.00000 (broken — transformer FP4 error accumulates over 12 denoise steps) |

> Tennis numbers are from a same-session serial A/B (`--no-wllm
> --no-vae-opt` vs default wLLM FP8 stack) measured on RTX 5060 Ti,
> CUDA 13.0, cuDNN 9.2, PyTorch 2.12. Run one process at a time —
> parallel invocations contend on the GPU and inflate wall time.
> Peak VRAM: fp16 ref 3.67 GB, wLLM default 2.57 GB.
> Steady-state (2nd call, post-calibration): default wLLM ~5.73 s
...
- **The current default** (all of the below + NVFP4 VAE + #1/#2/#3 fused
  ...
  `WLLM_NVFP4_FUSED_NORMQUANT=0 WLLM_FP8_FUSED_NORMQUANT=0` for the
...
- **Fused norm+quant + bias-into-rmsnorm (#1/#2/#3, default ON)**: three
  ...
  - **#1 NVFP4 fused norm+silu+NVFP4-quant + FP4 cache reuse**
    (`fp16_rms_silu_quant_nvfp4_cl_ndhwc`, env `WLLM_NVFP4_FUSED_NORMQUANT=1`):
  ...
  - **#2 FP8 fused norm+silu+running-amax+FP8-quant**
    ...
    `WLLM_FP8_FUSED_NORMQUANT=1`): same idea for the FP8-conv residual
...
switch to `WLLM_ATTN_MODE=sage_fp16` for cosine 0.9999 at a small
...
| `--no-wllm` | off | Run the reference diffusers fp16 path (no wLLM). |

## Environment variables

| Variable | Default | Effect |
|----------|---------|--------|
| `WLLM_ATTN_MODE` | `sage_fp8` | attention backend (`sage_fp8`/`sage_fp16`/`sage`/`sage_triton`/`triton_fp8`/`triton_fp16`/`fa2`) |
| `WLLM_FP4_GEMM` | `pingpong` | GEMM kernel variant (`pingpong`/`plain`/`widen`) (NVFP4 path) |
| `WLLM_FUSED_BLOCK` | `1` | fused QKV-quant-once + fused FFN-up GEMM+bias+gelu block (`0` = per-projection re-quant debug path) (NVFP4 path) |
| `WLLM_MANUAL_GRAPH` | `0` | capture the whole denoise loop as one CUDA Graph (NVFP4 path) |
| `WLLM_NUM_STEPS` | unset | override the denoise step count (default 12) |
| `WLLM_FP8_TARGET` | `all` | FP8 Linear scope (`all` / `ffn_only`) (FP8 path) |
| `WLLM_NORM_MODE` | `triton` | per-block LayerNorm kernel: `triton` (fp32-stat Triton, bit-exact) / `fp16` (wLLM `ada_layer_norm_fp16`, lower precision, debug only) |
| `WLLM_GELU_MODE` | `inplace` | FFN GELU kernel: `inplace` (wLLM fused `gelu_inplace*`) / `torch` (original `F.gelu`, debug only) |
| `WLLM_NVFP4_VAE` | `1` | Enable NVFP4 VAE conv3d (default ON). Set to `0` to disable. |
| `WLLM_NVFP4_VAE_DECODE_ONLY` | `0` | `1` = apply NVFP4 only to decoder (encoder stays FP8); `0` = both encode+decode. |
| `WLLM_NVFP4_VAE_MIN_CI` | `192` | Minimum Ci for NVFP4 eligibility. 192 covers WanVAE Ci=192/384; 384 is stricter (better PSNR, fewer layers). |
| `WLLM_NVFP4_NO_CACHE` | `0` | `1` = disable FP4 cache (re-quantize cache per call); `0` = use rolling 2-frame FP4 cache (default, faster). |
| `WLLM_NVFP4_FUSED_NORMQUANT` | `1` | #1: `1` (default) = patch `WanResidualBlock.forward` for fully-NVFP4 blocks so the sister norm emits FP4 directly (fused norm+silu+NVFP4-quant) and the conv reuses the rolling FP4 cache. `0` = separate norm→fp16→quant path (Direction-2). |
| `WLLM_FP8_FUSED_NORMQUANT` | `1` | #2: `1` (default) = same fused norm+silu+running-amax+FP8-quant for the FP8-conv residual blocks (`fp16_rms_silu_amax_quant_fp8_ndhwc_nozero`). `0` = separate norm+amax then quantize_dual. |
| `WLLM_FP8_EAGER_MANUAL` | `1` | `1` (default) = steady-state denoise runs the eager manual loop (avoids per-step `torch.cat` + scheduler CPU sync). `0` = diffusers `__call__`. |
| `WLLM_FP8_GRAPH` | `0` | `1` = capture the whole denoise loop as one CUDA Graph (FP8 path; loses ~1.1 s due to forcing a slower graph-safe attention backend — not recommended). |
...
```python
from wllm.native.models.minimax_remover import MiniMaxRemoverPipelineFP8

# `pipe` is a loaded diffusers Minimax_Remover_Pipeline (transformer + vae +
# scheduler). The wLLM pipeline consumes it in place. The first call
...
```python
from wllm.native.models.minimax_remover import MiniMaxRemoverPipelineFP8
from wllm.native.models.minimax_remover._vae_opt import install_vae_optimizations
from wllm.native.models.minimax_remover._vae_nvfp4 import install_vae_nvfp4
...
```python
from wllm.native.models.minimax_remover import MiniMaxRemoverPipeline
...
diffusers `pipe`). This wLLM pipeline module imports no MiniMax-Remover
source.
