# wLLM — Stable Public API

This document enumerates every symbol that is part of wLLM's public
...
## Top-level (`wllm_native`)

```python
import wllm_native

wllm_native.__version__   # str, e.g. "2.2.0"
wllm_native.load_model    # → VLAModel
wllm_native.VLAModel      # inference wrapper
```
...
- `config="cosmos3_edge"` is the Thor integration for NVIDIA Cosmos Framework.
  ...
  establishes the end-to-end reference; the fixture-backed wLLM backend
...
## Hardware dispatch (`wllm_native.hardware`)

```python
from wllm.native.hardware import detect_arch, resolve_pipeline_class
```
...
```python
wllm_native.hardware._PIPELINE_MAP: dict[tuple[str, str, str], tuple[str, str]]
```
...
## AttentionBackend protocol (`wllm_native.hardware.backend`)

```python
from wllm.native.hardware.backend import (
...
```python
from wllm.native.hardware.rtx.attn_backend       import RtxFlashAttnBackend
from wllm.native.hardware.rtx.attn_backend_groot import RtxFlashAttnBackendGroot
```
...
## Core utilities (`wllm_native.core`)

```python
from wllm.native.core.cuda_buffer import CudaBuffer
...
wLLM ships two pybind11 Python extension modules:

```python
from wllm.native import wllm_native_kernels   # always present
from wllm.native import wllm_native_fa2       # RTX (SM80/86/89/120) only
```

### `wllm_native.wllm_kernels`
...
`wllm_native_kernels.cpython-<abi>.so`, ~3 MB.
...
Motus beta SM120 kernels are built into `wllm_native_kernels` when CMake is
configured with `-DWLLM_ENABLE_MOTUS=ON` (currently the default).
...
```python
from wllm.native.runtime.rtc import (
    ActionChunkAdapter,
...
from wllm.native.runtime import (
...
from wllm.native.runtime.vlash import (
    AsyncVLAShRunner,
    VLAShChunkResult,
    VLAShConfig,
    VLAShStats,
...
wllm_native_fa2.fwd_bf16(...)            # same signature, bfloat16 dtype
```
...
Thor uses `wllm_native_kernels.attention_qkv_fp16` (cuBLAS-decomposed)
because FA2's Ampere tile shapes aren't tuned for Thor's unified
LPDDR memory model. Code importing `wllm_native_fa2` must therefore
...
**Spec file naming**: `wllm_native/frontends/{torch,jax}/_<model>_thor_spec.py`,
...
1. [`docs/adding_new_model.md`](adding_new_model.md) — end-to-end
   walkthrough for wiring a new VLA model into wLLM on Thor
