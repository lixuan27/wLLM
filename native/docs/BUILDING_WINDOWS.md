...
| Python | 3.10 / 3.11 / 3.12 | Same Python must run cmake AND `import wllm_native` |
...
wllm_native_kernels.cp311-win_amd64.pyd]`, you're done.
...
The `.pyd` files are dropped into `wllm_native\` automatically.
...
The default `wllm_native_kernels` build does not produce this DLL on
...
intervention, `import wllm_native` fails with
`ImportError: DLL load failed while importing wllm_native_kernels`,
because `cudart64_*.dll`, `cublas64_*.dll`, etc. live in the CUDA
toolkit `bin\` directory which the secure loader does not consult.

`wllm_native/__init__.py` calls `os.add_dll_directory()` for every
...
```bat
python -c "import wllm_native; print(wllm_native.__version__)"
python -c "from wllm.native import wllm_native_kernels; print('kernels OK')"
```
...
- **JAX FFI module** (`wllm_native_jax_ffi`) is detected only when
