# wLLM Runtime Export (`runtime/`)

The hand-off surface between a wLLM **model runtime** (producer) and a
...
─────────────────────────────────            ──────────────────────────────────
today:                                        e.g. wLLM-Nexus capsule host,
  Python setup/capture                        a robot loop, a server shell
    wllm_native/runtime/export.py
      └─ _wllm_runtime.Builder ──┐
...
```
runtime/
  include/wllm/runtime.h   the ABI (structs + builder). Consumers need ONLY
                              this header + exec.h — the struct is plain data.
  src/runtime_export.cpp      builder + export lifetime (no CUDA, no exec link)
  bindings/runtime_pybind.cpp `_wllm_runtime` (setup/dev bridge)
  tests/                      model-free acceptance
wllm_native/runtime/export.py    Python producer: RuntimeExport / build_export()
```
...
## The model runtime ABI (`wllm/model_runtime.h`)
...
one layer above it in wLLM's native C++ path:
...
and drives snapshot/restore/replay; wLLM model runtimes prepare inputs and
decode outputs.
...
conservative D2H action staging path. The `wllm_cpp_pi05_c` target exposes a
...
A provider with no wLLM graph resources may use the core metadata-only
...
validated in the wLLM-Nexus repo (adopt + snapshot/restore through the
capsule core).
