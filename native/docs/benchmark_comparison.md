...
| wLLM | Hardware | Baseline | Baseline latency | Speedup |
|---|---|---|---:|---:|
...
Local wLLM measurements below were taken on **July 2-3, 2026** on one
...
| `GR00T-N1.6-3B` | RTX 4090 (SM89) | local re-check; 2-view `wLLM.png` duplicated to `image` + `wrist_image`, prompt=`pick up the red block`, `state=zeros(128)`, FP8, `fp8_layout=nk` | - | - | first call excluded | **18.60 ms mean** over 5 steady-state replays | July 3 local steady-state-only re-check; samples: 19.53 / 18.86 / 18.25 / 18.18 / 18.20 ms | `(50, 128)` finite |
...
LLM rows list the baseline and wLLM measurements without speedup.

| Metric | HF SDPA baseline | wLLM |
|---|---:|---:|
...
Higgs Audio v3 TTS-4B on RTX 5090. wLLM numbers are the in-repo
single-stream AR decode path; SGLang is kept as reference data without
speedup.

| Metric | wLLM FP8 | wLLM BF16 | SGLang |
|---|---:|---:|---:|
...
| Mode | wLLM | Unoptimized PyTorch reference | Speedup |
|---|---:|---:|---:|
...
| Path | Hardware | Mode | wLLM | Baseline | Speedup |
|---|---|---|---:|---:|---:|
