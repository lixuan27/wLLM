# wLLM — Plugin Model Template

This document shows how an **external package** integrates a new VLA model
into wLLM without modifying the open-source repo. This is the intended
...
wLLM's dispatch is driven by `_PIPELINE_MAP` in
`wllm_native.hardware.__init__`. A plugin registers itself by:
...
After registration, `wllm_native.load_model(config="mymodel")` works
...
from wllm.native.hardware.backend import AttentionBackendBase, AttentionSpec
...
class MyModelAttentionBackend(AttentionBackendBase):
    ...
    def __init__(self, spec: AttentionSpec):
        ...
        # See wllm_native.hardware.rtx.attn_backend.TorchFlashAttnBackend
...
importable from `wllm_native.hardware.rtx.attn_backend[_groot]`.
...
from wllm.native.hardware import _PIPELINE_MAP
...
After this import, `wllm_native.load_model(config="mymodel")` will find
...
import wllm_native
model = wllm_native.load_model(config="mymodel", checkpoint="...")
...
# wLLM does NOT auto-discover entry_points today, but a future
...
| Import | Purpose |
|---|---|
| `wllm_native.hardware.backend.AttentionBackend` | Protocol to implement |
| `wllm_native.hardware.backend.AttentionSpec` | Build attention site specs |
| `wllm_native.hardware.backend.SiteSpec` | Attention site descriptor |
| `wllm_native.hardware._PIPELINE_MAP` | Registration point |
| `wllm_native.core.cuda_buffer.CudaBuffer` | GPU buffer allocation |
| `wllm_native.core.cuda_graph.CUDAGraph` | CUDA graph capture/replay |
| `wllm_native.core.quant.calibrator.*` | FP8 calibration cache |

Everything else (`wllm_native.models.*`, `wllm_native.frontends.*`,
`wllm_native.hardware.rtx.*` internals) is internal and may change.
...
# Verify dispatch
from wllm.native.hardware import resolve_pipeline_class, detect_arch
