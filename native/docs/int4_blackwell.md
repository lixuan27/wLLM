...
> validation harness. It is not wired into `wllm_native_kernels` or any model
...
kernels here reproduce and productize that finding for wLLM.
...
`huggingface.co/kernels/wllm/int4-blackwell`.
...
- `extern "C"` wrappers (`wllm_int4_sm120_*`) for non-C++ backends,
