...
> **Target audience**: anyone trying to understand what wLLM *is* as a system — not how to add a model (see [`adding_new_model.md`](adding_new_model.md)) or how a specific kernel works (see [`kernel_catalog.md`](kernel_catalog.md)). This doc names the eight infrastructure components that make up wLLM, says where each one lives in the repo, and explains how they compose into a working inference engine.
>
> **TL;DR**: wLLM core is **not** a compiler or graph rewriter. It is a **kernel library plus eight infrastructure components** wired together by a thin per-model pipeline. The repository also ships a `serving/` scenario-host layer on top of the execution contract for OpenAI-compatible LLM/audio endpoints and robot rollout hosts. Core mechanisms stay in `wllm_native/`, `exec/`, and `csrc/`; scenario policy stays in `serving/`.
...
serving/   scenario hosts: sessions, schedulers, protocols, robot loops,
           OpenAI-compatible HTTP/SSE, streaming audio, rollout policy
             │
             ▼
wllm_native/   Python frontends: weights, calibration, CUDA Graph capture,
...
┌──────────────────────────────────────────────────────────────┐
│  1. Public API                                               │
│     wllm_native/api.py — load_model() + VLAModel.predict()     │
└──────────────────────────────────────────────────────────────┘
        │ resolves (config, framework, arch) → frontend class
        ↓
┌──────────────────────────────────────────────────────────────┐
│  2. Hardware Dispatch Map                                    │
│     wllm_native/hardware/__init__.py::_PIPELINE_MAP            │
...
│     wllm_native/frontends/{torch,jax}/<model>_<arch>.py        │
...
└──────────────────────────────────────────────────────────────┘
        ...
        │   │     wllm_native/executors/weight_loader.py       │
        │   │     ModelWeightSpec → WeightLoader.run()       │
        │   └────────────────────────────────────────────────┘
        ├─→ ┌────────────────────────────────────────────────┐
        │   │  5. Attention Backend (protocol)               │
        │   │     wllm_native/hardware/backend.py              │
        │   │     AttentionSpec → AttentionBackend impl      │
        │   └────────────────────────────────────────────────┘
        ├─→ ┌────────────────────────────────────────────────┐
        │   │  6. Calibration Framework                      │
        │   │     wllm_native/core/quant/calibrator.py         │
        │   │     ckpt_hash + Se → ~/.wllm/calibration  │
        │   └────────────────────────────────────────────────┘
        └─→ ┌────────────────────────────────────────────────┐
            │  7. CUDA Graph Capture                         │
            │     wllm_native/core/cuda_graph.py               │
...
│     wllm_native/wllm_native_kernels.so   (~98 entries)           │
│     wllm_native/wllm_native_fp4.so       (~23 entries, SM120)    │
│     wllm_native/wllm_native_fa2.so       (vendored FA2, RTX)     │
│     wllm_native/libfmha_fp16_strided.so (Thor / SM100+)        │
│     csrc/ — source                                           │
└──────────────────────────────────────────────────────────────┘
```
...
### 3.1 Public API (`wllm_native/api.py`)

The single entry point. Two functions:

```python
model = wllm_native.load_model(checkpoint, config="pi05", framework="torch")
...
### 3.2 Hardware Dispatch Map (`wllm_native/hardware/__init__.py`)
...
_PIPELINE_MAP: dict[tuple[str, str, str], tuple[str, str]] = {
    ("pi05",   "torch", "thor"):       ("wllm_native.frontends.torch.pi05_thor",       "Pi05TorchFrontendThor"),
    ("pi05",   "torch", "rtx_sm120"):  ("wllm_native.frontends.torch.pi05",            "Pi05TorchFrontendRtx"),
    ("pi05",   "jax",   "thor"):       ("wllm_native.frontends.jax.pi05_thor",         "Pi05JaxFrontendThor"),
    ("groot",  "torch", "thor"):       ("wllm_native.frontends.torch.groot_thor",      "GrootTorchFrontendThor"),
...
### 3.3 Frontend (`wllm_native/frontends/{torch,jax}/<model>_<arch>.py`)
...
### 3.4 Weight Loading (`wllm_native/executors/weight_loader.py`)
...
### 3.5 Attention Backend (`wllm_native/hardware/backend.py`)
...
### 3.6 Calibration Framework (`wllm_native/core/quant/calibrator.py`)
...
- Cache location (`~/.wllm/calibration/`) — same path across models, configs, frameworks.
...
### 3.7 CUDA Graph Capture (`wllm_native/core/cuda_graph.py`)

A small helper that wraps `torch.cuda.CUDAGraph` (and a JAX equivalent) with two wLLM-specific extensions:
...
### 3.8 Kernel Library (`wllm_native/*.so`, source under `csrc/`)
...
Three modules, all loaded by `import wllm_native`:

| Module | Always built | Contents |
|---|---|---|
| `wllm_native_kernels.so` | yes | ~98 pybind entries: norm / activation / fusion / quant / GEMM / attention / RoPE / utils |
| `wllm_native_fp4.so` | SM100+ only | ~23 entries: NVFP4 weight prep + SM120 block-scaled GEMM |
| `wllm_native_fa2.so` | RTX only | 2 entries: FA2 fp16 / bf16 forward |
...
3. **Batching captured into the graph** — small batches (CFG, multi-policy, multi-frame) are supported within a captured graph. Cross-request continuous batching across users is the workload vLLM / SGLang are shaped for; wLLM can be deployed as one process per captured graph today, and a serving layer that fans out to wLLM workers is an explicit extension path. See [`inference_engine_differences.md`](inference_engine_differences.md) for the workload framing.
...
|---|---|---|
| Public API | `wllm_native/api.py` | 200 |
| Hardware dispatch | `wllm_native/hardware/__init__.py` | 100 |
| Frontends | `wllm_native/frontends/{torch,jax}/*.py` | 800–1500 each |
| Weight Loader | `wllm_native/executors/weight_loader.py` | 330 |
| Weight sources | `wllm_native/executors/{torch,jax}_weights.py` | 400 each |
| Attention protocol | `wllm_native/hardware/backend.py` | 405 |
| Attention backends | `wllm_native/hardware/{thor,rtx}/attn_backend*.py` | 300–600 each |
| Calibration cache | `wllm_native/core/quant/calibrator.py` | 167 |
| Calibration runner | per-frontend `_calibrate()` | 100–200 |
| CUDA Graph helper | `wllm_native/core/cuda_graph.py` | 150 |
| Kernel library | `csrc/`, built into `wllm_native/*.so` | (see catalog) |
