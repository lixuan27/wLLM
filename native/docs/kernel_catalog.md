...
> **Audience**: you are evaluating whether wLLM can accelerate
> *your* model (not necessarily a VLA). This doc lists every CUDA
> kernel shipped by wLLM and the fusion patterns they enable,
> so you can judge re-use against your model's compute graph
> before adopting the framework.
>
> **TL;DR**: wLLM exports pybind entries across three modules:
> **`wllm_native_kernels`** (~98 entries; memory-bound ops, fp8
> quant/dequant, cuBLASLt wrappers, Thor FMHA — always built),
> **`wllm_native_fp4`** (~23 entries; NVFP4 weight prep and SM120
> block-scaled GEMM wrappers — built on SM100+/SM120),
> and **`wllm_native_fa2`** (2 entries; vendored Flash-Attention 2
...
```python
from wllm.native import wllm_native_kernels as fvk       # 87 kernels, always built (pybind, PyTorch)
from wllm.native import wllm_native_fa2     as fa2       # 2 entries, RTX only (SM80/86/89/120)
# JAX side (training + inference): consumed via ctypes + jax.ffi
#   <repo>/wllm_native/wllm_native_jax_ffi.so             # 2 XLA FFI handlers, optional
```

`wllm_native_fa2` is skipped on Thor SM110 (Thor uses `fvk.attention_qkv_fp16`
cuBLAS-decomposed attention — hand-tuned for Thor's unified LPDDR).
`wllm_native_jax_ffi.so` is built only when JAX + the XLA FFI header are
importable at configure time; it carries **zero new compute kernels**
— each handler delegates to an existing entry in `wllm_native_kernels`
(see § 11).
...
This is where most of wLLM's latency wins come from. Each kernel
...
`wllm_native_fp4.so` module (SM120 only).

## 6. Attention (10 entries, fvk + fa2 combined)

### Thor / decomposed path (`wllm_native_kernels`)
...
### RTX / Flash-Attention 2 path (`wllm_native_fa2`)
...
| `has_nvfp4()` | SM120 NVFP4 quant + block-scaled GEMM available (separate `wllm_native_fp4.so`) |

## 11. JAX FFI bindings (2 entries, optional `wllm_native_jax_ffi.so`)
...
When evaluating wLLM for a new model, walk this checklist:
...
3. **Do you need FP8 anywhere?**
   ...
   calibrate-once-use-forever machinery in `wllm_native.core.quant`.
...
Shape features that currently have no fused kernels in wLLM:
...
- **Tree-attention / speculative decoding**. Out of scope —
  wLLM targets batch=1, not batch-N speculation.
