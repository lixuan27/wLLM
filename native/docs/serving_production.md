# wLLM Qwen3.6 agent serving — production architecture (seam-locking design)
...
not carry a backend-specific session id. A wLLM session id is therefore only a
...
(`prompt_cache_key`, `cache_salt`), but they should not need wLLM-specific
...
Pin source is an OpenAI-side field (`wllm_pin_prefix`) or a `/v1/sessions`
...
- **Stage 1 — capsule in the server + the correctness it needs (gaps B, C, and the
  ...
  rejected, not OOM), a `restore` `PrefixPlan` action, and the `wllm_pin_prefix`
...
3. **Pin API surface:** **`wllm_pin_prefix` request field** now (no new
