...
def test_qwen3_vl_decode_graph_cache_is_lru_bounded(monkeypatch):
    ...
    from wllm.native.frontends.torch.qwen3_vl_rtx import Qwen3VlTorchFrontendRtx
...
def test_qwen3_vl_prefill_graph_restages_after_clear(monkeypatch):
    ...
    import wllm_native
    monkeypatch.setitem(sys.modules, "wllm_native.wllm_kernels", fake_fvk)
    monkeypatch.setattr(wllm_native, "wllm_native_kernels", fake_fvk, raising=False)
    import torch
    monkeypatch.setattr(torch.cuda, "current_stream",
                        lambda: _FakeStream(), raising=False)
...
def test_qwen3_vl_sm89_text_decode_graph_cache_is_lru_bounded(monkeypatch):
    ...
    from wllm.native.frontends.torch.qwen3_vl_fp8_sm89 import (
...
def test_qwen3_vl_sm89_multimodal_graph_caches_are_lru_bounded(monkeypatch):
    ...
    from wllm.native.frontends.torch.qwen3_vl_fp8_sm89_multimodal import (
...
def test_qwen3_vl_rtx_bf16_graph_caches_are_lru_bounded(monkeypatch):
    ...
    from wllm.native.frontends.torch.qwen3_vl_rtx_bf16 import (
...
def test_qwen3_vl_rtx_bf16_graph_cache_init_surface(monkeypatch):
    ...
    monkeypatch.setenv("WLLM_QWEN3_VL_PREFILL_GRAPH_CACHE_MAX", "17")
    monkeypatch.setenv("WLLM_QWEN3_VL_DECODE_GRAPH_CACHE_MAX", "19")
    ...
    fake_weights = types.ModuleType(
        "wllm_native.frontends.torch._qwen3_vl_bf16_weights")
    ...
    fake_vision = types.ModuleType(
        "wllm_native.frontends.torch._qwen3_vl_vision_rtx")
    fake_vision.Qwen3VlVisionRtx = object
    fake_attn = types.ModuleType(
        "wllm_native.hardware.rtx.attn_backend_qwen3")
    fake_attn.RtxFlashAttnBackendQwen3 = object

    monkeypatch.setitem(sys.modules, "torch", fake_torch)
    monkeypatch.setitem(sys.modules, "transformers", fake_transformers)
    monkeypatch.setitem(
        sys.modules, "wllm_native.frontends.torch._qwen3_vl_bf16_weights",
        fake_weights)
    monkeypatch.setitem(
        sys.modules, "wllm_native.frontends.torch._qwen3_vl_vision_rtx",
        fake_vision)
    monkeypatch.setitem(
        sys.modules, "wllm_native.hardware.rtx.attn_backend_qwen3", fake_attn)
    try:
