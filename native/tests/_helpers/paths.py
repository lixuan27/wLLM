"""Centralised checkpoint-path resolution for wLLM tests.
...
public users. This helper unifies the resolution rule:
    ...
    env var (WLLM_<NAME>) > legacy bare env var (<NAME>) > built-in default
...
def _die(name: str, value: str, *, reason: str) -> None:
    print(
        f"  [ERROR] wLLM test path '{name}' unresolved ({reason}).\n"
        f"          looked at: {value or '<empty>'}\n"
        f"          override:  export WLLM_{name}=/path/to/your/ckpt\n"
