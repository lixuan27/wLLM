...
gates the *serving wiring* (CapsuleStore + wllm_pin_prefix + the "restore"
...
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        ...
        "WLLM_QWEN36_MTP_CKPT_DIR to run Qwen3.6 capsule-serving tests"
    ),
)
...
def _shared_system(fe, approx_tokens: int = 6000) -> str:
    unit = ("wLLM is a latency-first inference runtime. Capsules freeze a "
...
def test_serving_capsule_requires_budget(monkeypatch):
    """With no budget, an explicit wllm_pin_prefix fails fast instead of
