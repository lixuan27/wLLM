"""Smoke tests for MiniMax-Remover wLLM integration.
...
def _stub_kernels(symbols=()):
    """Register an empty wllm_native_kernels stub exposing only ``symbols``."""
    fake_mod = types.ModuleType("wllm_native.wllm_kernels")
    for s in symbols:
        setattr(fake_mod, s, lambda *a, **k: None)
    sys.modules["wllm_native.wllm_kernels"] = fake_mod
    return fake_mod


def _restore_kernels():
    sys.modules.pop("wllm_native.wllm_kernels", None)


# ── 1. Package import always succeeds (no optional deps, no kernels) ──

def test_package_import():
    """Importing the model package must not require wllm_native_kernels."""
...
def test_utils_module_import():
    ...
    from wllm.native.models.minimax_remover import _utils
...
def test_pipeline_reexports_kernel_surface():
    ...
    from wllm.native.models.minimax_remover import pipeline
    from wllm.native.models.minimax_remover import _utils
...
def test_attention_forward_fa2_does_not_import_sageattention(monkeypatch):
    ...
    import wllm_native
    from wllm.native.models.minimax_remover import _attention

    calls = []
    fake_fa2 = types.SimpleNamespace(
        fwd_fp16=lambda *args: calls.append(args))
    monkeypatch.setattr(wllm_native, "wllm_native_fa2", fake_fa2, raising=False)
    monkeypatch.setitem(sys.modules, "wllm_native.wllm_fa2", fake_fa2)
    monkeypatch.setattr(
...
def test_manual_fused_block_uses_shared_attention_forward():
    """The manual fused block must respect WLLM_ATTN_MODE."""
    from pathlib import Path

    root = Path(__file__).resolve().parents[1]
    src = (root / "wllm_native/models/minimax_remover/_manual_denoise.py").read_text()
...
def test_runtime_optional_dependencies_are_lazy_imported():
    ...
    root = Path(__file__).resolve().parents[1]
    for rel in (
        "wllm_native/models/minimax_remover/_fp8_pipeline.py",
        "wllm_native/models/minimax_remover/_fp8_manual_denoise.py",
        "wllm_native/models/minimax_remover/_manual_denoise.py",
    ):
        ...
        assert "from einops" not in "\n".join(
            line for line in src.splitlines()[:80])
    fp8_src = (root / "wllm_native/models/minimax_remover/_fp8_pipeline.py").read_text()
...
def test_minimax_remover_cmake_requires_blackwell_nvfp4():
    ...
    start = cmake.index("if(WLLM_ENABLE_MINIMAX_REMOVER AND NOT ENABLE_NVFP4)")
    end = cmake.index("endif()", start)
    block = cmake[start:end]
    assert "WLLM_ENABLE_MINIMAX_REMOVER requires Blackwell NVFP4" in block
...
def test_nvfp4_pipeline_constructor_validates_kernels(monkeypatch):
    ...
    def _raise_missing():
        raise RuntimeError(
            ...
            "compiled into wllm_native_kernels. Rebuild with the Blackwell NVFP4 "
...
def test_fp8_pipeline_constructor_validates_kernels(monkeypatch):
    ...
    from wllm.native.models.minimax_remover import _fp8_pipeline

    def _raise_missing():
        raise RuntimeError(
            "MiniMax-Remover FP8 requires wllm_native_kernels with the FP8 "
            "symbols (quantize_fp8_static_fp16 / fp8_gemm_descale_fp16 / "
            "add_bias_fp16). Rebuild wllm_native_kernels.")
...
def test_fp8_pipeline_call_does_not_patch_pipe_class(monkeypatch):
    ...
    monkeypatch.setenv("WLLM_FP8_EAGER_MANUAL", "0")
    ...
    def _fake_runtime():
        def install_wllm_fp8(_transformer, verbose=True, target="all"):
            return 0
        ...
        return (install_wllm_fp8, set_calibration, freeze_calibration,
...
def _get_kernels_or_skip():
    try:
        ...
        try:
            import wllm_native_kernels as fvk  # type: ignore
        except ImportError:
            pytest.skip("wllm_native_kernels not built")
    return fvk
...
def test_nvfp4_symbols_present_when_gated():
    ...
    from wllm.native.models.minimax_remover._utils import _REQUIRED_NVFP4_SYMBOLS
...
def test_fp8_symbols_present_when_gated():
    ...
    fvk = _get_kernels_or_skip()
    from wllm.native.models.minimax_remover._utils import _REQUIRED_FP8_SYMBOLS
...
def test_block_symbols_present_in_default_build():
    ...
    default wllm_native_kernels build must expose them regardless of NVFP4 gating.
    """
    fvk = _get_kernels_or_skip()
    from wllm.native.models.minimax_remover._utils import _REQUIRED_BLOCK_SYMBOLS
