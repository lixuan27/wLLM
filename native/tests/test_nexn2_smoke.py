...
def test_registry_resolves_nexn2():
    """wllm_native.load_model can discover the frontend via the pipeline map."""
    from wllm.native.hardware import _PIPELINE_MAP, resolve_pipeline_class
    ...
    assert cls.__module__ == "wllm_native.frontends.torch.nexn2_rtx"
...
def test_load_model_redirects_to_direct_construction():
    ...
    with pytest.raises(NotImplementedError) as ei:
        wllm_native.load_model("/nonexistent", config="nexn2")
    assert "Nexn2TorchFrontendRtx" in str(ei.value)


def test_frontend_imports():
    """The frontend module + class import without a GPU or checkpoint."""
    m = importlib.import_module("wllm_native.frontends.torch.nexn2_rtx")
...
def test_require_kernels_fails_fast_on_missing_symbols():
    ...
    without -DWLLM_ENABLE_QWEN35MOE=ON raises a clear RuntimeError instead
    of crashing mid-forward after loading the 35B checkpoint."""
    from wllm.native.frontends.torch.nexn2_rtx import _require_kernels
    ...
    assert "WLLM_ENABLE_QWEN35MOE" in str(ei.value)
...
def test_static_dims_consistent():
    ...
    fwd = importlib.import_module(
        "wllm_native.frontends.torch._nexn2_rtx_forward")
    assert fwd.HID == 2048
...
def test_attn_backend_shape_metadata():
    ...
    from wllm.native.hardware.rtx.attn_backend_nexn2 import (
...
def _fvk_or_skip():
    try:
        ...
        pytest.skip(f"wllm_native_kernels not importable: {e}")
    return fvk


def test_qwen35moe_kernel_symbols_present():
    """When built with -DWLLM_ENABLE_QWEN35MOE=ON the kernelized forward's
    ...
    if missing:
        pytest.skip(
            ...
            f"-DWLLM_ENABLE_QWEN35MOE=ON); missing {missing}")
    for s in required:
...
def test_fa2_causal_available():
    ...
    try:
        ...
        pytest.skip(f"wllm_native_fa2 not importable: {e}")
