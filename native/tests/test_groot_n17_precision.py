...
Each stage in the wLLM pipeline must match the corresponding fixture
activation with cosine >= 0.999. Tests are gated on (a) the fixture
existing locally and (b) wLLM pipeline producing the activation;
when the wLLM side is not yet wired, the per-stage tests skip
...
def _wllm_runs() -> "wLLMRun | None":
    """Try to construct a wLLM pipeline forward from this fixture's inputs.
...
class wLLMRun:
    """Wraps one wLLM inference invocation against a fixture's inputs.
