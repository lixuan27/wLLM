...
These tests anchor wLLM's serial / batched FP8 CFG implementations
against an independent FP32 PyTorch reference (R_fp32, see
``wllm_native/refs/pi05_cfg_reference.py``) and against the fused
...
# Why noise (and not v_cond / v_uncond)? wLLM's per-step
...
@pytest.mark.skipif(
    ...
    reason="C1 needs CUDA + wllm_native_kernels extension")
...
def test_c2_per_step_noise_vs_ref(fixtures, beta, path):
    ...
    wLLM's FP8 path and the FP32 reference. Both implementations
