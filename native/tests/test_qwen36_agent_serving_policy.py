...
def test_qwen36_agent_fastapi_non_stream_and_stream_endpoints():
    ...
    resp = client.post("/v1/chat/completions", json={
        ...
        "wllm_session_id": "http-session",
    })
    assert resp.status_code == 200
    body = resp.json()
    assert body["choices"][0]["message"]["content"] == "hi"
    assert body["wllm"]["prefix_action"] == "append"

    stream_resp = client.post("/v1/chat/completions", json={
        "messages": [{"role": "user", "content": "def"}],
        "max_tokens": 2,
        "stream": True,
        "wllm_session_id": "http-stream",
    })
...
def test_openai_request_rejects_invalid_wllm_k():
    for value in (0, -1):
        with pytest.raises(ValueError, match="wllm_K must be >= 1"):
            request_from_openai({
                "messages": [{"role": "user", "content": "abc"}],
                "max_tokens": 1,
                "wllm_K": value,
...
def test_openai_request_and_response_include_wllm_cache_metrics():
    engine = FakeAgentEngine()
    svc = AgentService(engine)
    ...
    assert body["wllm"]["session_id"] == "s"
    assert body["wllm"]["new_prefill_tokens"] == 2
...
def test_qwen36_agent_sm120_defaults_disable_exact_position_decode_graphs(monkeypatch):
    keys = (
        "WLLM_QWEN36_DECODE_FASTGEMM",
        "WLLM_QWEN36_VERIFY_WARPSPLIT",
        "WLLM_QWEN36_TQ_VERIFY_GRAPH",
    ...
    assert os.environ["WLLM_QWEN36_DECODE_FASTGEMM"] == "1"
    assert os.environ["WLLM_QWEN36_VERIFY_WARPSPLIT"] == "1"
    assert os.environ["WLLM_QWEN36_TQ_VERIFY_GRAPH"] == "0"
    assert os.environ["WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH"] == "0"
...
def test_qwen36_agent_sm121_defaults_do_not_enable_sm120_fastgemm(monkeypatch):
    ...
    assert "WLLM_QWEN36_DECODE_FASTGEMM" not in os.environ
    assert "WLLM_QWEN36_VERIFY_WARPSPLIT" not in os.environ
    assert os.environ["WLLM_QWEN36_TQ_VERIFY_GRAPH"] == "0"
    assert os.environ["WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH"] == "0"


def test_qwen36_agent_runtime_defaults_respect_overrides(monkeypatch):
    monkeypatch.setenv("WLLM_QWEN36_TQ_VERIFY_GRAPH", "1")
    monkeypatch.setenv("WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH", "1")

    Qwen36FrontendAgentEngine._set_agent_runtime_env_defaults(12)

    assert os.environ["WLLM_QWEN36_TQ_VERIFY_GRAPH"] == "1"
    assert os.environ["WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH"] == "1"
