#!/usr/bin/env python3
"""wLLM — Full precision test: all models, all backends.

Tests current wllm_native_kernels.so against saved reference outputs.
...
Compares:
  Pi0.5: Production vs wLLM Torch vs wLLM JAX (saved outputs in /tmp/v_*.npy)
  Pi0:   wLLM Torch vs PI0Pytorch reference (/tmp/pi0_ref_2view.npz)
  Pi0 JAX: wLLM JAX vs wLLM Torch (raw decoder output, same noise)
  GROOT: wLLM Torch vs PyTorch reference (groot_ref/groot_ref_e2e_full.pt)
...
# before launching the subprocess. Public users export WLLM_PI05_CKPT
...
# Re-run wLLM Torch with matched_noise, compare vs saved prod/jax
# ══════════════════════════════════════════════════════════════
PI05_SCRIPT = '''
import sys, os, time, json, torch, numpy as np
sys.path.insert(0, "ROOTDIR")
from wllm.native.frontends.torch.pi05_thor import Pi05TorchFrontendThor as ThorPipelineTorch
...
m = wllm_native.load_model("<your_pi05_torch_ckpt>",
...
for f in (pathlib.Path.home()/".wllm"/"calibration").glob("70bdf6f4*"):
    f.unlink()
...
from wllm.native.frontends.torch.pi0_thor import Pi0TorchFrontendThor as ThorPipelineTorchPi0
...
from wllm.native.frontends.torch.groot_thor import GrootTorchFrontendThor as ThorPipelineTorchGroot
...
from wllm.native.frontends.jax.pi0_thor import Pi0JaxFrontendThor as ThorPipelineJaxPi0
...
sys.path.insert(0, "ROOTDIR")
from wllm.native.frontends.jax.pi05_thor import Pi05JaxFrontendThor as ThorPipelineJaxPi05
...
def main():
    ...
    print("=" * 60)
    print("wLLM — Full Precision & Latency Test")
    print("=" * 60)
