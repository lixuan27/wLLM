...
def test_qwen3_fast_prefill_paths_are_default_with_escape_hatches():
    src = _read("wllm_native/frontends/torch/qwen3_rtx.py")
    attn = _read("wllm_native/hardware/rtx/attn_backend_qwen3.py")

    assert "WLLM_QWEN3_RESID_FUSE', '1'" in src
    assert "WLLM_QWEN3_SWIGLU_FOLD', '1'" in src
    assert 'WLLM_QWEN3_FP8_FMHA", "1"' in attn
    assert "WLLM_QWEN3_NO_RESID_FUSE" in src
...
def test_dev_probe_bindings_are_explicitly_gated():
    ...
    assert "option(WLLM_ENABLE_SM120_DEV_KERNELS" in cmake
    assert "WLLM_HAVE_SM120_NVFP4_DEV=1" in cmake
    ...
    for symbol in (
        ...
        guard = bindings.rfind("#ifdef WLLM_HAVE_SM120_NVFP4_DEV", 0, idx)
