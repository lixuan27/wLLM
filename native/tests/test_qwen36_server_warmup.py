...
def test_long_server_warmup_modes(monkeypatch):
    monkeypatch.delenv('WLLM_QWEN36_SERVER_LONG_WARMUP', raising=False)
    assert _long_warmup_flags() == (True, False)

    monkeypatch.setenv(
        'WLLM_QWEN36_SERVER_LONG_WARMUP', 'prefill_graphs')
    assert _long_warmup_flags() == (False, True)

    monkeypatch.setenv(
        'WLLM_QWEN36_SERVER_LONG_WARMUP', 'all_graphs')
    assert _long_warmup_flags() == (True, True)

    monkeypatch.setenv('WLLM_QWEN36_SERVER_LONG_WARMUP', 'off')
    assert _long_warmup_flags() == (False, False)


def test_qwen36_prefill_gdn_backend_defaults_to_native_wy(monkeypatch):
    from wllm.native.frontends.torch.qwen36_rtx import (
        _qwen36_tq_prefill_gdn_backend,
    )

    monkeypatch.delenv('WLLM_QWEN36_TQ_PREFILL_GDN_BACKEND',
                       raising=False)
    ...
    monkeypatch.setenv(
        'WLLM_QWEN36_TQ_PREFILL_GDN_BACKEND', 'fla_chunk')
    import pytest
...
def test_long_mtp_tail_auto_policy(monkeypatch):
    ...
    monkeypatch.delenv('WLLM_QWEN36_LONG_MTP_PREFILL_TAIL',
                       raising=False)
    ...
    monkeypatch.setenv('WLLM_QWEN36_LONG_MTP_PREFILL_TAIL', '512')
...
def test_spark_long_k_and_xqa_buckets(monkeypatch):
    ...
    monkeypatch.delenv('WLLM_QWEN36_TQ_SPEC_K', raising=False)
...
def test_long_tq_effective_k_uses_measured_context_buckets(monkeypatch):
    ...
    monkeypatch.setenv('WLLM_QWEN36_TQ_SPEC_K', '4')
...
def test_long_graph_capture_waterline_can_be_disabled(monkeypatch):
    ...
    fe = Qwen36TorchFrontendRtx.__new__(Qwen36TorchFrontendRtx)
    monkeypatch.setenv('WLLM_QWEN36_LONG_GRAPH_MIN_FREE_MB', '0')
...
def test_long_prefill_graph_capture_has_ctx_ceiling(monkeypatch):
    ...
    fe = Qwen36TorchFrontendRtx.__new__(Qwen36TorchFrontendRtx)
    monkeypatch.setenv('WLLM_QWEN36_LONG_GRAPH_MIN_FREE_MB', '0')
    monkeypatch.delenv('WLLM_QWEN36_LONG_PREFILL_GRAPH_MAX_CTX',
                       raising=False)
    ...
    monkeypatch.setenv('WLLM_QWEN36_LONG_PREFILL_GRAPH_MAX_CTX', '0')
