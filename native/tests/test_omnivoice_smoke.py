#!/usr/bin/env python3
"""OmniVoice wLLM kernel smoke test.

Validates all required kernel symbols are present when wLLM is built
with WLLM_ENABLE_OMNIVOICE=ON on SM120 GPUs. Skips cleanly when
...
# Add wLLM repo root to path (repo convention: tests/ run from root)
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))


class TestDefaultBuild:
    """Default build (WLLM_ENABLE_OMNIVOICE=OFF).

    wllm_native package imports work without GPU. The models.omnivoice
    module imports cleanly, _has_cfg_kernel is False.
    """

    def test_omnivoice_module_imports(self):
        from wllm.native.models.omnivoice import (
            wLLMLlm, wLLMLlmBF16,
    ...
    def test_cfg_kernel_disabled_by_default(self):
        from wllm.native.models.omnivoice import pipeline_rtx
        import importlib
...
class TestGatedBuild:
    """OmniVoice build (WLLM_ENABLE_OMNIVOICE=ON, SM120 GPU).

    All kernel symbols must be present across modules:
      wllm_native_kernels  — base FP4/fused kernels
      wllm_native_omnivoice — OmniVoice-specific kernels
      wllm_native_fa2       — FlashAttention2
    """

    _F = None  # wllm_native_kernels
    _O = None  # wllm_native_omnivoice
    _A = None  # wllm_native_fa2
    ...
    def _setup(self):
        if TestGatedBuild._F is None:
            try:
                ...
                pytest.skip("wllm_native_kernels not available (GPU build required)")
    ...
    # ── wllm_native_omnivoice symbols ──

    def test_omnivoice_cfg_logsoftmax(self):
        if TestGatedBuild._O is None:
            pytest.skip("wllm_native_omnivoice not built")
    ...
    def test_check_kernels_passes(self):
        from wllm.native.models.omnivoice import _check_kernels
        import importlib
        from wllm.native.models.omnivoice import pipeline_rtx
        importlib.reload(pipeline_rtx)
        # Reload to pick up the current .so state
        from wllm.native.models.omnivoice import pipeline_rtx as prx
    ...
    def test_fa2_module(self):
        if TestGatedBuild._A is None:
            pytest.skip("wllm_native_fa2 not available")
