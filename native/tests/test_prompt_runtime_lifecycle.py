...
def test_pi05_rtx_caches_recurring_state_prompt_lengths(monkeypatch):
    import torch
    from wllm.native.frontends.torch import pi05_rtx
...
def test_load_model_forwards_state_prompt_mode_to_pi05_thor(
    ...
    import wllm_native
    import wllm.native.hardware as hw
...
def test_groot_thor_rejects_prompt_changes_after_graph_build():
    source = Path("wllm_native/frontends/torch/groot_thor.py").read_text()
...
def test_groot_n17_rejects_second_prompt_runtime():
    source = Path("wllm_native/frontends/torch/groot_n17_thor.py").read_text()
    set_prompt_pos = source.index("    def set_prompt(\n")
    guard_pos = source.index('hasattr(self, "_backbone_features")',
                             set_prompt_pos)
    calibration_pos = source.index("from wllm.native.models.groot_n17",
                                   set_prompt_pos)
