"""Smoke tests for MelBandRoformer wLLM integration.

These tests run in **any** build configuration:
  - default build (WLLM_ENABLE_MELBAND_ROFORMER=OFF): import succeeds,
    pipeline construction raises RuntimeError.
  - gated build (WLLM_ENABLE_MELBAND_ROFORMER=ON): all mbr_* symbols present.
...
def test_package_import():
    ...
    from wllm.native.models.melband_roformer import MelBandRoformerPipeline
    assert MelBandRoformerPipeline is not None


def test_pipeline_module_import():
    """The pipeline module imports cleanly without wllm_native_kernels."""
    from wllm.native.models.melband_roformer import pipeline
...
def test_construction_without_kernels_raises():
    ...
    from wllm.native.models.melband_roformer.pipeline import _load_kernels

    try:
        _load_kernels()
    except RuntimeError as exc:
        msg = str(exc)
        assert "wllm_native_kernels" in msg
        assert ("WLLM_ENABLE_MELBAND_ROFORMER=ON" in msg or
                "compiled .so" in msg)
    except Exception:
        pass  # wllm_native_kernels not importable at all
    else:
...
def test_pipeline_constructor_validates_kernels(monkeypatch):
    ...
    def _raise_missing():
        raise RuntimeError("missing -DWLLM_ENABLE_MELBAND_ROFORMER=ON")

    monkeypatch.setattr(pipeline, "_load_kernels", _raise_missing)
    with pytest.raises(RuntimeError, match="WLLM_ENABLE_MELBAND_ROFORMER=ON"):
...
def test_mbr_symbols_present_when_gated():
    ...
    try:
        ...
        try:
            import wllm_native_kernels as fvk
        except ImportError:
