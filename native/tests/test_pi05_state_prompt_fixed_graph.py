...
model = wllm_native.load_model(ckpt, framework="torch", config="pi05",
...
_CHILD_SWITCH = r"""
import sys, numpy as np, wllm_native
m = wllm_native.load_model(sys.argv[1], framework="torch", config="pi05",
...
import sys, numpy as np, torch, wllm_native
m = wllm_native.load_model(sys.argv[1], framework="torch", config="pi05",
