...
vision + encoder + decoder + diffusion). Under WLLM_PI05_USE_EXEC=1 the
...
Run twice (inside the CUDA container, after building fp16 fa2):
  for v in 0 1; do
    PYTHONPATH=. \
    WLLM_PI05_USE_EXEC=$v PYTORCH_ALLOC_CONF=expandable_segments:True \
...
USE_EXEC = os.environ.get("WLLM_PI05_USE_EXEC", "0")
SEED = 1234
