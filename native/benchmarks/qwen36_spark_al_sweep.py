...
def _set_case_env(case: Case, *, direct: bool) -> int:
    old_k = os.environ.pop("WLLM_QWEN36_TQ_SPEC_K", None)
    if case.k_label != "auto":
        os.environ["WLLM_QWEN36_TQ_SPEC_K"] = case.k_label
    os.environ["WLLM_QWEN36_LONG_MTP_PREFILL_TAIL"] = case.tail
    os.environ["WLLM_QWEN36_LONG_MTP_TAIL_KV_ONLY"] = case.tail_kv_only
    if direct:
        os.environ["WLLM_QWEN36_TQ_VERIFY_GRAPH"] = "0"
        os.environ["WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH"] = "0"
    else:
        os.environ["WLLM_QWEN36_TQ_VERIFY_GRAPH"] = "1"
        os.environ["WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH"] = "1"
...
def main() -> int:
    ...
    parser.add_argument("--use-exec", action="store_true",
                        help="Leave WLLM_QWEN36_USE_EXEC enabled.")
    args = parser.parse_args()

    if args.mtp_dir:
        os.environ["WLLM_QWEN36_MTP_CKPT_DIR"] = args.mtp_dir
    if not args.use_exec:
        os.environ["WLLM_QWEN36_USE_EXEC"] = "0"
    if args.long_route_min is not None:
        os.environ["WLLM_QWEN36_LONG_CTX_ROUTE_MIN_SEQ"] = str(
            args.long_route_min)
    if args.long_kv_cache is not None:
        os.environ["WLLM_QWEN36_LONG_KV_CACHE"] = args.long_kv_cache
    ...
    if frontend_name == "spark":
        from wllm.native.frontends.torch.qwen36_spark import (
