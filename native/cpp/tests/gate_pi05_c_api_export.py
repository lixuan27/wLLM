...
This is intentionally an in-process phase-1 test:
  ...
  2. libwllm_cpp_pi05_c.so adopts that export.
...
def main() -> None:
    ...
    ap.add_argument("--lib", default=str(ROOT / "cpp/build/libwllm_cpp_pi05_c.so"))
    args = ap.parse_args()
    ...
    try:
        ...
        print("\nPASS - C++ Pi05 runtime adopts a real wLLM export")
    finally:
        if rt:
