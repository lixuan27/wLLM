...
int main() {
    ...
    assert(cudaMemcpy(wrt_buffer_dptr(action), action_host.data(), action_bytes,
    ...
    wrt_pi05_runtime* rt = nullptr;
    int rc = wrt_pi05_runtime_create(&exp, &cfg, &rt);
    assert(rc == 0);
    assert(rt);
    ...
    frame.width = 2;
    frame.height = 2;
    frame.pixel_format = WRT_PI05_PIXEL_RGB8;
    rc = wrt_pi05_runtime_prepare_vision(rt, &frame, 1);
    assert(rc == 0);
    std::vector<std::uint16_t> rgb_staged(image_bytes / 2);
    assert(cudaMemcpy(rgb_staged.data(), wrt_buffer_dptr(image), image_bytes,
                      cudaMemcpyDeviceToHost) == cudaSuccess);
    wrt_pi05_vision_frame invalid = frame;
    invalid.pixel_format = 999;
    rc = wrt_pi05_runtime_prepare_vision(rt, &invalid, 1);
    assert(rc == -4);
    assert(std::strstr(wrt_pi05_runtime_last_error(rt), "pixel format"));
    ...
    invalid.pixel_format = WRT_PI05_PIXEL_BGR8;
    rc = wrt_pi05_runtime_prepare_vision(rt, &invalid, 1);
    assert(rc == 0);
    std::vector<std::uint16_t> bgr_staged(image_bytes / 2);
    assert(cudaMemcpy(bgr_staged.data(), wrt_buffer_dptr(image), image_bytes,
                      cudaMemcpyDeviceToHost) == cudaSuccess);
    ...
    rc = wrt_pi05_runtime_prepare_vision(rt, &invalid, 1);
    assert(rc == -4);
    assert(std::strstr(wrt_pi05_runtime_last_error(rt), "stride"));

    float out[3] = {};
    uint64_t n_written = 0;
    rc = wrt_pi05_runtime_read_actions(rt, out, 3, &n_written);
    assert(rc == 0);
    assert(n_written == 3);
