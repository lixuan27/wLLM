...
//
// Prefixed `wllm_int4_sm120_` so the unmangled global symbols do not
...
// header keep the `wllm_native::gemm` namespace.

extern "C" {

int wllm_int4_sm120_w4a4_full_n_bf16out(
    const void* A, const void* B, void* D, int N, int K,
    const void* SFA, const void* SFB, float alpha, void* stream) {
  return wllm_native::gemm::int4_w4a4_mma_sm120_full_n_bf16out(
      A, B, D, N, K, SFA, SFB, alpha,
      reinterpret_cast<cudaStream_t>(stream));
}

int wllm_int4_sm120_quantize_bf16(
    const void* x, void* out_packed, void* out_sf,
    const void* global_scale, int rows, int K, void* stream) {
  return wllm_native::gemm::int4_quantize_bf16_sm120(
      x, out_packed, out_sf, global_scale, rows, K,
      reinterpret_cast<cudaStream_t>(stream));
}

int wllm_int4_sm120_global_scale_bf16(
    const void* x, void* scale_out, long long numel, void* stream) {
  return wllm_native::gemm::int4_global_scale_bf16_sm120(
      x, scale_out, numel, reinterpret_cast<cudaStream_t>(stream));
}

int wllm_int4_sm120_codebook_canary(void* stream) {
  return wllm_native::gemm::int4_w4a4_sm120_codebook_canary(
      reinterpret_cast<cudaStream_t>(stream));
}
