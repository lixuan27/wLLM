...
#define WLLM_BF16_MATMUL_CUBLASLT_CHECK(expr)                         \
    do {                                                                 \
...
static int get_bf16_autotune_algos() {
    const char* env = std::getenv("WLLM_BF16_CUBLASLT_AUTOTUNE_ALGOS");
    if (!env || !*env) return 8;
    return std::clamp(std::atoi(env), 1, 32);
}

static bool get_bf16_autotune_verbose() {
    const char* env = std::getenv("WLLM_BF16_CUBLASLT_AUTOTUNE_VERBOSE");
    return env && std::atoi(env) != 0;
}

static void ensure_bf16_lt() {
    if (g_bf16_lt) return;
    WLLM_BF16_MATMUL_CUBLASLT_CHECK(cublasLtCreate(&g_bf16_lt));
...
static Bf16LtPlan& get_bf16_lt_plan(int M, int N, int K) {
    ...
    WLLM_BF16_MATMUL_CUBLASLT_CHECK(
        cublasLtMatmulDescCreate(&plan.desc, CUBLAS_COMPUTE_32F, CUDA_R_32F));
    WLLM_BF16_MATMUL_CUBLASLT_CHECK(cublasLtMatmulDescSetAttribute(
        plan.desc, CUBLASLT_MATMUL_DESC_TRANSA, &op_n, sizeof(op_n)));
    WLLM_BF16_MATMUL_CUBLASLT_CHECK(cublasLtMatmulDescSetAttribute(
        plan.desc, CUBLASLT_MATMUL_DESC_TRANSB, &op_t, sizeof(op_t)));

    WLLM_BF16_MATMUL_CUBLASLT_CHECK(cublasLtMatrixLayoutCreate(
        &plan.a_desc, CUDA_R_16BF, M, K, K));
    WLLM_BF16_MATMUL_CUBLASLT_CHECK(cublasLtMatrixLayoutSetAttribute(
    ...
    WLLM_BF16_MATMUL_CUBLASLT_CHECK(cublasLtMatmulPreferenceCreate(&pref));
    WLLM_BF16_MATMUL_CUBLASLT_CHECK(cublasLtMatmulPreferenceSetAttribute(
        pref, CUBLASLT_MATMUL_PREF_MAX_WORKSPACE_BYTES,
        &g_bf16_workspace_size, sizeof(g_bf16_workspace_size)));
    cublasLtMatmulHeuristicResult_t heuristic;
    int returned = 0;
    WLLM_BF16_MATMUL_CUBLASLT_CHECK(cublasLtMatmulAlgoGetHeuristic(
...
void bf16_matmul_cublaslt_bf16(
    ...
    const float alpha = 1.0f;
    const float beta = 0.0f;
    WLLM_BF16_MATMUL_CUBLASLT_CHECK(cublasLtMatmul(
        g_bf16_lt, plan.desc, &alpha,
        x, plan.a_desc,
        W, plan.b_desc,
        &beta,
