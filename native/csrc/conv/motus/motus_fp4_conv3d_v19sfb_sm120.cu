...
constexpr int V19SFB_BLOCK_M = 128;
constexpr int V19SFB_BLOCK_N = 128;
constexpr int V19SFB_BLOCK_K_ELEM = 64;
constexpr int V19SFB_N_ATOMS  = V19SFB_BLOCK_N / 8;   // 16
constexpr int V19SFB_N_GROUPS = V19SFB_N_ATOMS / 4;   // 4
constexpr int V19SFB_NUM_WARPS = 8;
constexpr int V19SFB_THREADS = V19SFB_NUM_WARPS * 32;
constexpr int V19SFB_STAGES = 2;
constexpr int V19SFB_SMEM_K_STRIDE = 48;
constexpr int V19SFB_SF_K_PER_ROW = 4;               // 4 SF bytes per row per K-tile (Ci=64)

__device__ __forceinline__
void v19sfb_mma_m16n8k64_e2m1_4x(
...
__device__ __forceinline__
const uint8_t* v19sfb_x_byte_ptr(const uint8_t* cache_x_fp4,
...
__device__ __forceinline__
const uint8_t* v19sfb_w_byte_ptr(const uint8_t* w_fp4,
...
__device__ __forceinline__
const uint8_t* v19sfb_sfa_byte_ptr(const uint8_t* cache_sfa,
...
__device__ __forceinline__
const uint8_t* v19sfb_sfb_byte_ptr(const uint8_t* w_sfb,
...
__device__ __forceinline__
void v19sfb_cp_async_16(uint32_t smem_int_ptr, const uint8_t* src) {
...
__device__ __forceinline__
void v19sfb_cp_async_4(uint32_t smem_int_ptr, const uint8_t* src) {
...
__device__ __forceinline__
uint32_t v19sfb_to_smem_int(const void* p) {
  return static_cast<uint32_t>(__cvta_generic_to_shared(p));
}

__global__ void __launch_bounds__(V19SFB_THREADS, 2)
fp4_conv3d_v19sfb_kernel(
...
{
  __shared__ __align__(16) uint8_t A_smem [V19SFB_STAGES][V19SFB_BLOCK_M * V19SFB_SMEM_K_STRIDE];
  __shared__ __align__(16) uint8_t B_smem [V19SFB_STAGES][V19SFB_BLOCK_N * V19SFB_SMEM_K_STRIDE];
  // SF smem: 4 bytes per row, 128 rows. Pad each row to 4 bytes (same).
  __shared__ __align__(16) uint8_t A_sf_smem[V19SFB_STAGES][V19SFB_BLOCK_M * V19SFB_SF_K_PER_ROW];
  __shared__ __align__(16) uint8_t B_sf_smem[V19SFB_STAGES][V19SFB_BLOCK_N * V19SFB_SF_K_PER_ROW];
  ...
  int m_base = m_idx * V19SFB_BLOCK_M;
  int co_base = n_idx * V19SFB_BLOCK_N;
  if (m_base >= M_total || co_base >= Co) return;

  float dA[V19SFB_N_ATOMS] = {0};
  float dB[V19SFB_N_ATOMS] = {0};
  float dC[V19SFB_N_ATOMS] = {0};
  float dD[V19SFB_N_ATOMS] = {0};

  // ── Issue load: FP4 (cp.async.16) for A+B, plus SF (cp.async.4) for SFA+SFB.
  auto issue_load = [&](int stage, int k_base) {
    // FP4 A
    {
      const uint8_t* src = v19sfb_x_byte_ptr(cache_x, new_x,
                                            m_base + ld_row_a,
                                            k_base + ld_k_off_a,
                                            N, T_cache, T_new, H, W, Ci);
      uint32_t smem_int = v19sfb_to_smem_int(
          &A_smem[stage][ld_row_a * V19SFB_SMEM_K_STRIDE + (ld_k_off_a >> 1)]);
      v19sfb_cp_async_16(smem_int, src);
    }
    // FP4 B
    {
      const uint8_t* src = v19sfb_w_byte_ptr(w, co_base + ld_row_b,
                                            k_base + ld_k_off_b,
                                            Co, Ci);
      uint32_t smem_int = v19sfb_to_smem_int(
          &B_smem[stage][ld_row_b * V19SFB_SMEM_K_STRIDE + (ld_k_off_b >> 1)]);
      v19sfb_cp_async_16(smem_int, src);
    }
    // SF — one (kt_iter) per K-tile. Each thread loads 4 SF bytes for one
    // (M-row or N-col)'s 4 K-blocks at this K-tile's (kt, kr, ks).
    if (t < V19SFB_BLOCK_M) {
      const uint8_t* src = v19sfb_sfa_byte_ptr(cache_sfa, new_sfa,
                                              m_base + t, k_base,
                                              N, T_cache, T_new, H, W, Ci);
      uint32_t smem_int = v19sfb_to_smem_int(&A_sf_smem[stage][t * V19SFB_SF_K_PER_ROW]);
      v19sfb_cp_async_4(smem_int, src);
    } else {
      int n_idx_t = t - V19SFB_BLOCK_M;
      const uint8_t* src = v19sfb_sfb_byte_ptr(w_sfb, co_base + n_idx_t, k_base, Co, Ci);
      uint32_t smem_int = v19sfb_to_smem_int(&B_sf_smem[stage][n_idx_t * V19SFB_SF_K_PER_ROW]);
      v19sfb_cp_async_4(smem_int, src);
    }
  };
  ...
  for (int k_base = 0; k_base < K_total; k_base += V19SFB_BLOCK_K_ELEM) {
    int next_stage = compute_stage ^ 1;
    int k_next = k_base + V19SFB_BLOCK_K_ELEM;
    ...
    uint32_t A0 = *reinterpret_cast<const uint32_t*>(
        &A_smem[compute_stage][rA0 * V19SFB_SMEM_K_STRIDE + kA0]);
    uint32_t A1 = *reinterpret_cast<const uint32_t*>(
        &A_smem[compute_stage][rA1 * V19SFB_SMEM_K_STRIDE + kA0]);
    uint32_t A2 = *reinterpret_cast<const uint32_t*>(
        &A_smem[compute_stage][rA0 * V19SFB_SMEM_K_STRIDE + kA2]);
    uint32_t A3 = *reinterpret_cast<const uint32_t*>(
        &A_smem[compute_stage][rA1 * V19SFB_SMEM_K_STRIDE + kA2]);
    ...
    uint32_t SFA = *reinterpret_cast<const uint32_t*>(
        &A_sf_smem[compute_stage][sfa_m_row * V19SFB_SF_K_PER_ROW]);

    // Per N-group (4 scale_vec::4X calls per warp covering BLOCK_N=128 in 4×32 chunks).
    #pragma unroll
    for (int g = 0; g < V19SFB_N_GROUPS; ++g) {
      int base = g * 4;
      int co0 = (base + 0) * 8 + h;
      int co1 = (base + 1) * 8 + h;
      int co2 = (base + 2) * 8 + h;
      ...
      uint32_t B0 = *reinterpret_cast<const uint32_t*>(
          &B_smem[compute_stage][co0 * V19SFB_SMEM_K_STRIDE + kA0]);
      uint32_t B1 = *reinterpret_cast<const uint32_t*>(
          &B_smem[compute_stage][co0 * V19SFB_SMEM_K_STRIDE + kA2]);
      uint32_t B2 = *reinterpret_cast<const uint32_t*>(
          &B_smem[compute_stage][co1 * V19SFB_SMEM_K_STRIDE + kA0]);
      uint32_t B3 = *reinterpret_cast<const uint32_t*>(
          &B_smem[compute_stage][co1 * V19SFB_SMEM_K_STRIDE + kA2]);
      uint32_t B4 = *reinterpret_cast<const uint32_t*>(
          &B_smem[compute_stage][co2 * V19SFB_SMEM_K_STRIDE + kA0]);
      uint32_t B5 = *reinterpret_cast<const uint32_t*>(
          &B_smem[compute_stage][co2 * V19SFB_SMEM_K_STRIDE + kA2]);
      uint32_t B6 = *reinterpret_cast<const uint32_t*>(
          &B_smem[compute_stage][co3 * V19SFB_SMEM_K_STRIDE + kA0]);
      uint32_t B7 = *reinterpret_cast<const uint32_t*>(
          &B_smem[compute_stage][co3 * V19SFB_SMEM_K_STRIDE + kA2]);
      ...
      uint32_t SFB = *reinterpret_cast<const uint32_t*>(
          &B_sf_smem[compute_stage][sfb_n * V19SFB_SF_K_PER_ROW]);

      v19sfb_mma_m16n8k64_e2m1_4x(
          dA[base+0], dB[base+0],
          dA[base+1], dB[base+1],
          dA[base+2], dB[base+2],
          dA[base+3], dB[base+3],
  ...
  #pragma unroll
  for (int n_atom = 0; n_atom < V19SFB_N_ATOMS; ++n_atom) {
...
{
  if (Ci % V19SFB_BLOCK_K_ELEM != 0 || Co % 8 != 0) {
    std::fprintf(stderr,
        "[fp4_conv3d_v19sfb] Ci%%%d (got %d) or Co%%8 (got %d) bad\n",
        V19SFB_BLOCK_K_ELEM, Ci, Co);
    return -1;
  }
  if (T_cache != 2) {
  ...
  int M = N * T_new * H * W;
  int M_tiles = (M + V19SFB_BLOCK_M - 1) / V19SFB_BLOCK_M;
  int N_tiles = (Co + V19SFB_BLOCK_N - 1) / V19SFB_BLOCK_N;
  int total_tiles = M_tiles * N_tiles;

  dim3 grid(total_tiles);
  dim3 block(V19SFB_THREADS);
  fp4_conv3d_v19sfb_kernel<<<grid, block, 0, stream>>>(
...
// ================================================================
// v2 (Phase 10): NCDHW out + residual add + per-Co outer FP32 scale.
// Body byte-identical to v1 except final epilogue multiply.
// v1 kernel/wrapper unchanged. outer_w_fp32 must be non-null.
// ================================================================
__global__ void __launch_bounds__(V19SFB_THREADS, 2)
fp4_conv3d_v19sfb_kernel_v2(
    const uint8_t* __restrict__ cache_x, const uint8_t* __restrict__ new_x,
    const uint8_t* __restrict__ w,
    const uint8_t* __restrict__ cache_sfa, const uint8_t* __restrict__ new_sfa,
    const uint8_t* __restrict__ w_sfb,
    const float* __restrict__ outer_w_fp32,            // [Co]
    __nv_bfloat16* __restrict__ y,
    const __nv_bfloat16* __restrict__ bias,
    const __nv_bfloat16* __restrict__ residual,
    int N, int T_cache, int T_new, int H, int W, int Ci, int Co,
    ...
    int M_tiles, int N_tiles)
{
  ...
  // ── v2 Epilogue: NCDHW out + residual + per-Co outer FP32 ──
  const int warp_M_off = warp_id * 16;
  auto ncdhw_idx = [&](int row, int co) -> long long {
    int spatial = T_new * H * W;
    int b_idx = row / spatial;
  ...
  for (int n_atom = 0; n_atom < V19SFB_N_ATOMS; ++n_atom) {
    ...
    if (co_pair + 1 < Co) {
      if (row0 < M_total) {
        long long idx0 = ncdhw_idx(row0, co_pair);
        long long idx1 = ncdhw_idx(row0, co_pair + 1);
        y[idx0] = add_res(idx0, dA[n_atom] * ow0 + b0);
        y[idx1] = add_res(idx1, dB[n_atom] * ow1 + b1);
      }
      if (row1 < M_total) {
        long long idx0 = ncdhw_idx(row1, co_pair);
        long long idx1 = ncdhw_idx(row1, co_pair + 1);
        y[idx0] = add_res(idx0, dC[n_atom] * ow0 + b0);
        y[idx1] = add_res(idx1, dD[n_atom] * ow1 + b1);
      }
    } else {
      auto store = [&](int row, int co, float v, float ow, float bv) {
        if (row < M_total && co < Co) {
          long long idx = ncdhw_idx(row, co);
          y[idx] = add_res(idx, v * ow + bv);
        }
      };
    ...
    float alpha, cudaStream_t stream)
{
  if (Ci % V19SFB_BLOCK_K_ELEM != 0 || Co % 8 != 0) {
    std::fprintf(stderr,
        "[fp4_conv3d_v19sfb_v2] Ci%%%d (got %d) or Co%%8 (got %d) bad\n",
        V19SFB_BLOCK_K_ELEM, Ci, Co);
    return -1;
  }
  if (T_cache != 2) {
    std::fprintf(stderr, "[fp4_conv3d_v19sfb_v2] T_cache must be 2 (got %d)\n", T_cache);
    return -3;
  }
  if (outer_w_fp32 == nullptr) {
    std::fprintf(stderr, "[fp4_conv3d_v19sfb_v2] outer_w_fp32 must be non-null\n");
    return -4;
  }
  int M = N * T_new * H * W;
  int M_tiles = (M + V19SFB_BLOCK_M - 1) / V19SFB_BLOCK_M;
  ...
  dim3 grid(total_tiles);
  dim3 block(V19SFB_THREADS);
  fp4_conv3d_v19sfb_kernel_v2<<<grid, block, 0, stream>>>(
      reinterpret_cast<const uint8_t*>(cache_x_fp4),
      reinterpret_cast<const uint8_t*>(new_x_fp4),
      reinterpret_cast<const uint8_t*>(w_fp4),
      reinterpret_cast<const uint8_t*>(cache_sfa),
  ...
  cudaError_t e = cudaGetLastError();
  if (e != cudaSuccess) {
    std::fprintf(stderr, "[fp4_conv3d_v19sfb_v2] launch err: %s\n",
                 cudaGetErrorString(e));
    return -2;
  }
  return 0;
