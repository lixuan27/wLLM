// ================================================================
// wLLM — Attention dispatch implementation
//
...
int load_fmha_library(const char* path) {
    ...
    if (!g_fmha_lib) {
        fprintf(stderr, "[wLLM] Failed to load FMHA library: %s\n  %s\n",
                path, dyn_error());
        return -1;
    }
    g_fmha_fn = (fmha_fn_t)dyn_sym(g_fmha_lib, "fmha_fp16_attn");
    if (!g_fmha_fn) {
        fprintf(stderr, "[wLLM] Symbol 'fmha_fp16_attn' not found in %s\n", path);
        return -1;
    }
...
int load_fmha_strided_library(const char* path) {
    ...
    if (!g_fmha_strided_lib) {
        fprintf(stderr, "[wLLM] Failed to load strided FMHA library: %s\n  %s\n",
                path, dyn_error());
        return -1;
    }
    g_fmha_strided_fn = (fmha_strided_fn_t)dyn_sym(g_fmha_strided_lib, "fmha_fp16_strided");
    if (!g_fmha_strided_fn) {
        fprintf(stderr, "[wLLM] Symbol 'fmha_fp16_strided' not found in %s\n", path);
        return -1;
    }
    return 0;
}
...
int fmha_forward(
    ...
    fprintf(stderr, "[wLLM] No FMHA backend loaded. "
...
int fmha_strided_forward(
    ...
    fprintf(stderr, "[wLLM] Strided FMHA not loaded.\n");
    return -1;
}
