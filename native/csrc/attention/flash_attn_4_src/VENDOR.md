...
- License:   BSD-3-Clause (see `wllm_fa4/cute/LICENSE`, `wllm_fa4/cute/AUTHORS`).
...
`wllm_native_kernels`. The runtime deps (`nvidia-cutlass-dsl`, `quack-kernels`)
ship via the `thor-fa4` pip extra.

## Private package name (isolation)

The package is renamed `flash_attn` → **`wllm_fa4`** so that importing it
...
`flash_attn.cute.*` imports were rewritten to `wllm_fa4.cute.*`. The loader
(`wllm_native/hardware/thor/fa4_backend.py`) adds this directory to `sys.path`
only transiently and imports `wllm_fa4.cute`.
...
1. **`wllm_fa4/cute/interface_fwd_sm100.py`** (renamed from `interface.py`):
...
2. **`wllm_fa4/cute/__init__.py`**: imports from `interface_fwd_sm100`
   instead of the (removed) `interface`.
3. Package rename `flash_attn` → `wllm_fa4` (isolation; see above).
...
`wllm_fa4/cute/` (27 `.py` plus `LICENSE`, `AUTHORS`, `.flake8`), the
`wllm_fa4/__init__.py` namespace shim, and this `VENDOR.md`. The SM100
...
3. Re-apply the three local patches (forward-only `interface_fwd_sm100.py`,
   `__init__.py` entry, `flash_attn`→`wllm_fa4` rename:
   `grep -rl 'flash_attn\.cute' | xargs sed -i 's/flash_attn\.cute/wllm_fa4.cute/g'`).
