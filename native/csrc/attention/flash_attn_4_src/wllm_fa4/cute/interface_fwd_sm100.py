...
from wllm_fa4.cute.cache_utils import get_jit_cache
from wllm_fa4.cute.testing import is_fake_mode


if os.environ.get("CUTE_DSL_PTXAS_PATH", None) is not None:
    from wllm_fa4.cute import cute_dsl_ptxas  # noqa: F401
...
from wllm_fa4.cute import fa_logging
from wllm_fa4.cute.cute_dsl_utils import (
    to_cute_tensor, to_cute_aux_tensor, get_aux_tensor_metadata, get_broadcast_dims,
)
from wllm_fa4.cute.flash_fwd_sm100 import FlashAttentionForwardSm100, DescaleTensors
from wllm_fa4.cute.flash_fwd_combine import FlashAttentionForwardCombine

# --- wLLM Thor vendor: forward / SM100-only trim -------------------------
...
def _fa4_trimmed(_name):
    def _raise(*_a, **_k):
        raise RuntimeError(
            f"{_name} was removed from wLLM's forward/SM100-only FA4 vendor. "
...
from wllm_fa4.cute.block_sparsity import (
...
class FlashAttnFunc(torch.autograd.Function):
    ...
    def backward(ctx, dout, dlse):
        raise NotImplementedError(
            "wLLM's FA4 vendor is forward / inference-only; the backward "
