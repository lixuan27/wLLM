...
static int run_case(ShapeCase sc) {
  ...
  int rc = wllm_native::fp4::quantize_fp4_dynamic_fp16(
      d_src, d_packed, d_scales, sc.N, sc.D, stream);
  if (rc) { fprintf(stderr, "quant rc=%d\n", rc); return 1; }
  rc = wllm_native::fp4::dequantize_fp4_to_fp16(
  ...
  for (int i = 0; i < 10; ++i)
    wllm_native::fp4::quantize_fp4_dynamic_fp16(d_src, d_packed, d_scales, sc.N, sc.D, stream);
  cudaStreamSynchronize(stream);
