// ================================================================
// wLLM — single-GEMM kernel using the vendored production
// sm100_gemm_tma_warpspecialized.hpp kernel struct (now
// wllm::megakernel::WllmRtMegakernelGemm).
//
...
// commits — the operator() body in wllm_megakernel_kernel.hpp
...
#include "wllm_megakernel_kernel.hpp"
...
// Note: the SFINAE enable_if_t inside WllmRtMegakernelGemm matches on
// CollectiveMainloop's Schedule tag (KernelTmaWarpSpecializedSm100),
// so any Sm100 TMA WS mainloop instantiation is accepted.
using GemmKernel = cutlass::gemm::kernel::WllmRtMegakernelGemm<
...
extern "C" int wllm_megakernel_single_fp16(
    ...
    cudaStream_t stream)
{
    ...
    if (gemm.can_implement(args) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_megakernel_single] cannot implement M=%d N=%d K=%d\n", M, N, K);
        return -1;
    }
    if (gemm.initialize(args, workspace.get(), stream) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_megakernel_single] init failed\n");
        return -2;
    }
    if (gemm.run(stream) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_megakernel_single] run failed\n");
        return -3;
    }
    return 0;
}
