// ============================================================================
// wLLM — encoder GeGLU megakernel (host-side launcher).
//
// The kernel (struct in wllm_megakernel_geglu_kernel.hpp) uses
...
#include "wllm_megakernel_geglu_kernel.hpp"
...
using GemmKernel = cutlass::gemm::kernel::WllmRtMegakernelGeGLUFusedGemm<
...
extern "C" int wllm_megakernel_geglu_fp16(
    ...
    cudaStream_t stream)
{
    ...
    if (gemm.can_implement(args) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_megakernel_geglu] cannot implement M=%d N=%d K=%d\n", M, N, K);
        return -1;
    }
    if (gemm.initialize(args, workspace.get(), stream) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_megakernel_geglu] init failed\n");
        return -2;
    }
    if (gemm.run(stream) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_megakernel_geglu] run failed\n");
        return -3;
    }
    return 0;
}
