// ============================================================================
// wLLM — encoder GeGLU + down-proj megakernel (host-side launchers).
//
// The GeGLU megakernel (kernel struct in wllm_megakernel_geglu_g8_kernel.hpp)
...
#include "wllm_megakernel_geglu_g8_kernel.hpp"
...
using FusionGate = wllm::megakernel::fusion::LinCombEltActSmemAuxStore<
    cutlass::epilogue::thread::GELU_taylor, fp16_t, float, fp16_t>;

using FusionUp = wllm::megakernel::fusion::LinCombDeEltActSmemAuxLoad<
...
using GemmKernel = cutlass::gemm::kernel::WllmRtMegakernelGeGLUG8FusedGemm<
...
extern "C" int wllm_rms_qkv_fp16(
    void* x,
    void* rms_weight,
    void* x_norm_scratch,
    void* qkv_weight,
...
// ============================================================================
// wllm_megakernel_geglu_g8_fp16 — fused GeGLU + down-proj entry.
//
...
// ============================================================================
extern "C" int wllm_megakernel_geglu_g8_fp16(
    ...
    cudaStream_t stream)
{
    ...
    if (gemm.can_implement(args) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_megakernel_geglu_g8] mega phase: cannot implement M=%d H=%d D=%d\n", M, H, D);
        return -1;
    }
    if (gemm.initialize(args, workspace.get(), stream) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_megakernel_geglu_g8] mega phase: init failed\n");
        return -2;
    }
    if (gemm.run(stream) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_megakernel_geglu_g8] mega phase: run failed\n");
        return -3;
    }
...
// ============================================================================
// wllm_encoder_ffn_block_fp16 — bundled FFN-block entry
...
// ============================================================================
extern "C" int wllm_encoder_ffn_block_fp16(
    void* x_resid,
    void* rms_weight,
    void* x_norm,
    ...
    cudaStream_t stream)
{
    ...
    if (gemm.can_implement(args) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_encoder_ffn_block] mega: cannot implement M=%d H=%d D=%d\n", M, H, D);
        return -1;
    }
    if (gemm.initialize(args, workspace.get(), stream) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_encoder_ffn_block] mega: init failed\n");
        return -2;
    }
    if (gemm.run(stream) != cutlass::Status::kSuccess) {
        fprintf(stderr, "[wllm_encoder_ffn_block] mega: run failed\n");
        return -3;
    }
