// ============================================================================
//  Standalone test for wllm_native::fp4::cutlass_fp4_sq_fp16.
//
...
static int run_case(ShapeCase sc, int iters) {
  ...
  // ── Run kernel ──
  int rc = wllm_native::fp4::cutlass_fp4_sq_fp16(
  ...
  for (int i = 0; i < 5; ++i) {  // warmup
    wllm_native::fp4::cutlass_fp4_sq_fp16(
        blkA.device_data(), blkSFA.device_data(),
        blkB.device_data(), blkSFB.device_data(),
...
int main(int argc, char** argv) {
  if (!wllm_native::fp4::has_nvfp4_sm110()) {
