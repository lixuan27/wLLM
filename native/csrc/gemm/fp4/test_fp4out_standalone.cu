...
int main() {
    ...
    int x_sfa_sz = wllm_native::fp4::sfa_size_bytes(M, K, false);
    int w_sfb_sz = wllm_native::fp4::sfa_size_bytes(N, K, true);
    CHECK(cudaMalloc(&X_sfa, x_sfa_sz));
    CHECK(cudaMalloc(&W_sfb, w_sfb_sz));
    wllm_native::fp4::quantize_fp4_dynamic_sfa_fp16(d_X, X_p, X_sfa, M, K, false, 0);
    wllm_native::fp4::quantize_fp4_dynamic_sfa_fp16(d_W, W_p, W_sfb, N, K, true,  0);
    cudaDeviceSynchronize();

    // ─ Path A: new fp4out GEMM ─
    uint8_t *D_p, *D_sfd;
    int d_sfd_sz = wllm_native::fp4::sfa_size_bytes(M, N, false);
    ...
    int rc = wllm_native::fp4::cutlass_fp4_gemm_fp4out(
    ...
    rc = wllm_native::fp4::cutlass_fp4_sq_fp16(X_p, X_sfa, W_p, W_sfb, D_fp16, M, N, K, 1.0f, 0.0f, 0);
    CHECK(cudaDeviceSynchronize());
    if (rc) { fprintf(stderr, "fp16 ref GEMM failed rc=0x%x\n", rc); return 1; }
    rc = wllm_native::fp4::quantize_fp4_dynamic_sfa_fp16(D_fp16, D_p_ref, D_sfa_ref, M, N, false, 0);
    CHECK(cudaDeviceSynchronize());
    ...
    for (int i = 0; i < warmup; ++i) {
        wllm_native::fp4::cutlass_fp4_gemm_fp4out(X_p, X_sfa, W_p, W_sfb, D_p, D_sfd, M, N, K, 0);
    }
    cudaDeviceSynchronize();
    cudaEventRecord(e1);
    ...
    for (int i = 0; i < warmup; ++i) {
        wllm_native::fp4::cutlass_fp4_sq_fp16(X_p, X_sfa, W_p, W_sfb, D_fp16, M, N, K, 1.0f, 0.0f, 0);
        wllm_native::fp4::quantize_fp4_dynamic_sfa_fp16(D_fp16, D_p_ref, D_sfa_ref, M, N, false, 0);
    }
    cudaDeviceSynchronize();
    cudaEventRecord(e1);
    for (int i = 0; i < iters; ++i) {
