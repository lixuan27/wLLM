...
int main() {
    ...
    int x_sfa_sz  = wllm_native::fp4::sfa_size_bytes(S, D, false);
    int wg_sfb_sz = wllm_native::fp4::sfa_size_bytes(H, D, true);
    ...
    wllm_native::fp4::quantize_fp4_dynamic_sfa_fp16(d_X,  X_p,  X_sfa,  S, D, false, 0);
    wllm_native::fp4::quantize_fp4_dynamic_sfa_fp16(d_Wg, Wg_p, Wg_sfb, H, D, true,  0);
    wllm_native::fp4::quantize_fp4_dynamic_sfa_fp16(d_Wu, Wu_p, Wu_sfb, H, D, true,  0);
    CHECK(cudaDeviceSynchronize());

    // ─ Run gate_proj fp4out + up_proj fp4out ─
    uint8_t *gate_p, *gate_sfa, *up_p, *up_sfa;
    int gu_sfa_sz = wllm_native::fp4::sfa_size_bytes(S, H, false);
    ...
    int rc1 = wllm_native::fp4::cutlass_fp4_gemm_fp4out(
        X_p, X_sfa, Wg_p, Wg_sfb, gate_p, gate_sfa, S, H, D, 0);
    int rc2 = wllm_native::fp4::cutlass_fp4_gemm_fp4out(
    ...
    for (int i = 0; i < warmup; ++i) {
      wllm_native::fp4::cutlass_fp4_gemm_fp4out(X_p, X_sfa, Wg_p, Wg_sfb, gate_p, gate_sfa, S, H, D, 0);
      wllm_native::fp4::cutlass_fp4_gemm_fp4out(X_p, X_sfa, Wu_p, Wu_sfb, up_p,   up_sfa,   S, H, D, 0);
      wllm_native::fused_fp4::silu_mul_two_fp4_to_fp4(gate_p, gate_sfa, up_p, up_sfa, out_p, out_sfa, S, H, 0);
    }
    cudaDeviceSynchronize();
    cudaEventRecord(e1);
    for (int i = 0; i < iters; ++i) {
