#!/usr/bin/env python3
"""MiniMax-Remover full-frame mask removal -- wLLM FP8 quickstart.

End-to-end demo of the wLLM-accelerated MiniMax-Remover video mask
...
This is a wLLM optimization of the upstream project:
    https://github.com/zibojia/MiniMax-Remover

It wraps a loaded diffusers MiniMax-Remover pipeline with
``wllm_native.models.minimax_remover.MiniMaxRemoverPipelineFP8`` (default) or
...
Build
------------------------------------------------------------------
wLLM must be built for Blackwell so the generic SM120 NVFP4 kernels
are compiled in (GPU_ARCH=120 / 121 auto-enables them). The VAE
fused fp16 kernels are opt-in (WLLM_ENABLE_MINIMAX_REMOVER=ON):
...
# Make the wllm_native package importable when run directly from the repo root.
...
# in the upstream source tree. wLLM then patches its forward path.
# =====================================================================

class AttnProcessor2_0:
    """Reference SDPA attention processor (replaced by wLLM at runtime)."""
...
class Transformer3DModel(ModelMixin, ConfigMixin):
    ...
    the wLLM pipeline onto NVFP4 GEMMs + fused kernels.
    """
...
# the reference flow-matching loop; wLLM replaces it at runtime with
...
class MinimaxRemoverPipeline(DiffusionPipeline):
    ...
    Uses FlowMatchEulerDiscreteScheduler: the wLLM manual denoise loop
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="MiniMax-Remover full-frame mask removal (wLLM FP8).")
    ...
    p.add_argument("--no-wllm", action="store_true",
                   help="Run the reference diffusers path instead of wLLM (for diffing).")
    ...
    p.add_argument("--vae-opt", action=argparse.BooleanOptionalAction,
                   default=True,
                   help="Apply wLLM VAE optimisations: fused fp16 RMS_norm "
...
def main() -> None:
    ...
    print("=" * 60)
    print("MiniMax-Remover full-frame mask removal (wLLM FP8 W8A8)")
    print("=" * 60)
    ...
    # NVFP4 VAE conv3d (default ON for wLLM FP8 path; --no-nvfp4-vae to disable)
    if args.vae_opt and not args.no_wllm and not args.use_fp4 and not args.no_nvfp4_vae:
