...
def _load(ckpt, cfg_ratio=0.05, bookend=False):
    ...
    print(f"[wLLM] Engine loaded: cfg_ratio={cfg_ratio} bookend={bookend}", flush=True)
    return _model
...
def _generate(text, vcp=None, **kwargs):
    ...
    dur = len(audio) / 24000
    print(f"[wLLM] {dur:.2f}s audio in {elapsed*1000:.0f}ms "
          f"(RTF={elapsed/dur:.4f}, speedup={dur/elapsed:.1f}x)", flush=True)
    return audio


def _build_parser():
    p = argparse.ArgumentParser(description="OmniVoice wLLM TTS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mode == "clone":
        ...
        from wllm.native.models.omnivoice import free_encoder
        free_encoder(_model)
    ...
    print(f"[wLLM] Saved: {args.output}", flush=True)
    return 0
