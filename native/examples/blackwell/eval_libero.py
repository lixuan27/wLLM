#!/usr/bin/env python3
"""
wLLM Blackwell — LIBERO benchmark pointer.

The unified ``wllm_native.load_model(...)`` API is hardware-agnostic, so
...
def main():
    parser = argparse.ArgumentParser(description="wLLM Blackwell LIBERO benchmark pointer")
    ...
    print("=" * 60)
    print(f"wLLM Blackwell — LIBERO {args.task_suite}")
    print("=" * 60)
    ...
    print("```python")
    print("import wllm_native")
    print()
    print("model = wllm_native.load_model(")
    print(f"    checkpoint=\"{args.checkpoint}\",")
