# wLLM on RTX 5090 (Blackwell, SM120)
...
# NOTE: pip install flash-attn is NOT required — wLLM vendors
# Flash-Attention 2 source and builds it into wllm_native_fa2.so during
...
# Build wllm_native_kernels module
mkdir build && cd build
cmake ..              # auto-detects SM120
make -j$(nproc)
make install          # installs .so → wllm_native/
cd ..
```
...
**`ModuleNotFoundError: wllm_native_fa2`**: build step skipped or `cp *.so ../wllm_native/` not run after `make`. See [`docs/INSTALL.md`](../../docs/INSTALL.md) §6.
