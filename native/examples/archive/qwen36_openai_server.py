#!/usr/bin/env python3
"""
wLLM — Qwen3.6-27B NVFP4 OpenAI-compatible HTTP server.

Serves the /v1/chat/completions endpoint backed by the wLLM NVFP4
...
Usage:
    ...
    export WLLM_QWEN36_MTP_CKPT_DIR=/path/to/qwen36_fp8_ckpt
    ...
    # WLLM_QWEN36_SERVER_LONG_WARMUP=all_graphs to also capture long
    # prefill chunk graphs at startup. 128K+ buckets warm every early
    # decode position by default; tune with
    # WLLM_QWEN36_LONG_WARMUP_STRIDE/MAX_GRAPHS. Add --warmup
...
class Qwen36Engine:
    ...
    def __init__(self, checkpoint: str, *, K: int, max_seq: int,
        ...
        if cap == (11, 0):
            from wllm.native.frontends.torch.qwen36_thor import (
        ...
        if self.fe._weights.ptrs.get('mtp') is None:
            log.warning(
                'MTP head not loaded (WLLM_QWEN36_MTP_CKPT_DIR '
...
def build_app(engine: Qwen36Engine):
    ...
    app = FastAPI(title='wLLM Qwen3.6 NVFP4 OpenAI-compatible server')
...
def _long_warmup_flags() -> Tuple[bool, bool]:
    ...
    mode = os.environ.get(
        'WLLM_QWEN36_SERVER_LONG_WARMUP', 'graphs').strip().lower()
    ...
    if mode in ('graphs', 'graph', 'decode', 'decode_graphs',
        ...
        return True, False
    sys.exit(
        'invalid WLLM_QWEN36_SERVER_LONG_WARMUP '
...
def main():
    ...
    if 'WLLM_QWEN36_MTP_CKPT_DIR' not in os.environ:
        log.warning(
            'WLLM_QWEN36_MTP_CKPT_DIR is not set — speculative '
