#!/usr/bin/env python3
"""Minimal OpenAI-compatible vision chat server for Qwen3-VL on wLLM.

Exposes ``POST /v1/chat/completions`` (non-streaming) for image+text
chat, backed by the wLLM Qwen3-VL NVFP4 path. Multimodal messages use
...
URLs). Single-GPU, batch 1; concurrent requests are serialised.
    ...
    python examples/qwen3_vl_openai_server.py \\
        --checkpoint /path/to/Qwen3-VL-8B-wLLM-NVFP4 --port 8000
...
def main() -> None:
    ...
    lock = asyncio.Lock()
    app = FastAPI(title='wLLM Qwen3-VL OpenAI-compatible server')
