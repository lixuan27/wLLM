...
This directory is the **policy layer** above the wLLM execution contract. It
...
- A CUDA GPU (developed on RTX 5090, sm_120) and the wLLM runtime built/installed
...
Do not set `WLLM_QWEN36_TQ_VERIFY_GRAPH=1` or
`WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH=1` for normal agent serving. The server
...
The response is an OpenAI `chat.completion` with an extra `wllm` block of serving
...
- Automatic exact token-prefix reuse for OpenAI-compatible coding-agent turns:
  ...
  wLLM session id is an optional native hint, not a protocol requirement.
...
resend full history are matched by token/content prefix. `wllm_session_id`
...
for high-concurrency batch serving, but the first wLLM agent target is one
...
state. Requests without `wllm_session_id` can still attach to that hot state
...
| `--default-K` | `4` | default speculative decode K for live requests; OpenAI-compatible clients inherit this unless they pass `wllm_K`; production agent sweeps currently favor K=4 |
...
request supplies `wllm_pin_prefix`, the server requires a positive capsule
...
benchmark path (`WLLM_QWEN36_DECODE_FASTGEMM=1` and
`WLLM_QWEN36_VERIFY_WARPSPLIT=1`). On GB10/SM121 Spark these SM120
...
`WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH=0` for agent serving, because exact-position
...
Use `--default-K` when a standard OpenAI-compatible client cannot pass wLLM
extension fields. Native clients may hot-switch per request with `wllm_K`.
...
- `cache_salt`: vLLM-style namespace separator; lower priority than
  `prompt_cache_key`.
- `wllm_session_id` (or `session_id`): optional native session-affinity hint.
- `wllm_cache_salt`: legacy wLLM namespace separator.
- `wllm_K`: speculative decode K for this request; overrides server
  `--default-K`.
- `enable_thinking`: passed to the Qwen chat template (default false).
- `wllm_pin_prefix`: pin this request's shared prefix as a capsule for reuse —
...
OpenAI-compatible clients do not need to pass wLLM extension fields for the
...
response carries a `wllm` telemetry block:
...
The standard `usage.prompt_tokens_details.cached_tokens` mirrors the wLLM
cached-token count so OpenAI-compatible tools can observe prefix-cache hits
without reading the `wllm` extension block.
...
lead; wLLM's target is latency-first single/few-session work on consumer/edge
...
export WLLM_QWEN36_MTP_CKPT_DIR=/path/to/qwen36_mtp_ckpt
export WLLM_QWEN36_LONG_KV_CACHE=fp8
export WLLM_QWEN36_TQ_VERIFY_GRAPH=1
export WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH=1
python -m serving.qwen36_agent.server \
  --checkpoint /path/to/qwen36_nvfp4 \
  --max-seq 32768 \
  --route-min-seq 0 \
...
```bash
# turn 1 (cold): wllm.cached_tokens == 0
...
# wllm.cached_tokens > 0, prefix_action == "append" / "message_append"
curl -s :8000/v1/chat/completions -d '{"model":"qwen36-27b",
...
re-prefills only the suffix. This is wLLM's graph-replay-native prefix reuse
...
curl -s :8000/v1/chat/completions -d '{"model":"qwen36-27b",
 ...
 "wllm_pin_prefix": 6000, "max_tokens": 256}'
# -> wllm.prefix_action == "pin"
...
curl -s :8000/v1/chat/completions -d '{"model":"qwen36-27b",
 ...
 "wllm_pin_prefix": 6000, "max_tokens": 256}'
# -> wllm.prefix_action == "restore", cached_tokens == the chunk-aligned
#    boundary, new_prefill_tokens == only the suffix after it
```

Semantics and bounds:

- `wllm_pin_prefix` is an int (pin that many leading prompt tokens) or `true`
...
```bash
WLLM_QWEN36_NVFP4_CKPT_DIR=CHECKPOINT_DIR \
WLLM_QWEN36_MTP_CKPT_DIR=MTP_CHECKPOINT_DIR \
