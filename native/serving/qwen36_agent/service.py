...
class AgentService:
    ...
    def _capsule_prefill(
        ...
        if not self.capsules.enabled:
            raise ValueError(
                "wllm_pin_prefix requires --capsule-budget-mb > 0")
        supports = getattr(self.engine, "supports_capsule", None)
        if not callable(supports) or not supports():
            raise ValueError(
                "wllm_pin_prefix requires a capsule-capable Qwen3.6 engine")
        ...
        if aligned <= 0 or aligned > prompt_len:
            raise ValueError(
                "wllm_pin_prefix requires the long FP8-KV route and a "
    ...
    def _effective_k(req: AgentRequest) -> int:
        k = int(req.K)
        if k < 1:
            raise ValueError("wllm_K must be >= 1")
        return k
...
def parse_pin_prefix(value: Any) -> Optional[int]:
    """Parse ``wllm_pin_prefix``: a positive int (pin that many leading prompt
    ...
    try:
        ...
        raise ValueError(
            f"wllm_pin_prefix must be an integer or boolean, got {value!r}")
...
def request_from_openai(req: Dict[str, Any], *, default_k: int = 4,
    ...
    K = parse_int(req.get("wllm_K"), name="wllm_K", default=default_k)
    if K < 1:
        raise ValueError("wllm_K must be >= 1")
    return AgentRequest(
        messages=messages,
        ...
        session_id=req.get("wllm_session_id") or req.get("session_id"),
        cache_salt=str(
            req.get("prompt_cache_key")
            or req.get("cache_salt")
            or req.get("wllm_cache_salt", "")
        ),
        enable_thinking=parse_bool(req.get("enable_thinking"), default=False),
        K=K,
        pin_prefix=parse_pin_prefix(req.get("wllm_pin_prefix")),
    )
