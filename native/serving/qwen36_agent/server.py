...
def build_app(service: AgentService):
    ...
    app = FastAPI(title="wLLM Qwen3.6 Agent Serving")
...
def create_app_from_checkpoint(*, checkpoint: str,
    ...
    if not engine.spec_enabled:
        log.warning(
            ...
            "runs the slower non-spec path. Set WLLM_QWEN36_MTP_CKPT_DIR to "
    ...
    log.info(
        ...
        os.environ.get("WLLM_QWEN36_TQ_VERIFY_GRAPH", "<unset>"),
        os.environ.get("WLLM_QWEN36_TQ_MTP_CHAIN_GRAPH", "<unset>"),
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        description="wLLM Qwen3.6 agent-serving OpenAI API")
    ...
    parser.add_argument(
        ...
        help="Default speculative decode K for live requests. Requests may "
             "override it with the wLLM extension field wllm_K.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="GPU byte budget (MB) for pinned shared-prefix capsules. 0 (default) "
             "disables pinning. When >0, a request with wllm_pin_prefix pins "
