...
TTS inference server for OmniVoice on wLLM with BF16 + FP4 hybrid
acceleration.

This directory is the policy layer above the wLLM execution contract.
It owns the FastAPI HTTP surface, WAV framing, and request serialisation.
The model is driven through `wllm_native.models.omnivoice.inject()` which
patches OmniVoice for wLLM acceleration.

## What runs where

```
serving/omnivoice_agent/server.py          policy: HTTP, audio framing, serialisation
  └─ wllm_native.models.omnivoice.inject()    acceleration: BF16+FP4 hybrid MaskGIT loop
       ├─ wLLMLlmBF16                    BF16 CFG forward (step 1)
       └─ wLLMLlm                        FP4 noCFG forward (steps 2-32)
```

Single stream: TTS requests are serialised (one model instance). The engine
uses CUDA Graph for FP4 forward and fused wLLM kernels throughout.
...
- `WLLM_ENABLE_OMNIVOICE=ON` must be set at cmake build time to compile
  the `wllm_native_omnivoice` module (`omnivoice_cfg_logsoftmax_bf16` and
