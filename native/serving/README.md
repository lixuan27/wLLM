# serving/ — scenario hosts on the wLLM execution contract

The **serving layer**: native hosts that drive the execution contract
(`libwllm_exec`, see [`docs/exec_contract.md`](../docs/exec_contract.md)) for a
...
```
  wllm_native/  (Python frontend)  setup: weights · calibration · autotune · capture · adopt   [cold, once]
...
link the *same* `libwllm_exec`; the host language is chosen per scenario.
