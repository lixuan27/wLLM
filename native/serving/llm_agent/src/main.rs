//! Illustrative Rust LLM session host on the wLLM execution contract.
//!
...
//! same process, or ported to native. The `wrt_*` handles below are produced
//! by that setup. Reference structure, not a standalone runnable binary.

use std::os::raw::{c_char, c_int};

// ── Minimal FFI to the C-ABI subset (see exec/include/wllmrt/exec.h) ──
#[repr(C)] pub struct FrtCtx { _p: [u8; 0] }
#[repr(C)] pub struct FrtGraph { _p: [u8; 0] }
#[repr(C)] pub struct FrtBuffer { _p: [u8; 0] }

extern "C" {
    fn wrt_ctx_create() -> *mut FrtCtx;
    fn wrt_ctx_wrap_stream(ctx: *mut FrtCtx, external_stream: *mut std::ffi::c_void) -> c_int;
    fn wrt_buffer_dptr(b: *mut FrtBuffer) -> *mut std::ffi::c_void;
    fn wrt_graph_create(ctx: *mut FrtCtx, name: *const c_char, max_variants: usize) -> *mut FrtGraph;
    fn wrt_graph_adopt(g: *mut FrtGraph, key: u64, external_graph_exec: *mut std::ffi::c_void) -> c_int;
    fn wrt_graph_replay(g: *mut FrtGraph, key: u64, stream_id: c_int) -> c_int;
}

const WRT_OK: c_int = 0;
...
pub unsafe fn decode_step(h: &LlmHandles, cur_pos: u64, tok: i64) -> Result<*mut std::ffi::c_void, c_int> {
    ...
    let _dst = wrt_buffer_dptr(h.token_in);
    let _ = tok;
    let rc = wrt_graph_replay(h.decode, cur_pos, h.stream_id); // == cudaGraphLaunch
    if rc != WRT_OK { return Err(rc); }
    Ok(wrt_buffer_dptr(h.logits)) // host reads logits, argmax -> next token
}
...
fn main() {
    eprintln!("wllm llm_agent: skeleton — setup (capture+adopt) must build LlmHandles first.");
    let _ = (wrt_ctx_create as usize, wrt_ctx_wrap_stream as usize, wrt_graph_create as usize,
             wrt_graph_adopt as usize); // reference the FFI so the linker keeps it
}
