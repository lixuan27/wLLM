...
OpenAI-compatible ``POST /v1/audio/speech`` over the wLLM Higgs Audio v3
...
def build_app(frontend, model_name: str):
    ...
    app = FastAPI(title="wLLM Higgs Audio v3 TTS")
    lock = threading.Lock()
...
def main():
    ...
    print(f"wLLM Higgs TTS ready: http://{args.host}:{args.port}  "
