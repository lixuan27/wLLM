...
#
# Default cache: ~/.cache/wllm_native/paligemma_tokenizer.model
# Override:      $WLLM_PALIGEMMA_TOKENIZER (set to a target path)
#
# Usage:
#     bash scripts/download_paligemma_tokenizer.sh
#     # or, custom location:
#     WLLM_PALIGEMMA_TOKENIZER=/data/paligemma.model \
#         bash scripts/download_paligemma_tokenizer.sh

set -euo pipefail

URL="https://storage.googleapis.com/big_vision/paligemma_tokenizer.model"
DEST="${WLLM_PALIGEMMA_TOKENIZER:-$HOME/.cache/wllm_native/paligemma_tokenizer.model}"
