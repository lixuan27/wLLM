...
The fastest path to a working wLLM install. One image, one
...
`wllm_native_fa2.so`. Everything else builds the same way. Skip to
...
> `flash_vla → wllm_native` package-rename refactor that landed in
> [#6](https://github.com/LiangSu8899/wLLM/pull/6), the
> `ghcr.io/liangsu8899/wllm` image has not been re-pushed yet —
...
```bash
docker tag wllm:5090 <your-registry>/wllm:0.2.0
docker push <your-registry>/wllm:0.2.0
```
...
image = modal.Image.from_registry(
    "<your-registry>/wllm:0.2.0"
).pip_install("your-app-deps")

app = modal.App("wllm-app", image=image)

@app.function(gpu="L40S")  # or H100, A100, etc.
def infer():
    import wllm_native
    model = wllm_native.load_model(checkpoint="/path/to/ckpt", framework="torch")
    ...
```

Once `ghcr.io/liangsu8899/wllm:<tag>` is re-pushed, swap in the
...
```bash
# Default: drops you in a Python REPL with `wllm_native` already imported.
docker run --rm --gpus all -it wllm:dev
...
- wLLM source at `/workspace/wLLM`, editable-installed
- Kernel `.so` files prebuilt directly into `wllm_native/`. The exact
  ...
  |---|---|---|---|---|
  | `wllm_native_kernels.so`        | ✓ | — | — | — |
  | `wllm_native_jax_ffi.so`        | ✓ | — | — | — |
  | `wllm_native_fp4.so`            | ✓ | — | NVFP4 paths active here | — |
  | `wllm_native_fa2.so`            | — | ✓ | — | skipped |
  | `libfmha_fp16_strided.so`    | — | skipped | — | ✓ |

  In the default x86 build (auto-detected sm_120 on RTX 5090) you
  get 4 `.so` files: `wllm_native_kernels`, `wllm_native_fa2`,
  `wllm_native_fp4`, `wllm_native_jax_ffi`. The Thor build (sm_110) also
  produces 4 but swaps `wllm_native_fa2` for `libfmha_fp16_strided`.
...
pip wheel — the default RTX path uses the vendored `wllm_native_fa2.so`
and works without it. If you need legacy upstream attention or run
GROOT, install it yourself:

```bash
docker run --rm --gpus all wllm:dev \
...
```bash
# On the Thor host
docker build -t wllm:thor -f docker/Dockerfile.thor .

# Run (note --runtime=nvidia for Jetson — see below for why)
docker run --rm --gpus all -it --runtime=nvidia wllm:thor
```
...
- **Build targets**: 4 `.so` files (`wllm_native_kernels`,
  `wllm_native_fp4`, `libfmha_fp16_strided`, `wllm_native_jax_ffi`).
  Same artifact count as the default x86 build but with
  `libfmha_fp16_strided` swapped in for `wllm_native_fa2`.
- **No `wllm_native_fa2.so`**: Thor's `csrc/attention/fmha_dispatch.cu`
...
- **`wllm_native_fp4.so` on Thor**: built for sm_110a, but NVFP4 GEMM
...
is present and does NOT import `wllm_native_fa2`, so a future regression
