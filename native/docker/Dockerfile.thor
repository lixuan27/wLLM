# wLLM — Thor image (Jetson AGX Thor, SM110, aarch64).
#
...
#   wllm_native/wllm_native_kernels.cpython-312-aarch64-linux-gnu.so
#   wllm_native/wllm_native_fp4.cpython-312-aarch64-linux-gnu.so
#   wllm_native/libfmha_fp16_strided.so
#   wllm_native/wllm_native_jax_ffi.so
# No wllm_native_fa2.so on Thor — that's expected, not a build failure.
#
...
# if needed). All four .so targets land directly under wllm_native/ thanks
...
# ── Editable install — registers wllm_native with the system Python so
#    `python -c "import wllm_native"` works from any working directory. ──
...
#    wllm_native_fa2.so (FA2 is SM80/89/120-only); the smoke deliberately
#    does NOT import it. ──
RUN python3 -c "import wllm_native; \
print('wllm_native', wllm_native.__version__); \
from wllm_native import wllm_native_kernels; \
print('wllm_native_kernels symbols:', len([s for s in dir(wllm_native_kernels) if not s.startswith('_')])); \
import os; \
assert os.path.exists(os.path.join(os.path.dirname(wllm_native.__file__), 'libfmha_fp16_strided.so')), 'libfmha_fp16_strided.so missing'; \
print('Thor build OK — kernels + fmha_fp16_strided + fp4 + jax_ffi present')"

# Default to a Python REPL with wllm_native pre-imported. Override with
#   docker run --rm --gpus all -it --runtime=nvidia wllm:thor <cmd>
CMD ["python3", "-c", "import wllm_native; print('wllm_native', wllm_native.__version__, '— Thor (SM110) build ready'); import code; code.interact(local={'wllm_native': wllm_native})"]
