...
each ``β ∈ {1.0, 1.5, 2.0, 2.5}``:
  ...
  * ``serial_actions_b{β}``     — wLLM serial CFG final actions (10, 7)
  * ``serial_v_cond_b{β}``      — wLLM serial per-step v_cond traces
                                   (num_steps, action_horizon, action_dim)
  * ``serial_v_uncond_b{β}``
  * ``serial_noise_b{β}``

  * ``batched_actions_b{β}``    — wLLM batched CFG final actions
  * ``batched_v_cond_b{β}``
...
Run from the wLLM repo root inside the pi0-stablehlo container::
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for beta in BETAS:
            ...
            print("  wLLM serial ... ", end=" ", flush=True)
            ...
            print("  wLLM batched ...", end=" ", flush=True)
