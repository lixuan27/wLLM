/* wLLM Runtime Export — public C ABI (v1).
 *
 * The single hand-off surface between a wLLM model runtime (the PRODUCER:
 ...
 * model code — it sees only exec-contract handles (wllmrt/exec.h) and this
 * struct.
 *
 * Two producers, one struct (the point of this layer):
 *   today : Python setup/capture fills the struct in-process
 *           (wllm_native/runtime/export.py over the builder below)
 *   later : a native C++ model runtime .so exports WRT_RUNTIME_OPEN_V1_SYMBOL
 ...
 */
#ifndef WLLM_RUNTIME_H
#define WLLM_RUNTIME_H
...
#define WRT_RUNTIME_ABI_VERSION 1u
...
enum wrt_runtime_role {
    WRT_RT_ROLE_INPUT   = 1u << 0,
    WRT_RT_ROLE_OUTPUT  = 1u << 1,
    WRT_RT_ROLE_STATE   = 1u << 2,
    WRT_RT_ROLE_SCRATCH = 1u << 3
};
 ...
 * position. */
enum wrt_runtime_region_flags {
    WRT_RT_REGION_SNAPSHOT = 1u << 0,
    WRT_RT_REGION_RESTORE  = 1u << 1
};
...
typedef struct wrt_runtime_stream_desc {
    const char* name;      /* e.g. "main", "vision", "asr"                    */
    int   stream_id;       /* wrt_ctx-scoped id (usable with wrt_graph_replay) */
...
} wrt_runtime_stream_desc;

typedef struct wrt_runtime_graph_desc {
    const char* name;            /* e.g. "infer", "decode_only", "preprocess" */
    wrt_graph   handle;
    wrt_shape_key default_key;   /* the key a plain host fires                */
    const wrt_shape_key* keys;   /* captured variant keys at export time      */
    uint64_t    n_keys;          /* (discovery aid; the live table may grow — */
                                 /*  wrt_graph_has_variant is the truth)      */
    int         stream_id;       /* default replay stream (host may override) */
} wrt_runtime_graph_desc;

typedef struct wrt_runtime_buffer_desc {
    const char* name;
    wrt_buffer  handle;
    uint64_t    bytes;
    uint32_t    role;            /* wrt_runtime_role bitmask                  */
    uint32_t    reserved;        /* zero for v1                               */
} wrt_runtime_buffer_desc;
 ...
 * (and fingerprinted). */
typedef struct wrt_runtime_region_desc {
    const char* name;
    wrt_buffer  buffer;
    uint64_t    offset;
    uint64_t    bytes;
    uint32_t    flags;           /* wrt_runtime_region_flags bitmask          */
    uint32_t    reserved;        /* zero for v1                               */
} wrt_runtime_region_desc;

/* ------------------------------------------------------------------ */
/* The export object.                                                  */
/* ------------------------------------------------------------------ */
typedef struct wrt_runtime_export_v1 {
    uint32_t abi_version;        /* = WRT_RUNTIME_ABI_VERSION                 */
    uint32_t struct_size;        /* = sizeof(wrt_runtime_export_v1)           */

    wrt_ctx ctx;                 /* the exec context everything lives in      */

    const wrt_runtime_stream_desc* streams;         uint64_t n_streams;
    const wrt_runtime_graph_desc*  graphs;          uint64_t n_graphs;
    const wrt_runtime_buffer_desc* buffers;         uint64_t n_buffers;
    const wrt_runtime_region_desc* capsule_regions; uint64_t n_capsule_regions;
     ...
     *                 ONLY by wrt_runtime_builder_finish — one implementation,
     ...
     *   fingerprint : FNV-1a 64 of `identity` (wrt_runtime_fingerprint).
 ...
 *
 *   int wrt_runtime_open_v1(const char* config_json,
 *                           wrt_runtime_export_v1** out);
 *   -> 0 on success (out points to a retained export; caller must release),
 *      negative on failure.
 */
#define WRT_RUNTIME_OPEN_V1_SYMBOL "wrt_runtime_open_v1"
typedef int (*wrt_runtime_open_v1_fn)(const char* config_json,
                                      wrt_runtime_export_v1** out);

/* ------------------------------------------------------------------ */
/* Builder — the producer-side helper (libwllm_runtime).            */
...
/* ------------------------------------------------------------------ */
typedef struct wrt_runtime_builder_s* wrt_runtime_builder;

wrt_runtime_builder wrt_runtime_builder_create(wrt_ctx);
...
int wrt_runtime_builder_add_stream(wrt_runtime_builder, const char* name,
                                   int stream_id, int priority,
                                   void* native_handle);
int wrt_runtime_builder_add_graph (wrt_runtime_builder, const char* name,
                                   wrt_graph, wrt_shape_key default_key,
                                   const wrt_shape_key* keys, uint64_t n_keys,
                                   int stream_id);
int wrt_runtime_builder_add_buffer(wrt_runtime_builder, const char* name,
                                   wrt_buffer, uint64_t bytes, uint32_t role);
int wrt_runtime_builder_add_region(wrt_runtime_builder, const char* name,
                                   wrt_buffer, uint64_t offset, uint64_t bytes,
                                   uint32_t flags);
...
int wrt_runtime_builder_add_identity(wrt_runtime_builder, const char* key,
                                     const char* value);
int wrt_runtime_builder_set_manifest(wrt_runtime_builder, const char* json);
...
wrt_runtime_export_v1* wrt_runtime_builder_finish(wrt_runtime_builder,
                                                  void* owner,
...
uint64_t wrt_runtime_fingerprint(const void* data, size_t len);

#ifdef __cplusplus
}  /* extern "C" */
#endif

#endif /* WLLM_RUNTIME_H */
