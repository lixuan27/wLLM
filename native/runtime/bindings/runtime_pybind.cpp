/* wLLM runtime — pybind module (`_wllm_runtime`).
 *
 * Setup/dev bridge only: lets the Python frontend (phase-1 producer) assemble
 * an wrt_runtime_export_v1 from raw exec handles. The struct itself is the
...
int verb_prepare(void* self, uint32_t graph, wrt_shape_key key) {
    auto* v = static_cast<PyVerbs*>(self);
    py::gil_scoped_acquire gil;
...
struct PyRtBuilder {
    ...
    explicit PyRtBuilder(std::uintptr_t ctx_raw) {
        b = wrt_runtime_builder_create(reinterpret_cast<wrt_ctx>(ctx_raw));
        if (!b) throw std::runtime_error("wrt_runtime_builder_create failed (null ctx?)");
    }
    PyRtBuilder() {
        b = wrt_model_runtime_builder_create_metadata();
        if (!b) throw std::runtime_error(
            "wrt_model_runtime_builder_create_metadata failed");
    }
    ~PyRtBuilder() {
    ...
    std::uintptr_t finish(py::object owner) {
        ...
        wrt_runtime_export_v1* e = wrt_runtime_builder_finish(
    ...
    std::uintptr_t finish_model(py::object owner, py::object set_input,
        ...
        wrt_model_runtime_v1* mr = wrt_runtime_builder_finish_model(
...
const wrt_generic_stage_plan_ext_v1* generic_plan(wrt_model_runtime_v1* model) {
    if (model->struct_size < WRT_MODEL_RUNTIME_V1_QUERY_EXTENSION_SIZE ||
        !model->query_extension) return nullptr;
    const void* extension = nullptr;
    if (model->query_extension(model, WRT_EXT_GENERIC_STAGE_PLAN_V1, 1,
...
wrt_runtime_export_v1* as_export(std::uintptr_t p) {
    auto* e = reinterpret_cast<wrt_runtime_export_v1*>(p);
    if (!e || e->abi_version != WRT_RUNTIME_ABI_VERSION ||
        e->struct_size != sizeof(wrt_runtime_export_v1))
        throw std::runtime_error("not a valid wrt_runtime_export_v1 pointer");
    return e;
}

wrt_model_runtime_v1* as_model(std::uintptr_t p) {
    auto* m = reinterpret_cast<wrt_model_runtime_v1*>(p);
    if (!m || m->abi_version != WRT_MODEL_RUNTIME_ABI_VERSION ||
        m->struct_size < WRT_MODEL_RUNTIME_V1_BASE_SIZE)
        throw std::runtime_error("not a valid wrt_model_runtime_v1 pointer");
    return m;
}

}  // namespace

PYBIND11_MODULE(_wllm_runtime, m) {
    m.doc() = "wLLM runtime-export ABI (setup/dev binding)";

    m.attr("ABI_VERSION") = WRT_RUNTIME_ABI_VERSION;
    m.attr("ROLE_INPUT") = (unsigned)WRT_RT_ROLE_INPUT;
    m.attr("ROLE_OUTPUT") = (unsigned)WRT_RT_ROLE_OUTPUT;
    m.attr("ROLE_STATE") = (unsigned)WRT_RT_ROLE_STATE;
    m.attr("ROLE_SCRATCH") = (unsigned)WRT_RT_ROLE_SCRATCH;
    m.attr("REGION_SNAPSHOT") = (unsigned)WRT_RT_REGION_SNAPSHOT;
    m.attr("REGION_RESTORE") = (unsigned)WRT_RT_REGION_RESTORE;

    m.attr("MODEL_ABI_VERSION") = WRT_MODEL_RUNTIME_ABI_VERSION;
    m.attr("MODEL_V1_BASE_SIZE") = WRT_MODEL_RUNTIME_V1_BASE_SIZE;
    m.attr("MODEL_V1_QUERY_EXTENSION_SIZE") =
        WRT_MODEL_RUNTIME_V1_QUERY_EXTENSION_SIZE;
    m.attr("MOD_TENSOR") = (unsigned)WRT_RT_MOD_TENSOR;
    m.attr("MOD_IMAGE") = (unsigned)WRT_RT_MOD_IMAGE;
    m.attr("MOD_TEXT") = (unsigned)WRT_RT_MOD_TEXT;
    m.attr("MOD_STATE") = (unsigned)WRT_RT_MOD_STATE;
    m.attr("MOD_ACTION") = (unsigned)WRT_RT_MOD_ACTION;
    m.attr("MOD_AUDIO") = (unsigned)WRT_RT_MOD_AUDIO;
    m.attr("MOD_DEPTH") = (unsigned)WRT_RT_MOD_DEPTH;
    m.attr("MOD_FORCE") = (unsigned)WRT_RT_MOD_FORCE;
    m.attr("DTYPE_U8") = (unsigned)WRT_RT_DTYPE_U8;
    m.attr("DTYPE_F32") = (unsigned)WRT_RT_DTYPE_F32;
    m.attr("DTYPE_F16") = (unsigned)WRT_RT_DTYPE_F16;
    m.attr("DTYPE_BF16") = (unsigned)WRT_RT_DTYPE_BF16;
    m.attr("DTYPE_I32") = (unsigned)WRT_RT_DTYPE_I32;
    m.attr("DTYPE_I64") = (unsigned)WRT_RT_DTYPE_I64;
    m.attr("LAYOUT_FLAT") = (unsigned)WRT_RT_LAYOUT_FLAT;
    m.attr("LAYOUT_HWC") = (unsigned)WRT_RT_LAYOUT_HWC;
    m.attr("LAYOUT_NHWC") = (unsigned)WRT_RT_LAYOUT_NHWC;
    m.attr("LAYOUT_CHW") = (unsigned)WRT_RT_LAYOUT_CHW;
    m.attr("LAYOUT_NCHW") = (unsigned)WRT_RT_LAYOUT_NCHW;
    m.attr("PORT_IN") = (unsigned)WRT_RT_PORT_IN;
    m.attr("PORT_OUT") = (unsigned)WRT_RT_PORT_OUT;
    m.attr("PORT_SWAP") = (unsigned)WRT_RT_PORT_SWAP;
    m.attr("PORT_STAGED") = (unsigned)WRT_RT_PORT_STAGED;
    m.attr("PORT_SETUP") = (unsigned)WRT_RT_PORT_SETUP;
    m.attr("GENERIC_STAGE_GRAPH") = (unsigned)WRT_GENERIC_STAGE_GRAPH;
    m.attr("GENERIC_STAGE_OPAQUE") = (unsigned)WRT_GENERIC_STAGE_OPAQUE;
    m.attr("GENERIC_STAGE_NAME_MAX_BYTES") =
        (unsigned)WRT_GENERIC_STAGE_NAME_MAX_BYTES;
    m.attr("GENERIC_STAGE_DESC_V1_SIZE") =
        (unsigned)sizeof(wrt_generic_stage_desc_v1);
    m.attr("GENERIC_STAGE_PLAN_EXT_V1_SIZE") =
        (unsigned)WRT_GENERIC_STAGE_PLAN_EXT_V1_SIZE;
    m.attr("EXT_GENERIC_STAGE_PLAN_V1") = WRT_EXT_GENERIC_STAGE_PLAN_V1;
    ...
    py::class_<PyRtBuilder>(m, "Builder")
        ...
        .def("add_stream", [](PyRtBuilder& s, const std::string& name, int stream_id,
            ...
            s.need();
            check(wrt_runtime_builder_add_stream(s.b, name.c_str(), stream_id, priority,
        ...
        .def("add_graph", [](PyRtBuilder& s, const std::string& name, std::uintptr_t graph_raw,
                             ...
                             int stream_id) {
            s.need();
            check(wrt_runtime_builder_add_graph(s.b, name.c_str(),
                                                reinterpret_cast<wrt_graph>(graph_raw),
        ...
        .def("add_buffer", [](PyRtBuilder& s, const std::string& name,
            ...
            s.need();
            check(wrt_runtime_builder_add_buffer(s.b, name.c_str(),
                                                 reinterpret_cast<wrt_buffer>(buffer_raw),
                                                 bytes, role),
                  "add_buffer");
        ...
        .def("add_region", [](PyRtBuilder& s, const std::string& name,
            ...
            s.need();
            check(wrt_runtime_builder_add_region(s.b, name.c_str(),
                                                 reinterpret_cast<wrt_buffer>(buffer_raw),
                                                 offset, bytes, flags),
                  "add_region");
        ...
        .def("add_identity", [](PyRtBuilder& s, const std::string& k, const std::string& v) {
            s.need();
            check(wrt_runtime_builder_add_identity(s.b, k.c_str(), v.c_str()), "add_identity");
        })
        .def("set_manifest", [](PyRtBuilder& s, const std::string& json) {
            s.need();
            check(wrt_runtime_builder_set_manifest(s.b, json.c_str()), "set_manifest");
        })
        .def("add_port", [](PyRtBuilder& s, const std::string& name, unsigned modality,
            ...
            check(wrt_runtime_builder_add_port(
                      ...
                      cadence_hint_hz, reinterpret_cast<wrt_buffer>(buffer_raw),
                      offset, bytes),
                  "add_port");
        ...
        .def("add_stage", [](PyRtBuilder& s, unsigned graph,
            ...
            s.need();
            check(wrt_runtime_builder_add_stage(s.b, graph, after.data(),
                                                (uint32_t)after.size()),
                  "add_stage");
        ...
        .def("finish_model", &PyRtBuilder::finish_model, py::arg("owner"),
             ...
             "Consume the builder; returns the wrt_model_runtime_v1 pointer "
    ...
    m.def("fingerprint", [](py::bytes data) {
        std::string s = data;
        return wrt_runtime_fingerprint(s.data(), s.size());
    ...
    m.def("model_ports", [](std::uintptr_t p) {
        ...
        for (std::uint64_t i = 0; i < mr->n_ports; ++i) {
            const wrt_runtime_port_desc& d = mr->ports[i];
            py::dict e;
    ...
    m.def("model_stages", [](std::uintptr_t p) {
        ...
        for (std::uint64_t i = 0; i < mr->n_stages; ++i) {
            const wrt_runtime_stage_desc& d = mr->stages[i];
            py::dict e;
            e["graph"] = d.graph;
    ...
    m.def("model_generic_stages", [](std::uintptr_t p) {
        ...
        for (std::uint64_t i = 0; i < plan->n_stages; ++i) {
            const wrt_generic_stage_desc_v1& d = plan->stages[i];
            py::dict e;
            e["name"] = std::string(d.name);
