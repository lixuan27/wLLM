...
| Knife-edge token tie promoted by the measurement path that happens to agree | AR exact gate goes green | **Mitigated, no longer merely documented.** The adjudication forward used to be a teacher-forced *prefill* while generation runs on the *decode* path; in bf16 the two are not bitwise identical, so a tie measured at prefill need not be the tie that occurred (observed live as a "gap 0.0000 but not a tie" three-way degenerate case). `wllm/verify/adjudicate.py` now measures the disputed logit row on BOTH paths and returns `undecidable` — a refusal — whenever they disagree; a decode replay that cannot run is disclosed in `notes` and the verdict falls back to the older, weaker single-path rule, never to a stronger claim. Pinned by `tests/test_adjudicate.py::test_dual_path_disagreement_is_undecidable_not_a_pass` | Built (CPU-verified; per-checkpoint bf16 agreement is a GPU measurement) |
